...
def _resolve_docs_dir() -> str:
    ...
    while True:
        ...
        if parent == current:
            break
        current = parent
    usage_fail("docs/solutions/ directory not found")


def _check_doc_path(doc_path: str) -> None:
    real = os.path.realpath(doc_path)
    docs_dir = _resolve_docs_dir()
    if not (real.startswith(docs_dir + os.sep) or real == docs_dir):
        usage_fail(f"path must be under docs/solutions/: {doc_path}")
