...
- **Duplicating the same check at every layer.** Each layer should catch a distinct class of failure.
...
- **Leaving layer 4 (diagnostic breadcrumb) out.** When layers 1-3 still get bypassed, the breadcrumb is what makes the next bug debuggable.
