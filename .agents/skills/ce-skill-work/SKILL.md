...
- **The description is a context pointer.** It front-loads the leading word that should fire the skill, then lists one trigger per genuinely distinct branch in "Use when the user wants..., mentions..." form. No identity boilerplate ("This skill should be used when..."), synonym/catalog runs, workflow, flags, procedure, or body duplicate; adjacent negatives stay only as hard false-trigger guardrails.
