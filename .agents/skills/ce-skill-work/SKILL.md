...
- **Autonomy is one compact policy.** Name safe local actions, let them proceed in scope, and confirm only for external writes, destructive actions, purchases, or material scope expansion. Do not repeat "ask first", "do not mutate", or "wait for approval" at each step.
