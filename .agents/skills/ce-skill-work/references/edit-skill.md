...
- **Then delete the copy the body kept.** Relocating verbatim leaves the same gate stated twice, because the body keeps its own copy of what it owns; that is the intended half-step, not the finished move. Before the eval, diff each reference against the body and delete every gate, condition, or confirmation the body still owns — a second copy asks the user twice, records a snapshot twice, and drifts on the next edit, when only one of the two gets fixed.
