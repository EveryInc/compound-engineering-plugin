...
For each finding: file and block, class, the evidence its class requires, and the requested fix stated as a condition or a move. Lead with Change items; list Verify and Consider separately. End with the paths you checked that the restatement (if any) still serves.
