...
- **Scratch Space:** Default to OS temp. Use `.context/` only when explicitly justified by the rules below. Prefer paths that remain writable under sandboxed hosts (notably Claude Code on macOS, which sets `TMPDIR=/tmp/claude-*` and allowlists that directory for writes).
  - **Default: OS temp** — covers most scratch, including per-run throwaway AND cross-invocation reusable, regardless of whether a repo is present or whether other skills may read the files. A stable OS-temp prefix handles cross-skill and cross-invocation coordination equally well as an in-repo path; repo-adjacency is rarely the relevant property.
    ...
    - **Cross-invocation reusable**: use a stable, effective-user-owned prefix under `${TMPDIR:-/tmp}/compound-engineering-<effective-uid>/<skill-name>/` — **not** `mktemp -d` — so later invocations by the same OS user can find prior outputs without sharing a writable root with other users. Derive the effective UID with `id -u`, reject a symlink or path not owned by the current user, and create or repair the top-level root to mode `0700` before use. The default layout is one `<scratch-root>/<skill-name>/<run-id>/` directory per run; use it for caches keyed by session, checkpoints meant to survive context compaction, intermediate state, and outputs whose lifecycle or mutation belongs to one run.
      ...
      - **Prefer `$TMPDIR` when set; fall back to `/tmp` when unset.** Assignment form: `SCRATCH_ROOT="${TMPDIR:-/tmp}/compound-engineering-$(id -u)"`. Do **not** hardcode bare `/tmp/compound-engineering-<uid>` — sandboxes that only allow-write `$TMPDIR` (Claude Code's `/tmp/claude-*`) reject creating a new root under `/tmp`. The effective-UID segment still supplies the cross-user boundary. When `$TMPDIR` is the Darwin default under `/var/folders/.../T`, paths stay opaque; that is acceptable for agent-owned scratch. Hosts that want a human-readable inspection path can set a readable `TMPDIR` or widen the sandbox allowlist. Agents running as the same OS user intentionally remain in one discretionary-access-control principal.
