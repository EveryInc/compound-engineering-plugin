...
The script is `scripts/run-tests.ts`, which runs that parallel pass and then, only if every failed file shows the wedge shape, re-runs those files once, serially, in a fresh bun process. The wedge shape is timeouts from the file's first failure to its end: a worker that lost a subprocess event cannot complete any later spawn, so nothing after the first hang passes. A test that passed after a timeout, or an assertion failure or error anywhere in the report, keeps the first result, so a defect that shows only under parallel load, such as a race or a test leaning on another file's leftovers, is not retried away. The check needs the `TimeoutError` failure type bun has reported since 1.3; an older bun never re-runs, which is the previous behavior. That second pass exists because bun can lose a child process's exit or pipe notification inside a test worker (oven-sh/bun#34069, still open); once that happens every later `spawnSync` in that worker hangs until the per-test timeout, so a single lost event turns the rest of a subprocess-heavy file red at exactly the timeout, in a different file each run. The wedge is process-local, so the fresh-process re-run passes; a real failure fails in both passes. A run whose re-run rescued it says so in the log. Do not add `retry` counts to tests for this: a retry runs inside the same wedged worker. See `docs/solutions/developer-experience/bun-parallel-worker-loses-subprocess-exit.md`.
