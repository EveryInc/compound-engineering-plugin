...
The script is `scripts/run-tests.ts`, which runs that parallel pass and then, only if every first-pass failure is a `TimeoutError`, re-runs the failed files once, serially, in a fresh bun process. The lost child-exit is per spawn, not a dead worker: later tests in the same file can still pass, so a timeout followed by a pass is still the bun shape (PR 1680 CI). An assertion failure or error anywhere in the report keeps the first result with no re-run, so a defect that shows only under parallel load, such as a race or a test leaning on another file's leftovers, is not retried away. The check needs the `TimeoutError` failure type bun has reported since 1.3; an older bun never re-runs, which is the previous behavior. That second pass exists because bun can lose a child process's exit or pipe notification inside a test worker (oven-sh/bun#34069, still open); the hung `spawnSync` then fails at exactly the per-test timeout, in a different file each run. The failure is process-local, so the fresh-process re-run passes; a real failure fails in both passes. A run whose re-run rescued it says so in the log. Do not add `retry` counts to tests for this: a retry runs inside the same worker. See `docs/solutions/developer-experience/bun-parallel-worker-loses-subprocess-exit.md`.
