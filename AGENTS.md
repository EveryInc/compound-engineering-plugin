...
- When adding a user-facing skill, document it: create a `docs/skills/<skill-name>.md` page (purpose, novel mechanics, when to use, chain position — follow the shape of the existing pages), add a catalog row under the right category in `docs/skills/README.md`, and bump the skill count in `tests/release-metadata.test.ts`. `docs/skills/README.md` is the **only** hand-maintained skill inventory — the root `README.md` carries a grouped overview that names categories, not individual skills, so adding a skill does not require a README edit unless it opens a new category. Keeping one inventory instead of three is deliberate: the previous three-way sync was unenforced and had already drifted. Every current user-facing skill has a page, including `lfg` and `ce-dogfood`.
