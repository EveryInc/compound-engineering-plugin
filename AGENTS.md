...
PR CI (`.github/workflows/ci.yml`) is the merge gate. It runs, in order: PR-title lint (PRs only), `bun run release:validate`, `bun run plugin:validate`, and `bun run test`. A separate `docs-audit` job runs compound-cli's `compound audit` over `docs/solutions/`; it stays out of `bun run test` and `release:validate` so the local lane never needs compound-cli or network access. Do not invent a parallel local-only mechanical suite — if a check is deterministic and should block merges, put it in one of those steps (usually `bun run test`).
