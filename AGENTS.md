...
- **Scratch Space:** Every predictable, reusable, detached, or independently cleaned CE filesystem lifecycle goes through the byte-identical `scripts/scratch-root.py` resolver carried by each consuming skill. Never use a fixed shared `/tmp/compound-engineering` root, hand-roll fallback logic in prose, or reconstruct an opaque resolver-returned run path. A supervising process may use `mktemp` for a temporary file or directory shared only with its same-UID child process tree when it sets `umask 077`, uses an unpredictable template, retains the exact returned path, and owns cleanup before that process tree ends.
  - **Runtime resolution (`root`, `subdir`, `run-dir`):** (1) `COMPOUND_ENGINEERING_SCRATCH_ROOT`, accepted only when absolute, owner-private, non-symlinked, and readable/writable/searchable; (2) a valid owner-private `$XDG_RUNTIME_DIR/compound-engineering`; (3) `$HOME/.cache/compound-engineering/tmp`; (4) `/tmp/compound-engineering-$(id -u)`. Invalid candidates fall through to the next safe candidate. OS UID is the security boundary; a logical agent name is metadata, not isolation.
  - **Reusable cache resolution (`cache-subdir`):** (1) `COMPOUND_ENGINEERING_CACHE_ROOT`; (2) `$XDG_CACHE_HOME/compound-engineering`; (3) `$HOME/.cache/compound-engineering`; (4) an owner-private `cache/` namespace under the runtime root only when no persistent cache root is available.
  - **Durable state resolution (`state-subdir`):** (1) `COMPOUND_ENGINEERING_STATE_ROOT`; (2) `$XDG_STATE_HOME/compound-engineering`; (3) `$HOME/.local/state/compound-engineering`. Runtime XDG and `/tmp` are not durable-state candidates.
  - **Durable data resolution (`data-subdir`):** (1) `COMPOUND_ENGINEERING_DATA_ROOT`; (2) `$XDG_DATA_HOME/compound-engineering`; (3) `$HOME/.local/share/compound-engineering`. Use this only for final non-repo deliverables that otherwise have no durable user-selected destination.
  - **Security and creation:** set `umask 077`; lifecycle roots and directories are `0700`; files are `0600`. Validate every existing ancestor with descriptor-based no-follow checks. Reject symlinks, wrong owners, wrong types, group/other-writable ancestors (except a root-owned sticky system-temp ancestor), relative overrides, and traversal. Create per-run directories atomically through `scratch-root.py run-dir`, which combines skill identity with a run identifier and an unpredictable `mkdtemp` suffix. Never chmod, signal, remove, or reuse another UID's path/process.
  - **Per-run throwaway:** `<runtime-root>/<skill>/runs/<run-id>-<random>/`. Use for screenshots, intermediate outputs, recordings, delegation prompts/results, detached peer jobs, and single-run checkpoints. Keep peer jobs at `<run-dir>/jobs/<job-id>` so one exact run cleanup owns them. Best-effort exit cleanup is allowed; TTL cleanup must be ownership-, identity-, and lease-aware and must not reap a live run.
  - **Reusable cache:** `<cache-root>/<schema>/<semantic-content-key>/`. Cache identity comes from its content/semantic key, not a run UUID. Writes use exclusive creation where available plus atomic rename; readers retain file-owner checks as defense in depth.
  - **Durable workflow state:** `<state-root>/<skill>/<host>/<repo-or-object-key>/` with locking/lease metadata. It expires by the workflow's explicit lifecycle, not a blanket run TTL.
  - **Cross-agent handoff:** copy or attach artifacts; never relax permissions or point another UID at a private scratch subtree. A review bundle must be generated from a complete staged/indexed change set, include untracked additions, carry a checksum, and be inspected for expected added-file entries before sending. A diff that silently omits new files is not a review artifact.
  ...
  - **Durable outputs** (plans, specs, learnings, docs, final deliverables) belong in `docs/`, another repo-tracked/user-selected location, or the resolver's durable data root for non-repo work. They never live only in a runtime run directory.
  - **Cross-platform note:** the resolver canonicalizes macOS's root-owned `/tmp` symlink to `/private/tmp` before its descriptor-based no-follow walk; Linux and WSL use `/tmp` directly. `mktemp -d -t <prefix>-XXXXXX` also works on all three for the narrow process-local exception above. Skills authored here assume Unix-like shells; native Windows is not a current target.
- **Character encoding:**
...
Repo-grounding skills (`ce-pov`, `ce-plan`, `ce-optimize`, `ce-ideate`, `ce-brainstorm`, `ce-code-review`, plus lighter consumers `ce-compound` — which still derives **and persists** on a miss — and `ce-debug`, which only opportunistically reads `conventions.testing` and never derives/persists) reuse one cached **question-agnostic project profile** (stack, deps, conventions, structure) instead of each re-deriving it. The profile is git-keyed and stored at `<resolved-cache-root>/repo-profile-v1/<root-sha>/<inputs-digest>.json` (`inputs-digest` = sha256 of profile-input path+blob pairs at `HEAD`, so commits that do not touch those inputs share one entry).

The mechanism is four **byte-duplicated** assets per consuming skill (the plugin has no cross-skill import — see "File References in Skills"):
...
- `scripts/scratch-root.py` — validated owner-scoped root resolution shared with every other scratch consumer.
