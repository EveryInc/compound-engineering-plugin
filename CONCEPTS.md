...
The size class `ce-plan` assigns a Durable plan at intake — `lightweight`, `standard`, or `deep` — governing how much research, flow analysis, and per-section detail the plan receives. Distinct from the output contract (which decides whether a file is written at all) and from `artifact_readiness` (which records document completeness). Recorded as `depth:` in plan metadata.
