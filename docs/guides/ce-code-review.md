...
When adversarial is selected and the working tree is the reviewed head (current branch, or a PR whose local tree already matches the PR head), the adversarial lens runs through **one separately launched cross-model route** in a read-only process. Serving-family receipts establish when that route is independent from the host; receiptless routes such as ZCode are cross-harness checks with independence unverified. A started peer **replaces** the in-process `adversarial` persona. They never both receive the same brief. The in-process persona runs if the peer cannot start, or if the started peer returns only session-quota or auth-context failure — in that case the next announced different-family peer is tried when one is eligible, otherwise the local persona covers the lens. An exact provider-overload 529 gets one same-route retry; repeated overload, another stubborn transient rate limit, or max-turn exhaustion falls back locally without an unbounded retry loop. Remote PR or branch diffs stay on the in-process persona, because that reviewer can inspect the fetched refs.

Agreement between the peer and another in-process reviewer is a strong promotion signal in synthesis only when the peer's serving family is independently verified.

`cross_model_review_mode: off` in CE config keeps this pass from running at all — no peer is resolved and nothing leaves the host; the in-process reviewers cover the lens and Coverage says the pass was disabled by checkout config. A direct request in conversation for a peer overrides it for one run. Which target runs the peer is auto-chosen and overridable: conversation, `cross_model_peer:` in CE config, active project instructions, then `codex → claude → grok → composer`. `GLM` is an explicit/configured target through the `zcode` CLI and intentionally does not change that automatic order. `Cursor` means `cursor-agent` using its configured default/Auto model. `Composer` means a Composer model through Cursor. `Grok` binds the native grok CLI when it is installed; Grok through Cursor is a different route, used when asked or when the grok CLI is missing and Cursor is allowed. Cursor Auto does not count as independent agreement unless its serving family is verified different from the host. `cross_model_model:` and `cross_model_effort:` in CE config pin supported targets; ZCode instead requires an allowed GLM-family main model in its own configuration and fails closed before egress otherwise. Each launch and retry consumes a private snapshot of that validated configuration, so a later live-config change cannot silently change the requested model. The snapshot may select a custom provider endpoint, so disclosure names the configured ZCode provider endpoint and marks recipient identity unverified rather than assuming Z.AI. Its served model, family, and effort remain unverified, so its attributed findings do not receive independent-agreement promotion. A value the peer cannot honor skips the pass with a stated reason rather than substituting. See the [configuration reference](./configuration.md).

**Prerequisite: a peer agent CLI.** The pass drives a read-only *agent* CLI (`codex`, `claude`, `grok`, `zcode`, or `cursor-agent`); a bare provider key does not enable it. ZCode is the tool-less exception: it receives the attached diff and host-vetted brief but no filesystem tools, because its workspace root is not a path sandbox. Peers are discovered on `PATH`, plus the CLI bundled inside the Codex desktop app (`ChatGPT.app/Contents/Resources/codex` since the July 2026 app merger, or `Codex.app/…` on older installs; the app does not link it onto `PATH`). Gemini has no standalone peer target — it participates only through Cursor when `cursor-agent` attests a Gemini serving family. With no peer CLI installed the skill runs the in-process adversarial reviewer and reports "cross-model pass: not run"; the skip reason names what to install.
