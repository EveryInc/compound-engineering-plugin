...
## Set a knowledge-folder layout

`/ce-setup knowledge` (or asking in words for the folder's layout) interviews for a layout template -- `none`, `kieran`, `katie`, `nityesh`, `para`, `johnny-decimal`, or a custom template file -- proposes one when the folder's evidence supports it (`docs_root: learnings` plus `experiments/NNN-*` folders reads as `kieran`), and expands the template **into** the `knowledge:` block of `config.yaml` so [`ce-capture`](./ce-capture.md), [`ce-dream`](./ce-dream.md), [`ce-experiment`](./ce-experiment.md), and [`ce-compound`](./ce-compound.md) read one file. It shows the block before writing, validates it after, and dry-runs the folder against it: directories the layout expects, files whose frontmatter `type` belongs to another role, and inbox items past their retention age are listed as proposals and applied only on approval. An unusable layout is refused, never approximated. Key reference: [Knowledge layout](./configuration.md#knowledge-layout).
