...
Before each task, it checks whether the unit's work is already present and matches the plan's intent. If verification is already satisfied, it marks the task complete and moves on. No silent reimplementation. That matters most when resuming after context compaction, picking up someone else's branch, or returning to a partly-shipped plan weeks later. A unit whose deliverable is out-of-repo state (console, DNS, CMS, live rows) has no git-derived completion signal — do not treat a clean tree as not-started and re-apply a non-idempotent side effect. Decide from observed desired state: mark complete when it is satisfied, execute when it is positively unsatisfied and first-run or retry is authorized and safe, and ask or block when the state is unknown and execution is unsafe.
