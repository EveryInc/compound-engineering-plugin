...
Before each task, if any part of the unit's completion depends on out-of-repo state (console, DNS, CMS, live rows), decide from observed desired state first — that unit has no git-derived completion signal. Do not treat a clean tree as not-started and re-apply a non-idempotent side effect. Mark complete only when the desired state is satisfied; execute only when it is positively unsatisfied and first-run or retry has positive authorization and safety; ask or block whenever execution lacks that positive authorization or safety. Git evidence short-circuits only units whose entire completion signal is repository-derived: if that work is already present and matches the plan's intent, mark the task complete and move on. No silent reimplementation. That matters most when resuming after context compaction, picking up someone else's branch, or returning to a partly-shipped plan weeks later.
