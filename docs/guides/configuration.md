...
| [`ce-work`](./ce-work.md), [`lfg`](./lfg.md) | `work_engine_effort` | Reasoning effort for the external implementation worker, as a map from harness to one of that harness's own levels (claude `low`..`max`, codex `minimal`..`xhigh`, grok `low`..`high`, opencode its own variants; Cursor routes take none). A harness left out keeps its default. A level the harness cannot run makes that entry unavailable, and the list continues. See [Implementation routing](#implementation-routing). |
