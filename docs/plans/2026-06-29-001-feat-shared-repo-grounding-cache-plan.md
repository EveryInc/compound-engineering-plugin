...
A shared, git-keyed **project-profile cache**: the question-agnostic repo orientation (stack and versions, dependency surface, topology, conventions and instruction files, vocabulary, docs/solutions index) is derived once by a shared profiler, cached under the owner-private cache root returned by the bundled resolver, and reused by every repo-grounding skill running as the same OS UID. Each consumer keeps deriving only its question-specific slice fresh on top of the cached profile.
...
- R2. The cache lives at `<resolved-cache-root>/repo-profile/<root-sha>/<head-sha>.json` — an owner-private shared cross-skill prefix, keyed by `git rev-list --max-parents=0 HEAD` (repo identity; if the repo has multiple root commits, use the lexicographically-first for a deterministic key) and `git rev-parse HEAD` (state). Two worktrees/clones at the same commit and OS UID share the entry. An entry is valid only when its stored `profile_schema_version` matches the reader's — a mismatch is a MISS (a newer schema never reuses an older entry). Lookup uses git metadata only; on a hit, only the cache file is read.
...
- R4. **Graceful degradation.** No git, no safe writable resolver candidate, an unreadable or malformed cache, or any helper error → derive fresh (cache miss), never block. Non-git project folders are session-scoped only (no SHA key). The cache is never a correctness dependency.
