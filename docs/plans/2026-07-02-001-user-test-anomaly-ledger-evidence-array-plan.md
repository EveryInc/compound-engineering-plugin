...
- R19. The execution-index counter continues monotonically across iterate-mode iterations (no per-iteration reset). Iterate mode resets the ledger only before iteration 1, then each iteration reconciles the new ledger lines appended during that iteration into that iteration's run JSON. The aggregate commit payload unions dispositioned anomalies across iterations, carries the session ledger digest plus the run-global `final_execution_index`, and validates tiling from `0` through `final_execution_index` against the session ledger with the standard tolerances. Earlier iterations' anomalies are not re-reconciled from prior per-iteration run JSONs, but their refs validate against the run-global final index.
...
- AE9. **Covers R19.** A three-iteration iterate run logs one anomaly during iteration 2 (indices continuing monotonically across iterations); one session ledger tiles `0` through the run-global final index, and the aggregate committed run JSON contains that anomaly with a disposition and the session ledger digest.
