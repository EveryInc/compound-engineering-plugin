...
- KTD4. **The artifact is written to a stable, opaque owner-private run directory before the destination ask.** The bundled resolver creates and returns the exact run path; consumers pass that path through without reconstructing it. The destination ask copies out of the run dir; declining everything leaves the artifact there, reported (AE7). The run dir plus the artifact's visible metadata (topic, date, input shape) is the hook the deferred library layers onto without rework.
