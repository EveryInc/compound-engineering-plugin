...
- **Dependencies:** none
- **Files:** `skills/ce-pov/references/cross-model-panel.md`, `skills/ce-pov/scripts/cross-model-pov.sh`, `tests/skills/ce-pov-cross-model-routes.test.ts`
...
- **Test scenarios:**
  - Widen the existing artifact assertion in `tests/skills/ce-pov-cross-model-routes.test.ts` (already pins `pov-claude.json`) to also pin the panel doc's `pov-<target>.json` contract sentence and the `--result-path` instruction.
  - `bun test` parity suites (`cross-model-receipt-parity`, `peer-job-runner-parity`) and `tests/scratch-root-contract.test.ts` (pins the panel-doc strings U5 edits) pass unchanged — together with the routes test above, the four test files U5's Verification names.
- **Verification:** `bun test` green across the four named test files; a manual read of `cross-model-pov.sh --help` output shows target wording.
...
|---|---|---|
| Contract pins | `bun test tests/pov-skill-contract.test.ts` | U1-U4 |
