...
Status: completed; ordinary Fable adoption approved
...
## Receipt-valid adoption rerun — 2026-07-22

The approved reduced benchmark completed its immutable 160-call ceiling: 20 Opus/high and 20 Fable/high Anthropic critiques plus three blinded OpenAI judge votes for each of the 40 receipt-eligible outputs. All 40 critiques carried a matched final-assistant author receipt, were schema-valid non-refusals, and met the deadline. Opus receipts named `claude-opus-4-8`; Fable receipts named `claude-fable-5`. Auxiliary Haiku participation remained participant inventory only.

Both ordinary lanes passed the pre-registered adoption gate. Fable matched Opus at 1.0 severity-weighted detection with no P0/P1 regressions. Noise increased by 0.1 findings/review for product-lens and 0.4 for whole-doc, within the 0.5 limit. The corrected overall decision is `adopt`, authorizing Fable for product-lens and whole-doc while guarded lanes remain Opus.
...
```benchmark-aggregate-json
{
  ...
  "decision_table": [
    {
      "lane": "product-lens",
      "opus": {
        "severity_weighted_detection": 1,
        "noise_per_review": 1.2,
        "schema_success_rate": 1,
        "receipt_success_rate": 1,
        "non_refusal_success_rate": 1,
        "deadline_success_rate": 1,
        "quality_numerator": 10,
        "quality_denominator": 10,
        "median_latency_ms": 27849
      },
      "fable": {
        "severity_weighted_detection": 1,
        "noise_per_review": 1.3,
        "schema_success_rate": 1,
        "receipt_success_rate": 1,
        "non_refusal_success_rate": 1,
        "deadline_success_rate": 1,
        "quality_numerator": 10,
        "quality_denominator": 10,
        "median_latency_ms": 37911
      },
      "detection_delta": 0,
      "bootstrap_lower_bound_delta": 0,
      "p0_p1_regressions": [],
      "noise_delta": 0.10000000000000009,
    ...
    },
    {
      ...
      "opus": {
        ...
        "quality_numerator": 10,
        "quality_denominator": 10,
        "median_latency_ms": 20883.5
      },
      "fable": {
        "severity_weighted_detection": 1,
        "noise_per_review": 0.8,
        "schema_success_rate": 1,
        "receipt_success_rate": 1,
        "non_refusal_success_rate": 1,
        "deadline_success_rate": 1,
  ...
  "overall_decision": "adopt"
}
```

## Historical unverified ordinary-lane run — 2026-07-22
...
The initial scorer incorrectly printed `adopt` because both arms had the same 0% receipt success and the implementation checked only relative regression. That verdict was rejected before any routing change. The scorer now requires a matched, usable final-assistant author receipt, and it uses the full pre-registered trial count as the majority denominator. The same immutable redacted receipts rescore to `stop` in both lanes; the later receipt-valid rerun above supplies the evidence that authorizes the ordinary-lane default change.
