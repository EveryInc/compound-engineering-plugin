...
- **Objective:** The cost of reviewing a Durable plan is measurable per persona and against document size — wall time from dispatch to each persona's persisted return, and findings per persona, joined to the reviewed document's unit and line counts — and the product-lens reviewer runs only when a plan stakes a product position worth a product judgment.
- **Means:** persist every local persona's findings and a run manifest (KTD1, KTD8), restate the product-lens activation condition (KTD6).
...
- **Execution profile:** three units, one branch (`tmchow/doc-review-right-size-a`), one PR to `main`. U1 and U3 are independent; U4 depends on U3.
...
Two bounded changes to one skill. `ce-doc-review` writes each local persona's returned findings into its run directory, which it now creates on every review, so a later measurement can compare what the local roster found with what the cross-model peer found. `ce-doc-review`'s product-lens persona activates on a position the document stakes that a stakeholder could challenge, or on strategic weight, and no longer on the existence of plausible alternatives. The adversarial reviewer, the cross-model pass, the plan template, and the unified-plan floor do not change.

### Problem Frame

PR #1514 right-sized ceremony at intake; a small request that still lands as a Durable plan pays for document review in full. Measurement this session could not say what that review buys: only the cross-model peer files land on disk (55 runs under `/tmp/compound-engineering-501/ce-doc-review/`, zero local persona files), so review cost cannot be compared with what the peer found or read against document size. Separately, product-lens's premise leg lists "solution selection where alternatives plausibly exist", which holds for nearly any fix, so a judgment persona runs on routine bootstrap plans. Peer-job consolidation is a separate decision gated on a non-inferiority benchmark; it needs the persisted local findings this plan adds.
...
- R7. The run directory holds a `run.json` manifest written at dispatch — reviewed document path, document type, the document's unit and line counts, the announced team, and the dispatch time — and each persisted persona file records its return time, so cost is computable from what is on disk against the document's size.
...
- **No depth label in plan metadata** (session-settled: user-directed — chosen over recording `depth:` at plan-write: a recorded classification invites consumers to route on it; the manifest's unit and line counts answer the size question). Governs R7.
- **Verification is proportionate: bun-test pins for the measurement change, a small activation probe for the product-lens change** (session-settled: user-approved — chosen over a three-host matrix or TUI sessions). Governs the Verification Contract.
...
- Out: the plan template, its metadata fields, and the implementation-ready floor; exposing the run-directory path in the non-interactive envelope; run-directory cleanup.
...
- A measurement pass over persisted findings (peer-unique findings by persona, review cost against document size) once enough runs carry both.
...
- KTD8. **The manifest is the size axis and the clock.** `run.json` is written when the run directory is created and carries the document's unit and line counts, observable from the document itself; a persona's return time is stamped by whoever writes its file. Cost is wall time from dispatch to persisted return plus findings count per persona; token usage is not observable by the orchestrator and is not a requirement. A classification label is not recorded: a consumer would be tempted to route on it, and the counts answer the size question without that risk. Governs R7.
...
- **Approach:** grade the roster the run declares on its trailing `TEAM:` line (the task asks for it; the cell captures the final message, not mid-run announcements, and the non-interactive envelope carries no Coverage table): the grader scopes `must_include` / `must_not_include` to that line when present, so routine and settled-origin rows `must_include` the always-on pair and `must_not_include: ["product-lens"]`, and staked-position and strategic-weight rows `must_include` `product-lens` in both arms. Grade only the product-lens dimension; adversarial activates on every bootstrap fixture by its provenance rule and is reported separately, not graded.
...
- `skills/ce-plan/` is untouched; `ce-doc-review/SKILL.md` stays under the cap.
- U4 rows and fixtures are committed with their pre/post results in the PR body; the U1 run-dir listing is in the PR body.
- `docs/skills/ce-doc-review.md` describes the persisted artifacts.
