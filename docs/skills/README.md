...
| [`/ce-commit-push-pr`](./ce-commit-push-pr.md) | Go from working changes to an open PR with adaptive descriptions, related-reference handling, three modes (full workflow / description update / description-only generation), optional opt-in PR-stack submit, and a concept-teaching section for anything the change newly introduces |
