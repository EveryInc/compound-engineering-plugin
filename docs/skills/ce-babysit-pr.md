...
For an independent PR against a normal base branch, it first previews the exact observed base without changing the PR. It can complete the merge only when every conflict has one mechanically determined answer that preserves the PR's established intent, and when normal push authority to the exact head is positively known. Otherwise it aborts and reports a sticky `needs-human` decision with the competing intents in plain language. It never rebases or force-pushes.

Managed stacks stay on their manager's refresh/restack path; the ordinary branch-maintenance route does not restructure them. Manual PR dependency chains are also outside this automatic path.
