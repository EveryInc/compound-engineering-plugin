...
The two repair paths have different authority. A `BEHIND` PR can use the hosting service's ordinary branch-update capability when that capability is positively reported. If the PR object's cached base OID lags the independently read live base ref, that stale cache still blocks readiness but may enter this guarded `BEHIND` route bound to the live base OID; a failed live-ref probe remains fail-closed. A conflicted `DIRTY` PR requires separate proof that the authenticated Git route can push normally to the exact PR head, plus a clean preview against the exact observed base. Host update capability is not proof of direct push authority.
