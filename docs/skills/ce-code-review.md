...
**Which target runs the peer** is auto-chosen and overridable — the same independence system as `ce-doc-review`, scoped to the adversarial lens only. Host harness and serving family are tracked separately. Preference order is conversation → `cross_model_peer:` → active project instructions → `codex → claude → grok → composer`; a preferred same-family target is disclosed and skipped before route fixation. Explicit `Fable` means Fable 5 high-thinking through Cursor, explicit `Cursor` means `cursor-agent` using its configured default/Auto model, and `Composer` means a Composer model through Cursor. Grok prefers its native CLI and may use a sanctioned Grok-via-Cursor route. Cursor-routed targets without a served-model receipt, including Fable, do not count as independent agreement; Fable also carries Cursor's NO ZDR designation and sends reviewed code to Cursor plus Anthropic.
