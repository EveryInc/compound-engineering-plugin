...
A small config change triggers 5 reviewers (the 4 always-on + 1 CE always-on). A Rails auth feature with migrations might trigger 9. The skill decides which personas fit the diff:

- **Always-on (every review)** — `ce-correctness-reviewer`, `ce-testing-reviewer`, `ce-maintainability-reviewer`, `ce-project-standards-reviewer`, `ce-learnings-researcher`
