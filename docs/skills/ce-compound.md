...
Full mode always runs a cheap two-stage session-history probe. A discovery+metadata pass runs in parallel with the research subagents. It escalates to extraction and synthesis only when a candidate session clears a relevance bar (current-branch match or at least two topic-keyword hits). On a hit, findings fold into "What Didn't Work" (bug track) or "Context" (knowledge track). Lightweight skips this entirely.
