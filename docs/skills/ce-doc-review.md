...
The host identity and the current command lane are evaluated separately before detachment. When Codex's child sandbox cannot reach the selected CLI's credentials or network, the skill requests one-shot approved host execution for the unchanged, disclosed runner start. If that authority is denied or unavailable, no peer job starts; the in-process twins continue and the missing broad whole-document sweep is named. This is a host-execution limitation, not evidence that the user's provider account is logged out.

Every leg is artifact-backed and input-bound. The start declares its deterministic `<reviewer>-<target>.json` result path and the SHA-256 of the exact document or slice; the worker removes stale output and recomputes the digest before prompt construction. The orchestrator retrieves by job ID, validates the full schema, and reconciles route/model identity. A valid empty findings artifact proves the critique ran; only a verified served-model receipt makes it independent corroboration. Headless coverage reports each leg as `verified`, `local_only`, `host_denied`, `auth_failed`, `timeout`, `quota_limited`, or `unusable_output`, with environment-scoped reasons and no raw provider diagnostics.
