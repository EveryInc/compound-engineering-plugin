...
**Which target runs the peer** is auto-chosen and overridable. Host harness and serving family are tracked separately so the pass can exclude same-model checks. Preference precedence is conversation, `cross_model_peer:` in `.compound-engineering/config.local.yaml`, active project instructions, then `codex → claude → grok → composer`; a preferred same-family target is disclosed and skipped before route fixation. `Fable` is accepted explicitly and means Fable 5 high-thinking through Cursor; `Cursor` means `cursor-agent` using its configured default/Auto model; `Composer` means a Composer model through Cursor; Grok prefers its native CLI and may use a sanctioned Grok-via-Cursor route. Cursor-routed targets without a served-model receipt, including Fable, are not promoted as independent corroboration; Fable also carries Cursor's NO ZDR designation and sends the document to Cursor plus Anthropic.
