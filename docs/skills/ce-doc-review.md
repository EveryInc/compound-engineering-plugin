...
**Which target runs the peer** is auto-chosen and overridable. Host harness and serving family are tracked separately so the pass can exclude same-model checks. Preference precedence is conversation, the effective merged `cross_model_peer` setting, active project instructions, then `codex → claude → grok → composer`. `Cursor` is also accepted explicitly and means `cursor-agent` using its configured default/Auto model; `Composer` means a Composer model through Cursor; Grok prefers its native CLI and may use a sanctioned Grok-via-Cursor route. Cursor Auto is not promoted as independent corroboration unless a receipt proves a different serving family. See the [configuration reference](./configuration.md) for global/project precedence and compatibility behavior.

Generalized routing may bind the `review` class or individual `ce-doc-review.*` roles after lens selection. It changes supported execution selectors and receipts only: configured roles cannot activate a lens, rewrite its prompt, widen read scope, grant mutation, or bypass the peer egress gate.
