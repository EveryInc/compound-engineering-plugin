...
The controller runs the exact frozen stability repeats inside its confined descendant supervisor, deterministically aggregates them, then writes one private `result-marker.json` and bound worktree `result.yaml` mirror before the orchestrator updates the main log. On resume, the skill accepts only controller-validated markers not yet in the log. A worktree marker, process exit, or caller claim alone cannot authorize recovery, reset, cleanup, or recipient fallback.
...
The author and judge never share a session, workspace, prompt, mutable authority, attempt lock, or receipt. Judges receive only rubric plus sampled candidate output and cannot receive an experiment worktree. Codex authors/judges and controller measurement run through a self-contained Landlock plus network-denying seccomp adapter with an empty launcher environment, isolated HOME/XDG/temp/backend roots, and explicitly staged private JSON auth. Sanctioned values are applied only to the confined child and loader/interpreter hooks are forbidden. Real home, `.env*`, ambient tokens, SSH/cloud stores, hidden references, controller state, canonical siblings, unrelated absolute paths, and sockets are inaccessible. Executable owner/mode/ancestor trust and project/worktree exclusion are enforced before dispatch.
