...
- **Crash recovery** — a sealed manifest commits controller state; its hash-chained journal and controller-owned process receipts/result markers enable resume without trusting a worker-authored `result.yaml`
...
The Phase 1 baseline and every experiment measurement run inside the controller's confined descendant supervisor. It performs the exact frozen stability repeats, accepts only the exact declared metric keys with finite numeric/boolean scalar values, deterministically aggregates them, then writes one private `result-marker.json` and bound worktree `result.yaml` mirror before the orchestrator updates the main log. Raw measurement stdout/stderr is reduced to digests rather than persisted. `state.json` is the commit point for controller transitions; journal records are completely written and hash-chained to its committed head. A locked resume truncates only a valid contiguous journal suffix left before manifest publication and rejects corruption, gaps, rewrites, or a journal behind the manifest. On resume, the skill accepts only controller-validated markers not yet in the log. A worktree marker, process exit, or caller claim alone cannot authorize recovery, reset, cleanup, or recipient fallback.
...
Routing changes model identity only through the backend already selected by the optimization spec. A Codex run stays on controller-confined `codex exec`. OpenCode's ordinary `ce_task` adapter attests model/variant and native Task permissions, but not the inherited Landlock, sanitized process environment, launch barrier, PID/subreaper, or all-descendant cleanup required by routed Optimize work. Configured native worktree author/judge candidates are therefore unavailable before dispatch rather than encoded into a prompt or accepted through caller-authored evidence. Profiles cannot switch backend, alter mutable/immutable scope, change the measurement command, weaken `codex_security`, increase concurrency, modify judging/stopping policy, or bypass checkpoints.

Every role/experiment-or-judge instance and candidate ordinal is locked at most once before dispatch to its exact recipient, backend, worktree, spec/measurement/stability/scope digests, full filesystem inventory, and resolver lock. The controller derives typed outcome, terminal/integrated state, strictly monotonic prior history, and serving evidence from its supervised Codex receipt. The public caller-minted host receipt/token path is absent. Author output remains quarantined until full inventory and scope checks accept it; judge output is finalized separately before parsing. Required unavailable, mismatched, or unverified identity blocks. Preferred fallback requires controller-verified terminal unintegrated discarded output and fresh material/environment, and is impossible after measurement/checkpoint/integration.

The author and judge never share a session, workspace, prompt, mutable authority, attempt lock, or receipt. Judges receive only rubric plus sampled candidate output and cannot receive an experiment worktree. Codex authors/judges and controller measurement run through a self-contained Landlock plus network-denying seccomp adapter that validates the Linux audit architecture and blocks native/compat socket creation, socket pairs, and `io_uring_setup`. Author/judge routes use isolated HOME/XDG/temp/backend roots and explicitly staged private JSON auth. Measurement uses a separate credential-free HOME/XDG/temp/scratch environment with no provider config pointer, staged auth, or sanctioned route values; the repository/worktree is read-only except for absent disposable output directories explicitly declared by the spec and removed after each run. Real home, `.env*`, ambient tokens, SSH/cloud stores, hidden references, controller state, canonical siblings, unrelated absolute paths, and network creation paths are inaccessible. Executable owner/mode/ancestor trust and project/worktree exclusion are enforced before dispatch.

Experiment worktrees also carry durable controller leases. Controller-confined Codex authors have a finite one-hour route timeout and per-stream 2 MiB limits enforced while reading; timeout or overflow runs TERM/KILL descendant containment before terminal failure evidence. Measurement retains its separately configured timeout. Codex and measurement launch barriers plus child-subreaper receipts keep descendants in durable lifecycle state across controller crashes. Existing worktrees and branches cannot be reset or cleaned while any bound attempt is live, launching, unknown after a crash, quarantined, or missing an exact controller-verified CP-3 checkpoint. Reset/clean/remove/branch mutation executes inside the controller while it holds the same per-worktree lock used by new attempts. Only verified `completed` or `abandoned` state permits reuse; `result.yaml`, placeholder hashes, and caller flags have no override authority.

With no generalized route or with `ce-default`, author/judge model selection, backend choice, worktree adapter compatibility, and `metric.judge.model` behavior remain built-in-compatible. Native Task completion is recorded explicitly as `native-compatibility-unverified`: it makes no strict model, confinement, environment, barrier, PID/subreaper, or descendant-cleanup claim. Controller attempt locks, canonical paths, measurement confinement, and lease/result-marker protections still apply; Codex routes retain their stronger process and filesystem controls.
