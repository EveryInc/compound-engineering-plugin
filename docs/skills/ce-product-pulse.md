...
The skill never mutates the product, the database, or any external system. It reads effective global/project pulse settings, but setup writes only `.compound-engineering/config.local.yaml` (gitignored, machine-local) plus the report file (`docs/pulse-reports/...`); it never materializes inherited global values or edits global config. MCP and other data-source tools are invoked read-only; if a tool offers write modes, they're not used.
...
The skill detects this is configured (`pulse_product_name` is present in the effective merged settings), so it skips the interview and goes straight to Phase 2. It applies the 15-minute trailing buffer: `[Friday 5:45pm — Monday 8:45am]`.
...
Effective configuration merges user-global and project-local `pulse_*` keys: product name, primary event, value event, completion events, quality scoring, quality dimension, analytics source, tracing source, payments source, DB enabled, per-metric source overrides, pending metrics, excluded metrics, and default lookback. The Product Pulse setup interview writes only the project source. See the [configuration reference](./configuration.md) for the complete key list and writer ownership.
