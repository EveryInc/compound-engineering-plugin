...
Declining the one-time Spiral setup offer writes a project-local override. The skill reads the effective global/project value but never writes global config; see the [configuration reference](./configuration.md#local-writer-ownership).
