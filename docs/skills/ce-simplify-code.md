...
| `mode:structured manifest:<path>` | Composition mode: simplify only manifest-owned files after fail-closed repo-relative manifest entry validation, disable branch-diff fallback, and return structured status/files/verification JSON |
