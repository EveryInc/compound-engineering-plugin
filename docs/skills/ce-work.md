...
Verification commands themselves come from configuration first: when the checkout's `.compound-engineering/config.local.yaml` sets active `quality_commands` keys (`test`, `lint`, `typecheck`, `build`), `ce-work` runs those commands verbatim as single shell calls — for per-task verification, the Phase 3 full-suite/lint pass, and the incremental-commit test check — and includes them in worker dispatch packets so subagents don't rediscover the project's tooling each run. Unset keys fall back to discovering commands from the project's active instructions and context, and a more specific instruction always wins (task instruction > session/project instructions > config).
