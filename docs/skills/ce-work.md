...
Before any repository material leaves the host, `ce-work` discloses and records the instruction/config source, fixed recipient and intermediaries, bounded unit material exposed, and which restrictions are adapter-enforced versus cooperative. The host explicitly names only the selected backend's required JSON auth files in a private manifest; the controller copies those files, not their containing directory, into an attempt-scoped config root under the private run directory. It also creates isolated `HOME`, XDG, cache/data, and temp roots, rejects `.env*` material, and derives redactions from staged credential values. The detached runner gives the adapter its job identity; the controller validates the actual runner metadata and exact adapter argv before the external CLI starts, so a shell prefix or substituted worker cannot egress under a valid unit authorization. It also binds a co-located Linux Landlock adapter and pinned interpreter paths/digests, ABI, the exact route executable identity/digest, and allowed filesystem roots into attempt and dispatch receipts. Missing or unsafe staged authentication or confinement is rejected before the external CLI receives the packet rather than exposing ambient home/config roots, SSH/cloud variables, another CLI store, or credential bytes in argv/logs/receipts. Bounded JSON credentials rotated by the backend are added to redaction before output retention. The adapter cannot switch recipients, widen scope, push, open a PR, or choose fallback. If a required restriction cannot be enforced, the route is unavailable.
...
Not by themselves. They isolate concurrent Git state and contain accidental mutation. Before an external CLI can receive a packet, `ce-work` additionally applies controller-bound Linux Landlock filesystem confinement. The recipient gets only its workspace, isolated attempt environment and staged backend auth, selected executable runtime, and required system files; real-home absolute paths, host-home enumeration, the canonical checkout, controller results, and linked Git metadata are denied. If that OS boundary cannot be established or its adapter/executable identity changes, the route is unavailable rather than weakened.
