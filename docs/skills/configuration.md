...
Run `/ce-setup` to create or repair the file and its `.gitignore` coverage. The committed `.compound-engineering/config.local.example.yaml` lists the available settings; uncomment only the keys you want to change. Do not put credentials or harness flags in this file; the one sanctioned command surface is `quality_commands`, because configured test/lint/typecheck/build commands give skills deterministic verification instead of per-run command rediscovery.
...
| [`ce-work`](./ce-work.md) ([`lfg`](./lfg.md) indirectly) | `quality_commands` | Map of deterministic verification commands with sub-keys `test`, `lint`, `typecheck`, `build`. Each value is a single command string run verbatim as one argv-style shell call; exit status is the result. Unset keys fall back to discovering commands from the project's active instructions; configured commands never override a more specific instruction (task instruction > session/project instructions > config). |
