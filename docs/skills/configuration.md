...
| [`ce-code-review`](./ce-code-review.md), [`ce-doc-review`](./ce-doc-review.md) | `cross_model_peer` | Preferred cross-model review target: `codex`, `claude`, `grok`, `cursor`, or `composer`. `grok` prefers the native grok CLI; Grok through Cursor is a conversation-asked route, not this key. The review skills still apply host-independence and route-availability gates. |
