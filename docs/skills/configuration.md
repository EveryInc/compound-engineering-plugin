...
Feature setup writers remain project-local: Product Pulse, Sweep, Promote, and similar flows use the resolver's `patch_source` operation only for `.compound-engineering/config.local.yaml`. They use the inspected project source revision for a compare-and-swap update, preserve unrelated project keys, and never copy inherited global values into the checkout. Because portable POSIX rename has no inode-conditional replace, the writer first displaces the current destination without destroying it and installs the candidate with no-overwrite semantics. A competing save is restored or retained, the uncommitted candidate is preserved at the `candidate_path` reported by `WRITE_CONFLICT`, and the caller must re-inspect rather than overwrite either version. Only `ce-setup`, after explicit global intent, writes the global source.
