...
**Refusal message, after:** a bare `watch --pr N` still exits 2, but now the stderr also carries `WATCH_BOOTSTRAP_HINT` — the two-step bootstrap-then-arm recipe verbatim and "Never mint these values yourself; they must come from the bootstrap snapshot" — so the same context-absent agent that produced the illegal call gets a self-contained recovery path from the tool's own output rather than a generic argparse usage dump.
