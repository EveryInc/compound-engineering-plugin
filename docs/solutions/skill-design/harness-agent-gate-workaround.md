...
- **A narrow `allowed-tools` pin and a bundled setup script cannot be reconciled by narrowing globs.** `ce-resolve-pr-feedback` pinned Bash to `gh` and `git`, so the setup step was dead on arrival there. Widening produced three findings on one root: `Bash(command *)` pre-approved arbitrary shell because `command` executes its argument; narrowing to `Bash(node *)` pre-approved `node -e '<arbitrary JS>'`, the same hole through a different door; and the block still could not run unattended because `if [ -n "$NODE" ]` executes a bare `[` no pin covers. A single pinned launcher does not escape it either — the setup script resolves through a model-supplied `SKILL_DIR`, so the concrete path is dynamic and a static pin cannot match it.

  The resolution was to drop the pin rather than keep narrowing it. A pin that reads as `gh`/`git`/`Read` while granting arbitrary JS execution is worse than no pin, because it invites a trust it does not earn. The lesson generalizes: when successive narrowings each surface an equivalent hole, the enumeration is the wrong instrument, not the current glob.
