...
Documentation-sourced from the installed docs of Devin CLI 3000.10.31 (`subagents.mdx`, `models.mdx`, `reference/configuration/config-file.mdx`, `enterprise/controls.mdx`), rechecked on 2026-09-19 against the [subagent documentation](https://docs.devin.ai/cli/subagents); `devin doctor` and `devin models list` verified live on that build.
...
- Custom profiles support either a flat `agents/<name>.md` file or an `agents/<name>/AGENT.md` directory layout under `.devin/`, `.agents/`, or `~/.config/devin/` (`%APPDATA%\devin\` on Windows). Frontmatter fields include `name` (defaults to the file or directory name), `description`, optional `model` (a model identifier; when omitted the profile runs the org default subagent model), `allowed-tools` (hard restriction on the child's toolset, unlike the skill-frontmatter auto-approval semantics above), and `max-nesting` for profiles whose agents dispatch their own subagents.
...
- `devin doctor` validates custom profile frontmatter (verified live with one existing custom profile loaded).
- [Devin Desktop's Devin Local agent](https://docs.devin.ai/desktop/devin-local) shares the same agent harness as the CLI, so the same profile locations and semantics apply there.
...
- Observability limit: the subagent panel labels profile, title, status, elapsed time, and tool-call count, but not which model is running. Live profile-to-model attribution was not verified.
