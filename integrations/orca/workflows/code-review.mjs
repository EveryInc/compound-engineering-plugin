...
export const ROLE_POLICY = Object.freeze({
  ...
  "persona-review": Object.freeze({
    ...
    "performance-reviewer": Object.freeze({ required: false, repeatable: false }),
    "api-contract-reviewer": Object.freeze({ required: false, repeatable: false }),
    "data-migration-reviewer": Object.freeze({ required: false, repeatable: false }),
    "reliability-reviewer": Object.freeze({ required: false, repeatable: false }),
    "adversarial-reviewer": Object.freeze({ required: false, repeatable: false }),
    "previous-comments-reviewer": Object.freeze({ required: false, repeatable: false }),
    "julik-frontend-races-reviewer": Object.freeze({ required: false, repeatable: false }),
    "swift-ios-reviewer": Object.freeze({ required: false, repeatable: false }),
...
function validateNode(node, index, seenIds, seenRoles) {
  if (!isRecord(node) || !ownKeysAre(node, NODE_KEYS)) throw new Error(`nodes[${index}] must be a data-only CE review node`)
...
export function validatePacket(packet) {
  ...
  if (packet.nodes.length > 32) throw new Error("packet cannot contain more than 32 review nodes")
...
export function makeWorkerPrompt(node) {
  return [
    ...
    `You own exactly one already-selected CE review node: ${node.stage}/${node.role}.`,
    "Do not invoke Agent, Task, spawn_agent, a Skill, or any other delegation primitive.",
    "Do not create, edit, or delete project files; return review evidence only.",
...
export async function executeReadWorkflow({ engine, packet, runDir }) {
  ...
  for (const [wave, nodes] of waveGroups(packet.nodes)) {
    engine.phase(`review wave ${wave}: ${[...new Set(nodes.map(({ stage }) => stage))].join(", ")}`)
