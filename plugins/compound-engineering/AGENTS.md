...
- **Hard rules** for deterministic safety. Examples from this plugin: strong-language load instructions ("STOP. Read `references/X.md` before composing"); "don't silently `cd` to another repo and write outputs there"; "headless mode skips synthesis phases entirely"; "re-present after any revision before doc-write"; "use absolute paths in chat handoff." The agent's judgment must not vary — the failure modes (silent doc-write, unreachable file references, mis-filed plans) are bad enough that mechanical adherence is right.
- **Strong guidance with examples** for judgment calls where there's a clear bias to teach. Examples: the three-bucket synthesis structure (Stated/Inferred/Out); decision-level granularity ("name the decision; don't expand it" with bad-vs-good pairs); Summary vs Problem Frame discipline; the "no fourth status" rule for open questions; Phase 3.7 anti-expansion. Concrete examples teach better than abstract principles, but anchor them at the principle level so the agent can generalize.
- **Trust** for cases where prescription would harm: how the agent does codebase exploration, how many clarifying questions to ask, when to lean on memory, how to phrase user-facing prose, how to interpret a user's invocation when context is ambiguous. Over-prescription robs the agent of intelligence and memory it has that we couldn't have anticipated at skill-design time.

Match the level to the failure mode in both directions. Over-prescribing produces rote-feeling output and forces agents past their judgment; under-prescribing produces inconsistent behavior and drifted artifacts. The right test: can you name a specific bad outcome the prescription prevents? If yes, prescription is justified — and most of this plugin's hard rules and strong guidance pass that test. If the rule exists "to be safe" without a concrete failure mode, lean toward trust.
