...
| [`/ce-brainstorm`](../../docs/skills/ce-brainstorm.md) | Interactive Q&A to think through a feature or problem and write a requirements-only unified plan before planning. Pass `output:html` to write the artifact as a single self-contained HTML file instead of markdown (exclusive — md OR html, never both) |
| [`/ce-plan`](../../docs/skills/ce-plan.md) | Create or enrich structured plans for any multi-step task -- software features, research workflows, events, study plans -- with automatic confidence checking. For brainstormed software work, enriches the same unified plan to implementation-ready. Pass `output:html` to write the plan as a single self-contained HTML file instead of markdown (exclusive — md OR html, never both) |
...
| [`/ce-work`](../../docs/skills/ce-work.md) | Execute implementation-ready work items systematically |
