...
*User Research Scan* — Run `compound-engineering:research:user-research-analyst` to surface relevant personas, interview insights, and opportunities from `docs/research/`. If findings are returned, briefly summarize them before starting the collaborative dialogue. If no research data exists, skip this silently and do not mention the absence.
