...
def find_concepts_path(explicit_path):
    ...
    # Default: look for CONCEPTS.md two levels up from this script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    repo_root = os.path.abspath(os.path.join(script_dir, "..", "..", "..", ".."))
    candidate = os.path.join(repo_root, CONCEPTS_FILENAME)
    if os.path.isfile(candidate):
        return candidate

    # Fallback: CWD
    return CONCEPTS_FILENAME
...
def find_entries(lines):
    """Return [(heading_line_index, heading_text), ...] for ## Term headings."""
    entries = []
    for i, line in enumerate(lines):
        stripped = line.strip()
        if stripped.startswith("## ") and not stripped.startswith("### "):
            entries.append((i, stripped[3:].strip()))
    return entries
