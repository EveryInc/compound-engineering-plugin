...
- **Create Issue** — Detect the project tracker and create the issue from the plan file as described under "Issue Creation" in `references/plan-handoff.md`. Do not assume Linear means a `linear` CLI; Linear may be available through a platform connector/MCP tool, documented direct API/GraphQL credentials, or a documented local CLI. After creation, display the issue URL and ask whether to proceed to `/ce-work` via the platform's blocking question tool.
