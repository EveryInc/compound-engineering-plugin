...
|------|---------|-------------------|----------|----------|
| 1. Add column | `npx drizzle-kit push` | < 1 min | N/A | Drop column |
| 2. Backfill data | `npx tsx scripts/backfill.ts` | ~10 min | 1000 rows | Restore from backup |
...
**Sample verification query (run 1 hour after deploy):**
```sql
-- Quick sanity check
SELECT COUNT(*) FROM records WHERE new_column IS NULL AND old_column IS NOT NULL;
-- Expected: 0
