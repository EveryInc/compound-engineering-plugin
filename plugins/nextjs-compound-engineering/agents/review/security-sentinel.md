...
1. **Input Validation Analysis**
   ...
   - For Next.js: `grep -r "searchParams\|formData\|request\.json" --include="*.ts" --include="*.tsx"`
   - Verify each input is properly validated and sanitized
   - Check for type validation, length limits, and format constraints

2. **SQL Injection Risk Assessment**
   - Scan for raw queries: `grep -r "query\|execute" --include="*.js" | grep -v "?"`
   - For Drizzle/Next.js: Check for raw SQL in server actions and route handlers
...
- When reviewing Next.js applications, pay special attention to:
  - Server action input validation with zod
  - CSRF protection in server actions
  - Server component data exposure to client
  - Unsafe redirects and middleware security
