...
2. **One durable decision set.** Every returned typed `needs-human` residual enters the snapshot's canonical `needs_human_residuals` set unchanged. Its complete `sources` set owns its lifetime: if any source reopens or disappears, the whole residual leaves the set and every surviving source becomes actionable again. A decision without a complete valid payload is actionable work, never a parked item. Render the set under `## Needs your decision`; leave its review threads open. Anything with no thread home also goes into one run-report PR comment, never the PR body.
