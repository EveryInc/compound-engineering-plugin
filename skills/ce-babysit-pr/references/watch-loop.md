...
```json
{
  ...
  "needs_human_residuals": ["<complete typed residual whose sources are parked as one unit>"],
...
`needs_human_residuals` is the one durable decision set across review and CI. The snapshot emits it unchanged across checkpoints, schedulers, and resumed invocations. Item dispositions and CI dispatch state only say whether each source remains parked; a residual stays in the set only while every source does. Reopening one source invalidates the grouped residual and re-actionizes every surviving source. Legacy parked records without a complete payload are re-actionized instead of being handed off without a decision.
...
- **Review threads.** A thread is actionable while it is unresolved and you have not recorded acting on it. After a resolve pass, `mark --thread <id> --disposition dispatched` handles an ordinary unresolved thread. A typed decision is parked only through the shared residual mark in `tick.md`, which validates and parks its complete source set atomically. A later reviewer comment reopens the thread; when it belongs to a grouped residual, that source change invalidates the group and reopens its surviving siblings. Every mark carries the active invocation tuple, so stale ticks cannot silence work in a newer invocation.
- **Non-thread feedback candidates** (top-level PR comments + review-submission bodies). These appear as `actionable.comments` when feedback has no inline thread. The detector excludes only empty bodies and never classifies content, authors, or posting surfaces; `ce-resolve` owns that judgment. Because there is no remote resolve, every passed candidate must either be marked `dispatched` or be covered by one validated residual mark. A dispatched candidate stays silent across body edits because status bots routinely rewrite comments; a residual-covered candidate reopens when its acted identity changes. A new comment has a new ID and is actionable. Both feedback surfaces remain one review stream for trajectory and backlog accounting.
...
- **Branch currency.** `branch_currency` is the third attention stream. Its identity binds host-qualified live base repository/ref/OID, head SHA, merge status, and route. "Mergeable against the current base" and "head contains the latest base" are separate: only `BEHIND`, `DIRTY`, a branch-protection requirement, or an explicitly selected always-current policy requires maintenance. Ordinary historical base movement with GitHub reporting `CLEAN` does not. `UNKNOWN` mergeability or any non-null `base_ref_blocker` emits no item. Managed stacks and manager/relationship `probe-error` are excluded; a `normal-base` target may be independent or an eligible target-local manual dependency, and a root with open child dependents remains allowed. Never mutate those dependents.
  ...
  Re-entry into `claimed` is reconciliation-only, never a direct resubmission. Record the observed recovery as `--currency-outcome mutation-observed`, `--currency-outcome proven-no-mutation`, or `--currency-outcome ambiguous`. Exactly one retry follows only conclusive no-mutation proof and the engine backoff. Ambiguous recovery never retries or resubmits. Confirm the same exact item with `--currency-disposition confirmed`. Every mark whose result is `needs-human`, including unchanged inspection and exhausted no-mutation recovery, includes one complete `--residual-file` in that same locked mark with source `{ id: <currency_key>, kind: "currency" }`. Reopening any grouped source preserves attempt and mutation history; when that history still requires a decision, regenerate the canonical residual rather than claiming. When the human answers a semantic park, consume the exact current decision with `--currency-answered-fingerprint` and preserve their exact response through `--currency-answer-file`; if a grouped review source exposes the answer first, reconciliation retains that payload on the unchanged currency observation until the consuming mark. The engine then retains the answer and its decision payload until confirmation, clears only the answered fingerprint, and leaves unrelated decisions parked. Never transfer a claim or decision to a moved head/base observation.

`needs_human_residuals`, `ci_dispatched`, the thread and feedback dispositions, and `branch_currency_state` are the journal. They are written by `mark` and read by `snapshot`; there is no separate crash-recovery record. An unmarked review or CI item stays actionable, a complete decision persists with all of its parked sources, and a claimed currency item stays reconciliation-only until explicitly confirmed or parked.
