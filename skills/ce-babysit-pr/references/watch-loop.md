...
**User-runnable resume syntax.** Whenever this reference tells the skill to print or copy a resume invocation, default to `/ce-babysit-pr <url>` and append the selected non-default posture plus `approval:codex` when active, so checkpoint / durable / session re-entry keeps both stack scope and completion gate. Use `$ce-babysit-pr <url> [posture:…] [approval:codex]` only when the active host is Codex or explicitly documents dollar-prefixed skill invocation. Render only the invocation as inline code and output one form only.
...
When `approval:codex` is selected, the snapshot also emits `approval_mode`, `codex_review_request`, `codex_review_request_required`, `codex_approval_signals`, `codex_approval_signal`, and `codex_approval_satisfied`. The watcher reports `codex-review-request` until the agent has issued and marked a request for the current head; it reports `codex-approval-signal` when approval evidence changes. A signal is accepted only when it is Codex-authored, current-head bound (or a post-request PR-body reaction), and newer than the recorded request.
