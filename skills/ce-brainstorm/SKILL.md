...
**When the shape matches, offer — do not silently switch** — using the platform's blocking question tool, and make the offer carry its own justification so the user chooses on the merits, not on trust. In plain words it must: name the candidate; say in one line *why* this is a decision rather than a scoping exercise (you'd be committing to a specific outside thing, not shaping something you've already chosen to build); and state what `ce-pov` returns — a decisive, project-grounded verdict on the candidate, weighing fit, migration cost, and whether it's worth it here. The two choices are: get that `ce-pov` verdict, or keep brainstorming the space here. On accept, hand off to `ce-pov` with the candidate and the question; on decline, continue to Phase 0.2. Keep it to one offer — a bare "route to ce-pov, yes/no?" stripped of the why and the value is not enough.
