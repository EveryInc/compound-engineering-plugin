...
7. **Offer `ce-prototype` when the decision is expensive to unravel.** If committing an approach would encode substantial behavior or interaction that later planning and implementation will treat as given, and talk or a cheap visual probe cannot settle it, offer `ce-prototype` once — when you recognize that bar, not at a fixed phase. Do not offer for routine UI that follows an existing pattern (adding a known button, placing a standard control). On accept, invoke `ce-prototype` via the host's normal skill-invocation mechanism, passing the named question, the surface, any hard constraints, and any artifact path; do not build it here. On decline, continue here; do not re-offer unless the decision itself changes.
...
If choosing among approaches meets Interaction Rule 7, offer `ce-prototype` there; do not run a design campaign in this skill. If the differences are display-only (shape, layout, relationship), use `references/visual-probes.md` before presenting the choice. If the Phase 0.3 tripwire fired, those differences are display-only, and no shape decision has yet been through the gate, the visual-probe offer fires here, per that reference. The visual path remains opt-in and display-only; text remains a first-class path.
