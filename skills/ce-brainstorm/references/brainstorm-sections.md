...
- **Path:** `<root>/plans/YYYYMMDDTHHMMSSZ-<type>-<topic>-plan.<md|html>` (UTC ISO 8601 basic timestamp; no daily sequence number). Reserve the path atomically; on collision, retry with the smallest available numeric suffix before the extension rather than overwriting.
...
- **`date`** — creation date in ISO 8601 (`YYYY-MM-DD`), ASCII digits only.
  This frontmatter date remains the human-readable calendar date; the filename
  uses the UTC write timestamp (`<root>/plans/YYYYMMDDTHHMMSSZ-<type>-<topic>-plan.<md|html>`).
