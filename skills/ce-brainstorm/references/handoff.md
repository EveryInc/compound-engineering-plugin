...
brainstorms (`execution` other than `code`).

Count the visible options. If the count fits the host blocking-question tool's
option cap — Claude Code `AskUserQuestion` allows 4 explicit options, Codex
`request_user_input` only 2-3 — ask with that tool. If it exceeds the cap,
render a numbered list in chat rather than trimming, since plan, ship, review,
Proof/browser, and refine are all distinct destinations; add "Pick a number or
describe what you want." so free-form input stays open. Never silently skip the
question. When no user can answer (headless or pipeline run), skip the menu,
emit the closing summary with its recommended next step, and end.
...
Open the menu with `Brainstorm complete.` and, when an artifact was created, its
absolute path plus this line verbatim:

Planning and shipping will use this artifact as the definition of what to build.

Omit both lines when no artifact was created. In blocking-tool mode, drop the
"Pick a number" hint and pass the remaining stem as the question.

When blocking questions remain and the user wants to pause, open instead with
`Brainstorm paused.` plus: *I'm holding planning until the remaining questions
are resolved — say the word and I'll proceed anyway, recording each open item as
an explicit assumption or a question deferred to planning.* That override
sentence is load-bearing: the planning options are hidden while `Resolve Before
Planning` is non-empty, so without it the user is told planning is blocked and
never told the block is theirs to lift. Hiding the option withholds the
*recommendation*, never the *choice*.
...
Selections may be the option label (or a close paraphrase) or its number in the currently-rendered list. Treat free-form input that matches no option and describes no alternative action as clarification — ask a follow-up rather than guessing.
...
autonomously — plan, implement, simplify, review and apply fixes, commit/push/open
PR, and CI watch to green. Do not also start a `/goal` or load `ce-work`
directly — `lfg` orchestrates them.
...
Report the artifact under a `Plan artifact:` line carrying the absolute path
actually written this run — `.md` for `OUTPUT_FORMAT=md`, `.html` for
`OUTPUT_FORMAT=html`, never a hardcoded `.md` path — and omit that line entirely
when no artifact was created.

When complete and ready for planning, emit "Brainstorm complete!", the artifact
line, the key decisions as bullets, then this line verbatim:

Recommended next step: `ce-plan <plan artifact path>`

If the user pauses with `Resolve Before Planning` still populated, emit
"Brainstorm paused.", the artifact line, the blocking questions as bullets under
"Planning is held on:", then that resuming `ce-brainstorm` resolves them — or they can say to
plan anyway, and each open item is recorded as an explicit assumption or a
question deferred to planning.
