...
Hand over the working context as **file paths the subagent reads itself**, never a re-narrated prose brief. If a needed piece lives only in context, **write it to a fresh scratch file** (e.g. `mktemp` under the OS temp dir). On the Claude CLI route the worker passes `--add-dir` for the OS temp root, so scratch files there stay readable even though they sit outside the launch directory:
