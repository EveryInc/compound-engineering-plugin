...
**Then offer next steps** using the platform's blocking question tool, per Interaction Rule 4 (which owns the per-harness roster and the numbered-list fallback). Never silently skip the question.
