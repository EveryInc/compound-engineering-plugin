...
When adversarial was selected and scope is `local-aligned` or standalone, read `references/cross-model-review.md` from this skill's directory in full, attest the host, resolve and sanction one fixed route, and make its required egress announcement. Separately distinguish the current execution lane from host identity and resolve launch authority as exactly one of `normal`, `approved_host_launch`, `host_denied`, or `authority_unavailable` before any credential-sensitive native peer detaches and before the exclusive local roster is finalized. Only the first two outcomes may start the detached peer job using the reference's exact invocation; persist its job ID, target, fixed route, requested model/reasoning, deterministic result path, exact input digest, authority outcome, and deadline anchor in working state. The latter two outcomes return no job and keep the in-process adversarial reviewer as the local fallback.

- If the runner returns a job ID, the peer permanently owns the adversarial lens for this run. Remove `adversarial-reviewer` immediately and never dispatch it later, even after timeout, failure, malformed schema, digest mismatch, missing identity receipt, or no usable artifact.
- If no job ID exists because of a dispatch-infrastructure failure, attempt the bounded same-route hand recovery before finalizing the roster. If recovery returns a job ID, the peer owns the lens. Only after recovery is exhausted, or when the peer was never eligible, keep the local fallback and record an environment-scoped reason. No job ID means no provider command ran, so do not claim authentication failure.
