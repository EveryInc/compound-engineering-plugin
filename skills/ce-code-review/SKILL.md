...
- **No blocking prompts.** Never use `AskUserQuestion`, `request_user_input`, `ask_user`, `ask`, or other blocking question tools. Infer intent, plan, and scope from explicit tokens, git state, PR metadata, and conversation. Note uncertainty in Coverage or the verdict — do not stop to ask.
