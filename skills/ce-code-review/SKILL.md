...
4. **Soft-bucket demotion.** After the final helper pass, inspect both surviving `findings` and the remaining `suppressed_findings`. Move a P2/P3 advisory finding supported only by `testing` to `testing_gaps`, or only by `maintainability` to `residual_risks`, using `<file:line> -- <title>`. Any other corroborating reviewer keeps it primary. Suppressed candidates routed here remain absent from primary `findings`; discard all other `suppressed_findings` after this step. Record the mode-aware demotion count.
