...
When the `frontend` signal is present (React components, TypeScript types, CSS/Tailwind, hooks, or state management in the diff), also invoke `ce-ui-engineering` to load the authoritative UI engineering conventions into context. Then read `references/frontend-review-checklist.md` from this skill's directory. Fold both into the matching personas' review prompts at Stage 4 dispatch — `ce-ui-engineering` carries the convention rules (naming, component structure, state management, JSX hygiene, styling, cleanup, Red Flags); `frontend-review-checklist.md` carries reviewer-specific checks (rendering perf, type correctness, a11y, bundle size). Authority order: repo instructions (`AGENTS.md`, `architecture.md`, `projectbrief.md`) > repo `docs/solutions/*` > `ce-ui-engineering` > `frontend-review-checklist.md` > general guidance.
