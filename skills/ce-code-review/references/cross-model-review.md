...
- In final Coverage and `mode:agent` output, include a compact per-lane `critique_receipts` entry with `lane`, caller-owned `required`, `model_requested`, `model_actual`, `receipt_source`, `receipt_status`, and `critique_status`, plus route/effort and `independence_verified` when present. Preserve valid sibling lanes when one lane fails. Keep the literal `unverified`; never compress a request into a serving claim such as "via Codex high" when actual model or effort is unverified.
