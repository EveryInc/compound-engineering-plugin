...
- In final Coverage, name `cross_model_route`, `model_requested`, `effort_requested`, `receipt_supported`, `model_actual`, `effort_actual`, and `independence_verified` from the artifact. Keep the literal `unverified`; never compress a request into a serving claim such as "via Codex high" when actual model or effort is unverified. In prose the user reads, name the peer by target and requested model; `unverified` means no receipt, not an unknown model, so add a serving caveat only when a receipt disagrees with the request or the route requested no model (Cursor default/Auto).
