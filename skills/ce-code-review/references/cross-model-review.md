...
1. Run `inspect` once and read legacy `cross_model_peer` only from `settings.effective` plus its provenance. Never reopen or parse a project/global settings file here. Normalize a conversational peer preference as current-task route intent and a non-null merged setting as `legacy` route intent; neither enters the review prompt.
2. Issue one `ce-routing/v1` `resolve_batch` for stable role `ce-code-review.adversarial-reviewer`, then freeze that snapshot, source revisions, binding, and ordered candidates for the entire pass. Reuse an inherited matching snapshot on recovery.
3. Current-task intent wins. Otherwise an explicit generalized task/role binding or `ce-default` reset wins. A project legacy intent outranks project/global class defaults; a global legacy intent outranks a global class default; narrower generalized role or project-class bindings remain higher. A legacy target synthesizes policy `prefer`: that target first, the remaining shipped default order `codex -> claude -> grok -> composer`, then the built-in no-peer outcome. With no generalized route and no legacy/current-task preference, preserve the exact old default discovery behavior.
...
- Read the artifact through the runner's verified read — `python3 "$SKILL_DIR/scripts/peer-job-runner.py" result --path <run-dir>/adversarial-<target>.json` — but keep it quarantined. Convert adapter evidence for the resolver: `model_identity_status: matched` reports the candidate's requested model token while retaining the dated served ID in adapter evidence; literal `unverified` omits the corresponding actual field; `mismatched` passes the actual ID. Do the same for effort. Call `finalize_attempt` with the frozen binding and `{ordinal, terminal:true, integrated:false}` **before consuming** findings.
- `accept` permits fold-in. A successful preferred result with absent identity evidence has receipt `accepted_unverified`. `next_candidate` is legal only for a known mismatch while retry-safe: delete/discard this output, freshly sanction and disclose the next declared recipient/material, and launch a new fixed-route job. `block` discards output and reports the required route unsatisfied without prompting. Required unavailable, mismatched, or unverified model/effort never folds in.
