...
CE_PEER_HARD_SECS="$(( PEER_HARD + 30 ))" CROSS_MODEL_HOST_HARNESS="<host-harness>" CROSS_MODEL_FIXED_ROUTE="<fixed-route>" "$PY" "$SKILL_DIR/scripts/peer-job-runner.py" start --skill ce-code-review --run-id "<run-id>" --label adversarial -- env CROSS_MODEL_HARD_SECS="$PEER_HARD" CROSS_MODEL_HOST_HARNESS="<host-harness>" CROSS_MODEL_FIXED_ROUTE="<fixed-route>" bash "$SKILL_DIR/scripts/cross-model-adversarial-review.sh" "<host-serving-family>" "<target>" "<base-ref>" "<run-dir>"
```

The three nested windows are one budget with one knob. `CROSS_MODEL_HARD_SECS` (default 1200s) is the script's own backstop; the runner supervisor gets `+30s` and the orchestrator's shared deadline is `+10s`, both derived above so a user who raises the knob raises all three. Never hardcode any of the three: when they drift, the tightest one silently reaps a healthy peer and the peer's full spend is wasted. Read the printed `peer-deadline-secs=<n>` and use that `<n>` as the shared deadline below.
