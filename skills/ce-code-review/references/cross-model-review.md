...
Keep requested **target**, CLI **harness/intermediary**, serving **family/provider**, and served model separate. `cursor` means `cursor-agent` with its configured default/Auto model and no `--model` flag. `composer` means an explicit Composer-family model through Cursor. `grok` prefers its native CLI; Grok through Cursor is a distinct route and recipient. `glm` means the ZCode CLI with an allowed GLM main model selected in ZCode's own configuration; that configured request does not prove the serving family.
...
The peer runs on **one editorially selected model and reasoning tier per provider**. The concrete model IDs and route effort flags live in one mapping in `scripts/cross-model-adversarial-review.sh`; this reference does not duplicate them. Claude Opus and native Grok currently use high, Codex uses extra-high; cursor-agent routes use their model-implied tier or ceiling. ZCode is the documented exception: the route validates and records its configured GLM request, but the CLI does not expose a served-family, served-model, or reasoning-effort receipt, so all remain unverified and the return receives no independent-agreement promotion. Users choose the peer target, and may pin supported targets through `cross_model_model` / `cross_model_effort` (Step 1); the script validates and never substitutes. Never inherit an unspecified harness default. A lower tier is adopted only after a discriminating effectiveness eval, never from cost alone.
...
- **Interactive host, default mode:** surface a **prominent standalone line** that frames it as a **cross-model adversarial review** (say "cross-model" — not the internal "peer" jargon), names the requested **model and reasoning level** from the in-script mapping, and — because two different models can arrive over the *same* `cursor-agent` CLI — names **the route as well as the model** for cursor-agent routes, and states that reviewed code/diff content is sent to that provider. Say **independent model** only where the serving-family receipt supports it; otherwise say **cross-harness review; independence unverified**. **Announce wording follows the receipt:** name a model as serving only where the route carries a served-model receipt; on receipt-less routes say "requested <model> at <effort>; serving model/effort unverified on this route." Placed with the Stage 3 team announce, not buried after it.
...
- `<target>` is one of `codex`, `claude`, `grok`, `glm`, `cursor`, or `composer`; `<fixed-route>` is its already-sanctioned concrete route token from the Step 1 table (`codex`, `claude`, `grok-cli`, `grok-cursor`, `zcode`, `cursor`, or `composer`).
...
The peer reviews the **current work tree** (read-only) against `git diff <base-ref>`. Reviewed code/diff content is sent to an external model provider (OpenAI, Anthropic, xAI, Cursor, or the provider endpoint configured for ZCode, depending on the resolved peer). The ZCode route does not attest that endpoint's operator. `CROSS_MODEL_PEERS` restricts which targets and intermediaries may receive content; for ZCode it sanctions the configured endpoint without identifying its operator.

**Isolation differs from ce-doc-review by design.** Doc-review embeds a self-contained document into a tool-less empty scratch. Most code-review peers need surrounding code context, so they run **in-tree read-only**; ZCode is the fail-closed exception because its workspace root is not a path sandbox:
...
- **zcode:** empty-scratch cwd plus a non-matching `--allowed-tools` allowlist, with the core read/mutation/shell/web tools also explicitly denied; only the complete attached diff and host-vetted brief are available. The allowlist excludes dynamically registered MCP/plugin tools by default. Oversized diffs that require selective repository reads skip before egress.
