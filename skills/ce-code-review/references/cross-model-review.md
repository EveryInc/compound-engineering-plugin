...
Runs the **adversarial** review through one separately routed model target in a read-only process. The peer gets the **same** `references/personas/adversarial-reviewer.md` brief the in-process reviewer uses, returns the same `findings-schema.json` shape, and folds into Stage 5 as reviewer `adversarial-<provider>`. It counts as independent corroboration and can promote agreement only when its receipt records `independence_verified: true`, `critique_status: usable`, and `receipt_status: matched`; otherwise it remains attributed review evidence without a promotion bonus.

## Critique author receipt gate

The centralized pass predicate is: `receipt_version: critique-author/v1` AND `critique_status: usable` AND `receipt_status: matched`. Missing fields, legacy artifacts, and tampered values are `unverified`; never fill `model_actual` from `model_requested`. Apply the predicate before agreement promotion, validator shortcuts, any automatic fix path, or a clean-review claim. `independence_verified` remains an additional requirement for different-model corroboration, never a substitute for the author receipt.

Promotion is deliberately available only to routes that expose an authoritative author receipt. Today the Claude stream route does; a non-Claude peer without an equivalent receipt remains diagnostic even when it is independently routed. Consequently, a Claude host paired with a non-Claude peer gets findings but no agreement promotion. This host-specific limit is intentional fail-closed behavior until that route can prove its serving author.

Requiredness is consumer-owned. The caller decides whether this adversarial lane is required for its workflow. A producer's `required: false` describes the producer default and must not downgrade a caller-declared required lane. An optional unmatched lane stays diagnostic and non-blocking; a required unmatched lane fails that caller's gate.
...
Preferred mappings run first. Only after an observed unavailable, obsolete, or incompatible model may the host choose the closest compatible same-target/same-family replacement. Bind it with `CROSS_MODEL_MODEL_OVERRIDE_TARGET=<target>` and `CROSS_MODEL_MODEL_OVERRIDE=<model-id>`. The Claude code-review lane is adversarial and therefore accepts only the `opus` alias or an exact `claude-opus-*` replacement; reject Fable, Sonnet, Haiku, and other-family overrides before egress. Never leak an override to another route, silently change an explicit model, or add a recipient.

## Step 2 — Provider model + reasoning tier (owned by the script)

The peer runs on **one editorially selected model and reasoning tier per provider**. The concrete model IDs and route effort flags live in one mapping in `scripts/cross-model-adversarial-review.sh`; this reference does not duplicate them. Claude code adversarial remains **Opus at high effort**. Document product/whole-document lanes may request Fable under their separate policy; this code lane does not. POV likewise remains an explicit **Opus compatibility exception**. Native Grok uses high, Codex uses extra-high; cursor-agent routes use their model-implied tier or ceiling. Users choose the peer target, not an arbitrary model/effort matrix. Never inherit a harness-configured default model. A lower tier is adopted only after a discriminating effectiveness eval, never from cost alone.
...
- Read the artifact through the runner's verified read — `python3 "$SKILL_DIR/scripts/peer-job-runner.py" result --path <run-dir>/adversarial-<target>.json`. Its findings enter ordinary dedup, but agreement promotion is allowed **only when `independence_verified` is `true` and the critique author receipt gate passed**. A false or absent value may contribute findings but never raises confidence. Peer findings never grant silent-apply authority.
- In final Coverage and `mode:agent` output, include a compact per-lane `critique_receipts` entry with `lane`, caller-owned `required`, `receipt_version`, `model_requested`, `model_actual`, `receipt_source`, `receipt_status`, and `critique_status`, plus route/effort and `independence_verified` when present. Preserve valid sibling lanes when one lane fails. Keep the literal `unverified`; never compress a request into a serving claim such as "via Codex high" when actual model or effort is unverified.
- **Never started / not run** — the job was never started (gates not met, host un-attestable, no different provider reachable, CLI missing/unauthed): the pass simply didn't run. Note "cross-model pass: not run" in Coverage for human-facing markdown. In both human and `mode:agent` output, keep the selected lane visible with `receipt_version: critique-author/v1`, caller-owned `required`, the requested model when known, `model_actual: unverified`, `receipt_source: unverified`, `receipt_status: unverified`, and `critique_status: skipped`. Ignore any `*.raw.json` leftovers — they are not fold-in artifacts.
...
- **Ran but produced no usable output** — the job reached `done` (or any terminal state) yet no `adversarial-<provider>.json` exists (the peer ran and egressed but returned nothing schema-shaped — unparseable output, empty findings the script dropped). Distinct from not-run: note "cross-model pass: peer ran, no usable output" in human-facing markdown Coverage and emit the lane's normalized receipt with `critique_status: failed` and unverified actual/source/status. Never fail the review.
- **Started but not `done`** — the final status read reports `failed`, `timeout`, or `died-without-result` (a job reaped at the 610s deadline records `timeout`, with the reap noted in its reason) → still non-blocking, but never silent: name the peer and its terminal state in Coverage (e.g. "cross-model adversarial peer: timeout") and emit the lane's normalized failed receipt. Silent absence stays correct only in prose for passes that never started or were skipped; their receipt entry remains present.
