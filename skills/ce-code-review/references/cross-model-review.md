...
Keep requested **target**, CLI **harness/intermediary**, serving **family/provider**, and served model separate. `cursor` means `cursor-agent` with its configured default/Auto model and no `--model` flag. `fable` means Fable 5 high-thinking through Cursor's explicit `fable-cursor` route; it is a Claude-family target but sends the diff to both Cursor and Anthropic and is marked NO ZDR by Cursor. `composer` means an explicit Composer-family model through Cursor. `grok` prefers its native CLI; Grok through Cursor is a distinct route and recipient.
...
1. A preference the user **states in conversation** (e.g. "use Fable 5 through Cursor for the cross-model pass").
...
If a preference from steps 1–3 resolves to the attested host serving family, disclose the family collision and continue to the next eligible preference/default target before fixing a route. Do not dispatch a same-family target or freeze an ineligible route merely because it was preferred.

Before egress, resolve the target to one concrete installed route, verify every recipient against `CROSS_MODEL_PEERS`, announce it, and pass it as `CROSS_MODEL_FIXED_ROUTE`. A failed route returns no artifact and never changes provider or intermediary internally. A retry is a new disclosed and sanctioned dispatch. `fable` resolves only to `fable-cursor`; never silently substitute native Claude or Cursor Auto. For backward compatibility, either `cursor` or `composer` in `CROSS_MODEL_PEERS` sanctions Cursor as an intermediary, but selecting Cursor-default requires target `cursor`; an explicit allowlist needs both `fable` and Cursor sanction for Fable-via-Cursor, and `grok` alone never sanctions Grok-via-Cursor.
...
The peer runs on **one model per provider at high reasoning** (composer's `-fast` tier is its ceiling — an accepted exception). The concrete model IDs and per-route reasoning flags live in a **single mapping in the script** (`scripts/cross-model-adversarial-review.sh`, the `M_CODEX`/`M_CLAUDE`/`M_GROK`/`M_GROK_CURSOR`/`M_FABLE_CURSOR`/`M_COMPOSER` constants and the `adapter_argv` builder). This reference deliberately does **not** restate the IDs — one source of truth prevents the reference and script from drifting. The IDs are the current instance of the tier principle (a single maintenance point), not the contract.

The script always uses the adversarial persona brief; fold-in forces `reviewer` to `adversarial-<provider>`.

## Step 3 — Announce

- **Interactive host, default mode:** surface a **prominent standalone line** that frames it as an **independent cross-model adversarial review** (say "cross-model" / "independent model" — not the internal "peer" jargon), names the concrete **model and reasoning level** from the in-script mapping (e.g. GPT-5.6-sol at high reasoning, Opus at high, Grok 4.5 at high, Fable 5 at high-thinking, Composer 2.5-fast), and — because different models can arrive over the *same* `cursor-agent` CLI — names **the route as well as the model** for cursor-agent routes so Grok-4.5-via-cursor-agent, Fable-5-via-cursor-agent, Composer-via-cursor-agent, and Grok-4.5-via-the-grok-CLI are unambiguous, **and states that reviewed code/diff content is sent to that provider** (third-party egress; for cursor-agent routes the egress is to Cursor *plus* the serving provider). For Fable also disclose Cursor's NO ZDR designation. **Announce wording follows the receipt:** name a model as serving only where the route carries a served-model receipt; on receipt-less routes say "requested <model>; serving model unverified on this route" instead of asserting the concrete model. Placed with the Stage 3 team announce, not buried after it. Wording is yours; the falsifiable requirements: prominent, reads as a **cross-model reviewer**, names the requested model (with the unverified marker on receipt-less routes), names the route when it is cursor-agent, names the egress.
...
- `<target>` is one of `codex`, `claude`, `grok`, `fable`, `cursor`, or `composer`; `<fixed-route>` is its already-sanctioned concrete route (`fable` uses only `fable-cursor`).
...
On macOS, an authenticated `cursor-agent` can fail inside a host sandbox with `SecItemCopyMatching failed -50` because the sandbox cannot read Cursor's Keychain credential. Treat that exact failure as Keychain isolation, not an authentication failure: request the host's narrow approved unsandboxed execution path for the single detached-runner start call and retry once. Never copy, print, or relocate Cursor credentials to bypass the sandbox.
...
- A finding sharing a fingerprint with in-process `adversarial` promotes only when the artifact records `independence_verified: true`. Cursor-default and Fable-via-Cursor artifacts default false because Cursor emits no served-model receipt; an unattested host skips automatic dispatch.
...
- **grok / cursor-agent (including Fable):** ask/dontAsk + no write/force/yolo; Read allowed; workspace/cwd at the repo root.
