...
Show the fast pass only when it finds an urgent P0/P1 candidate, under a clearly preliminary header (e.g. `### Fast pass (preliminary — deep review in progress)`) as a short list of `severity — file:line — what`, plus one line stating they are unverified and will be deduplicated into the final report. Keep P2/P3 candidates internal until the final report. Do **not** assign stable `#` numbers here. In `mode:agent` emit no preliminary block — that response must be a single raw JSON object with nothing before it — but still seed the scan's findings into Stage 5.
