...
The artifact file **must** carry the full detail-tier fields (`why_it_matters`, `evidence`) **and** `pre_existing` under those exact schema keys — not synonyms such as `notes`. The compact *return* omits all detail-tier fields **except `first_evidence`**, but every compact finding still includes merge-tier `pre_existing` as a boolean. Writing the compact shape, or a free-form `notes` stand-in, to the artifact (a common reviewer slip) silently strips the detail Coverage and the keyed detail lines depend on. However review context is delivered — inlined, or staged to disk for a large diff — each reviewer still receives the full subagent-template output contract; staging context never licenses a thinner one. `suggested_fix` is optional in both tiers -- included in compact returns when present so callers can apply fixes after review. If the file write fails, the compact return still provides everything the merge needs.
