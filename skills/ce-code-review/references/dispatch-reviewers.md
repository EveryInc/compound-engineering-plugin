...
After the inline fast pass has completed and the local reviewer batch has started, prepare synthesis inputs while reviewers run. Do not poll the peer during that wave. After local reviewers finish, if Stage 3d persisted a peer job ID, perform the reference's bounded status/wait/reap sequence. A failure stays non-blocking and never triggers a late in-process adversarial dispatch. If the bounded progress sidecar classifies the first attempt as `productive_scope_timeout`, the reference permits one same-route, same-model, same-base, same-hard-cap retry from a mechanically narrower single-division scope; unchanged scope, widened paths, or budget escalation fail before egress. Fold only a terminal normalized schema-shaped peer artifact. Progress, event logs, heartbeats, usage, and timeout sidecars are non-finding evidence and cannot enter synthesis.

The terminal peer return enters Stage 5 as reviewer `adversarial-<provider>`, like any persona artifact. A pass that never started is recorded as not run or covered by the in-process fallback. A started peer that fails, times out, dies, or exhausts its one narrowed retry is named with its exact terminal state rather than vanishing silently. Agreement promotion still requires top-level `independence_verified: true`.
