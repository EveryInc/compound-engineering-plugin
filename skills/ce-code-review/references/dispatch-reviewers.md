...
After the inline fast pass has completed and the local reviewer batch has started, prepare the finish input while reviewers run (`references/finish-input.md`). Do not poll the peer during that wave. After local reviewers finish, write the peer's job ID, target, route, start epoch, deadline, and preference source into `finish-input.json` and do not touch the peer again: the merge leaf performs the reference's single bounded status/wait/reap sequence, folds in whatever terminal artifact is available, attributes from the artifact, and cleans up through the runner. Only this context may change a recipient; when the merge leaf's receipt says the in-process adversarial lens is required, dispatch `adversarial-reviewer` here, append its return to `raw-returns.json`, and re-run the merge leaf. A failure or timeout stays non-blocking and is named in Coverage. It never triggers a late in-process adversarial retry except when the owning fold-in rules in `references/cross-model-review.md` require the did-not-run fallback or the in-process restore after a failed same-route rate-limit retry. Peer findings enter ordinary synthesis, but agreement promotion requires top-level `independence_verified: true`; false or absent independence is useful evidence, not different-model corroboration. Coverage must say whether the adversarial lens ran cross-model or used the in-process fallback.
