...
**Exception — tree-mutating reviewers.** Dispatch `testing` (and any other persona whose method mutates the tree) with `isolation: "worktree"` so it gets its own copy. Read-only siblings stay on the shared checkout. Mutation testing on the shared tree is forbidden: a concurrent reviewer can observe a transient write as if it were the diff. The isolated reviewer must receive a faithful snapshot of the reviewed tree — native worktrees start from committed `HEAD`, so for `local-aligned` reviews that include staged or unstaged changes, materialize a scratch snapshot (or another host-supported copy that preserves those changes) before isolating `testing`. Isolation that inspects stale committed `HEAD` is invalid for that scope. If the host cannot isolate, the testing persona must copy-to-scratch rather than edit in place.
