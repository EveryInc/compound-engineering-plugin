...
**Protected artifacts.** `docs/brainstorms/*`, `docs/plans/*.{md,html}`, and `docs/solutions/*.md` are compound-engineering pipeline artifacts. Reviewers reliably read them as stale clutter — discard during synthesis any finding that recommends deleting, removing, or gitignoring a file in those directories.
