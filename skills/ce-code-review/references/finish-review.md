...
5. **Protected artifacts are respected.** Discard any finding that recommends deleting or gitignoring a CE pipeline artifact — a file directly inside the artifact root's `plans/` or `solutions/` subdirectory (root is `docs` by default or the configured `docs_root`), or the legacy `brainstorms/` one — per the Protected Artifacts rule in SKILL.md. Anchor to the artifact root, not a bare segment: a `references/personas/` skill asset is not a protected artifact.
