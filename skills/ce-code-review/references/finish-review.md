...
Each `critique_receipts` entry carries `{ "lane", "required", "model_requested", "model_actual", "receipt_source", "receipt_status", "critique_status" }`. Requiredness comes from the caller/consumer contract, not the producer artifact. Missing or legacy `receipt_version` and malformed or tampered fields normalize to `model_actual: "unverified"` and `receipt_status: "unverified"`; never copy the request into the actual-author field. Preserve every sibling lane entry even when one required lane blocks a clean verdict.
