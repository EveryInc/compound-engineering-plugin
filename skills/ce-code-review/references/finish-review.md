...
5. Consume the verdicts. Before reading any verdict, classify every selected finding yourself from the actual failure it alleges, using the eight protected subjects `references/validator-batch-template.md` defines. Read category, title, and body together; keywords alone never establish protection, and a naming preference about a token helper is not a token-handling defect. A valid `protected_subject` key from the validator adds protection, never removes it; `null` is a valid value that remains subject to your own classification, while a missing field or an invalid key is malformed output rather than a correction. Match every verdict to its original input `#` before using its contents. For a missing or malformed entry whose input number is unambiguous, apply failure handling to that original finding and keep the valid verdicts for unaffected inputs; never guess which finding an unknown or ambiguous number refers to, and when the response cannot be mapped reliably treat the whole batch as validator infrastructure failure. Then apply these outcomes:

   - **`confirmed`:** retain the finding with its existing severity, confidence, and action route. Confirmation grants no new apply authority.
   - **`rejected`, unprotected:** drop the finding and record the reason.
   - **`rejected`, protected subject:** accept the rejection only when `reason` cites one of the policy's evidence forms: a quoted refuting line with file and line number, version-specific or configuration-specific documentation naming the version in force, short-hash provenance establishing unrelated pre-existing code, or a discriminating test result that names the reviewed revision, runtime version, configuration, exercised trigger, assertion, and observed result. An uncited, self-referential, or "no supporting evidence found" rejection is not a rejection: reroute it through the unresolved rule below and record the reclassification.
   - **`unresolved`, protected subject or P0/P1:** keep it visible as validation-degraded. Add `validation_status: "unresolved"` and `validation_reason`, preserve its stable `#`, severity, and confidence, and route it report-only with `autofix_class: "advisory"`, `owner: "human"`, and `requires_verification: true`. Name the evidence or authority needed next. A `budget exhausted, uninspected` reason is validator infrastructure failure for that finding alone and takes this route.
   - **`unresolved`, unprotected P2/P3:** remove it from primary findings and record the inspection gap and its reason in Coverage. Do not label it disproven.
