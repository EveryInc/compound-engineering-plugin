...
**Report completion gate:** do not finish until stable `#` identifiers appear on every primary finding and the report contains `### Actionable Findings`, `### Coverage`, and `### Verdict` (or their exact JSON fields in `mode:agent`). Coverage must name the cross-model outcome and validator shortcut/batch outcome. Carry the compact per-lane `critique_receipts` summary from `references/cross-model-review.md`; a cross-model lane is successful only when `receipt_version: critique-author/v1`, `critique_status: usable`, and `receipt_status: matched`. The Actionable section must include every `downstream-resolver` finding; never silently replace it with a count.
...
Each `critique_receipts` entry carries `{ "lane", "required", "receipt_version", "model_requested", "model_actual", "receipt_source", "receipt_status", "critique_status" }`. Requiredness comes from the caller/consumer contract, not the producer artifact. Missing or legacy `receipt_version` and malformed or tampered fields normalize to `model_actual: "unverified"` and `receipt_status: "unverified"`; never copy the request into the actual-author field. Preserve every selected lane entry—even a never-started lane as `critique_status: "skipped"`—and every sibling lane when one required lane blocks a clean verdict.
