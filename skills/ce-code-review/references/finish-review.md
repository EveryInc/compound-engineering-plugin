...
Assemble the final result. **Default:** write a full technical report to `report.md`, then present a separate plain-English owner summary. **`mode:agent`:** skip both markdown files and emit JSON (see ### JSON output format) — the structured fields are how a downstream agent consumes the review.

**Technical report completion gate:** do not finish until stable `#` identifiers appear on every primary finding and `report.md` contains `### Actionable Findings`, `### Coverage`, and `### Verdict` (or their exact JSON fields in `mode:agent`). Coverage must name the cross-model outcome and validator shortcut/batch outcome. The Actionable section must include every `downstream-resolver` finding; never silently replace it with a count.

**Before writing, load both output templates.** Mirror `references/review-output-template.md` in the technical `report.md`. Mirror `references/owner-summary-template.md` in the default user response and in `summary.md`. The technical report is the evidence record. The owner summary is the decision surface.
...
Write the technical report in an ASD-STE100 Simplified Technical English (STE)-inspired style. Use short, direct sentences. Keep one consequence, action, or supporting idea per sentence, and use one consistent term for each concept. Preserve exact code identifiers, error text, and domain terms in that artifact. Preserve coverage, evidence, technical depth, and every distinct consequence or required action there.
...
- **Make the technical closing self-sufficient.** The technical artifact ends with the Verdict and Actionable list. The owner summary does not repeat those internal fields. It translates them into readiness, user impact, required actions, and missing proof.
...
- **The technical Verdict and Actionable list are present, last, and self-sufficient.** This is satisfied inside `report.md`. The default owner summary follows *Emit owner summary* in `SKILL.md` and does not repeat technical fields.
...
**Final check before delivering (default only).** First, verify the technical artifact: no box-drawing or per-item horizontal-rule separators (`────`), no Unicode arrows or middot (`·`), stable `#`s across sections, literal `|` escaped (`\|`) in table cells, and a self-sufficient technical closing. Then verify the owner summary against `owner-summary-template.md`: it starts with readiness and user impact; it contains every decision-relevant action; it contains no priority codes, requirement IDs, file paths, line numbers, code symbols, reviewer or model names, confidence values, route names, or validation machinery; and it offers technical details only on request. Write that exact summary to `summary.md` and return it as the only default user-facing result. Skip when `mode:agent` is active.
