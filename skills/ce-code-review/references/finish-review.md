...
1. Skip a validator only when the finding has `first_evidence` and both an ordinary reviewer plus a `ran_verified_independent` adversarial peer. That status requires `independence_verified: true` plus a verified model receipt. Same-model or unverified-identity corroboration never licenses this shortcut. Record the skip count in Coverage.
...
10. **Coverage.** First emit the stable cross-model envelope fields `label`, `target`, `fixed_route`, `authority_outcome`, `terminal_state`, `artifact_verified`, `identity_verified`, `input_digest`, `status`, `reason`. Artifact verification requires terminal `done`, `result <job-id>`, full schema, exact reviewer, matching artifact/runner job ID, and matching digest; identity verification additionally reconciles route/target/harness/requested/served identity. Status is exactly `verified`, `ran_attributed`, `local_only`, `host_denied`, `auth_failed`, `timeout`, `quota_limited`, or `unusable_output`; reason is redacted and environment-scoped, and quota wins over authentication when both match. Then include applied count, suppression/demotion/validator counts, residual risks, testing gaps, failed/timed-out reviewers, and inferred-intent uncertainty as applicable. Preserve existing lite-roster, validator, settlement, and removable-surface reporting requirements.
