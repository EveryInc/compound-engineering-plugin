...
4. Run the validator batch foreground with background execution off — that call is the wait, so the Stage 4 anti-poll rules apply unchanged.
...
**Before writing, load `references/review-output-template.md` and mirror its section skeleton** — that file is the canonical skeleton for *which sections appear and in what order*; its example shows one good rendering, not the only permitted layout.

**Presentation direction — optimize for the reader's next action, not a fixed layout.** The report is *acted on*: by a human deciding what to fix and whether to merge, or by a downstream agent applying fixes.

- **Per finding, make four things unambiguous:** *what & where* — one scannable line, the symptom + `file:line`; *why it matters* — what breaks or who's hit, never a restatement of the code; *what response it needs* — a bug states its fix, a **design call** presents the options and the tradeoff without forcing one answer, a coverage gap names the test and precedent to mirror, a residual risk is marked informational, an applied item gives what changed and how it was verified; *how sure* — confidence, and whether cross-reviewer or cross-model agreement corroborated it.
- **Let the shape serve the finding type but stay consistent within a section.** A terse table, a short keyed block, or a compact list are all fine.
- **Group by the unit of work or decision, not just severity.** Surface the split: **decisions a human must make** (design calls, ambiguous semantics) vs **mechanical work that can just be done** (tests, dedup, concrete fixes) vs **informational** (residual risks) — an agent clears the mechanical work and must stop at decisions. Name the order/dependency for a shared fix path ("decide X once -> resolves #1 and #7; do #1 first").
- **Cover every finding, in the least that lets the consumer act.** **Do not paste file contents or re-print the diff** — cite `file:line` and spend words on what the diff can't show (why it breaks, the fix, the repro). This governs expression, never coverage: never drop a finding or its why/fix to be shorter, and match weight to weight.
- **The bottom is the most-read screen.** The **Verdict and Actionable list must stand alone without scrolling**: the verdict plus the single most important thing to do, then the prioritized actionable list where each item carries severity, `file:line`, the terse what, and its response-type. The findings above are drill-down evidence, referenced by stable `#`.
...
**Protected artifacts.** A compound-engineering pipeline artifact is any file under a `plans/`, `solutions/`, or legacy `brainstorms/` directory whose immediate parent is the artifact root — a directory named `docs`, or the configured `docs_root` when this run resolved it. Categories nest (`solutions/<category>/`); a `references/personas/` skill asset, parented by `references`, is not a protected artifact. Reviewers reliably read these artifacts as stale clutter — discard during synthesis any finding that recommends deleting, removing, or gitignoring one.
