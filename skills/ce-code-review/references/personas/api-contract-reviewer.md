...
- **Missing versioning on breaking changes** (SemVer: a breaking change is a major bump) -- a breaking change shipped without a version bump, deprecation period, or migration path. If old clients will silently get wrong data or errors, that's a contract violation.
