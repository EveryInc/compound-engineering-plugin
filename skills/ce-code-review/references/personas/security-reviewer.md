...
- **Feature-gate leaks** (CWE-1244 improper internal access) -- a diff that makes a flag-gated, internal, or unreleased feature reachable without its gate: a default flipped on, a guard dropped from one call path while sibling paths keep it, or a route registered outside the gated block.
