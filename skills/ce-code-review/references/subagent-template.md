...
- In `manifest-scoped review`, keep target scope and context scope separate. Findings may target only manifest paths. Context-only paths are not review targets and must not appear in finding `file` fields unless they are also manifest paths. The only v1 context-only read exception is for the `project-standards` reviewer reading exact `<standards-paths>` entries as rule sources; other reviewers do not inherit that exception.
- For `project-standards` in manifest-scoped review, AGENTS.md / CLAUDE.md paths from `<standards-paths>` may be quoted in artifact `evidence` or `why_it_matters` as governing rules, but the finding must point to the violating manifest file. Do not discover arbitrary neighboring files, recurse through standards directories, read unrelated source files, add standards files to the manifest, or suggest modifying a context-only standards file.
- In `pr-remote` or `branch-remote` scope, do not read local workspace standards files as authoritative for the reviewed head. Read standards context with `git show <pr-head-ref>:<standards-path>` or `git show <branch-head-ref>:<standards-path>` when a head ref is provided; otherwise use only explicitly supplied standards content or report degraded standards coverage.
