...
const SUBAGENT_AUTHORIZATION = [
  ...
  'A dispatch the harness rejects before the agent launches — an unsupported option, a schema validation error,',
  'an incompatible argument — is a correctable invocation error, not a failed pass:',
  'correct or drop only the rejected optional arguments, keep the prompt, scope, and required capabilities intact,',
  'and retry once. A failure after the agent launched is not retried this way.',
