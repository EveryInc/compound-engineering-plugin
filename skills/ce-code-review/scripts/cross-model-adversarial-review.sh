...
adapter_argv() {
  case "$1" in
    codex)
      # ce-dispatch-site:ce-code-review.adversarial-cli-codex
    ...
    claude)
      ...
      # ce-dispatch-site:ce-code-review.adversarial-cli-claude
    ...
    grok-cli)
      ...
      # ce-dispatch-site:ce-code-review.adversarial-cli-grok
      ...
      ;;
    grok-cursor)
      # ce-dispatch-site:ce-code-review.adversarial-cli-grok-cursor
      ...
      ;;
    cursor)
      # ce-dispatch-site:ce-code-review.adversarial-cli-cursor
      ...
      ;;
    composer)
      # ce-dispatch-site:ce-code-review.adversarial-cli-composer
...
if [ -z "${CROSS_MODEL_DRY_RUN:-}" ]; then
  [ -n "${CROSS_MODEL_FIXED_ROUTE:-}" ] || skip "host must resolve one fixed route before egress; skipping"
  validate_candidate_selectors "$CROSS_MODEL_FIXED_ROUTE" || skip "routing candidate selectors are unsafe or incompatible with fixed route '$CROSS_MODEL_FIXED_ROUTE'; skipping before egress"
fi
...
run_timeout_cmd() {
  ...
  set -m
  if [ -n "$TO_BIN" ]; then
    ( cd "$PEER_WORKDIR" && exec "${MIN_ENV[@]}" "$TO_BIN" -k 10 "$HARD_SECS" "${CMD[@]}" ) < "$stdin_file" > "$PEERLOG" 2>"$PEERERR" &
  else
    ( cd "$PEER_WORKDIR" && exec "${MIN_ENV[@]}" perl -e 'alarm shift; exec @ARGV' "$HARD_SECS" "${CMD[@]}" ) < "$stdin_file" > "$PEERLOG" 2>"$PEERERR" &
  fi
  local pid=$!
  ACTIVE_PEER_PID="$pid"
  [ "$prev" = 0 ] && set +m
...
run_provider() {
  ...
  if [ -s "$RAW_OUT" ]; then
    ...
    if jq --arg r "adversarial-$provider" --arg route "$ACTUAL_ROUTE" \
         ...
         --arg identity "$MODEL_IDENTITY_STATUS" \
