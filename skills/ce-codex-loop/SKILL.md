...
Before any mutation, normalize the plan path once and keep both values:

- `raw_plan_argument` is exactly what the user supplied.
- `canonical_plan_path` is the normalized repo-relative POSIX path used by the workflow.

To produce `canonical_plan_path`, resolve the repository root, resolve relative paths against that root, resolve dot segments and filesystem symlinks, require the final target to exist, be readable, be a regular file, and remain inside the repository root, then convert the resolved target back to a repo-relative POSIX path without `.` or `..`. Accept repo-relative, `./`-prefixed, absolute-in-repo, and lexical-equivalent paths such as `docs/plans/../plans/foo.md`. Reject repo escapes, symlinks whose target escapes the repository, directories, unresolved placeholders, and missing files.
...
- `ce-code-review mode:agent plan:<canonical-plan-path> base:<stable-base> manifest:<manifest-path> run-id:<run-id> artifact-dir:<artifact-dir>` exists and is report-only.
- `ce-compound mode:headless` is available for post-success compounding.

Resolve each downstream skill through the host skill mechanism before use. Invoke `ce-work`, `ce-simplify-code`, `ce-code-review`, and `ce-compound` via the platform Skill tool or native skill invocation primitive, preserving the exact public argument strings above. Do not execute skill names as shell commands and do not narrate the handoff as plain text.
...
1. **Preflight.** Normalize the raw plan argument into `canonical_plan_path`, parse the canonical plan, reject invalid input, extract planned scope, resolve the downstream public tokens above, and record the verification commands implied by the plan.
2. **Snapshot.** Capture `HEAD`, staged entries, unstaged paths, untracked paths, and one stable review base before any mutation. For local loop review, `stable_review_base` is the captured pre-mutation `HEAD` snapshot unless the user explicitly supplied another safe base before the run. Do not use a moving branch merge-base after implementation has changed the tree. This `stable_review_base` is passed unchanged to every review attempt.
...
4. **Implementation.** Invoke the resolved `ce-work` skill through the host skill mechanism with `mode:implementation-only <canonical-plan-path>` and require a structured implementation result. `already_satisfied` is terminal only when it includes proof and identified files.
...
6. **Simplification.** Invoke the resolved `ce-simplify-code` skill through the host skill mechanism with `mode:structured manifest:<manifest-path>` and the refreshed loop-owned manifest only. If relevant verification cannot be identified, return terminal `unverified`. If verification fails, return terminal `failed`.
7. **Review loop.** Invoke the resolved `ce-code-review` skill through the host skill mechanism with `mode:agent plan:<canonical-plan-path> base:<stable-base> manifest:<manifest-path> run-id:<run-id> artifact-dir:<artifact-dir>`, the canonical plan path, the current refreshed loop-owned manifest, and a per-attempt artifact directory only. Every review attempt uses the same canonical plan path and stable base, and that stable base remains the captured snapshot HEAD unless an explicit safe base was supplied before mutation. `run-id` is the logical review identifier; `artifact-dir` is the exact directory used for `review.json` fallback and must match the returned `artifact_path`. Requirements completeness is evaluated from the explicit canonical plan, never inferred from branch diff or prose. The returned review JSON must include top-level `plan_path` equal to `canonical_plan_path`, top-level `plan_source: "explicit"`, non-null `requirements_completeness`, `manifest_path` equal to the supplied manifest path, and `reviewed_manifest` exactly equal to the manifest supplied for that attempt. Do not compare review `plan_path` with `raw_plan_argument`. Primary JSON, `review.json`, and `metadata.json` must agree on `manifest_path` and `reviewed_manifest`. Any missing, malformed, inferred, stale, or mismatched plan or manifest context is terminal `failed`; stop before review fixes, another review attempt, final verification, or compound. The exact manifest supplied to the final clean review attempt becomes terminal `reviewed_manifest`. Clean review requires all three predicates: status == complete, verdict == Ready to merge, and actionable_findings.length == 0.
...
Every terminal report includes `raw_plan_argument`, `canonical_plan_path`, `plan_path` as the canonical path alias, stable base, planned scope, manifest checkpoints, `current_manifest`, `reviewed_manifest`, `compound_outputs`, `final_repository_delta`, stage results, verification outcomes, review attempts with run IDs and artifact paths, finding decisions, compound state, and terminal status. `current_manifest` is the latest loop-owned manifest at terminal completion and may be unreviewed on early exits. `reviewed_manifest` is non-null only after a clean final review and is the only repository-state manifest represented as having passed simplification, review, review-followup, and final code verification. `compound_outputs` are reported separately and must not be described as reviewed.
