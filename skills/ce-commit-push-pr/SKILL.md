...
**Read `references/apply-and-handoff.md`** — apply routes, preview-before-edit and its branding-only no-op, archival, trailer, handoff. Two rules bound the external writes: re-run the existing-PR check immediately before `gh pr create`, switching to the existing-PR path if one appeared or Step 1 came back unknown; and pass the body via `--body-file <path>`, never stdin — `gh` exits 0 with an empty body.
