...
**Load effective PR settings** before composition. Read `references/execution-routing.md`, construct a `ce-routing/v1` `inspect` request whose `cwd` is the absolute repo working directory, and write it to an effective-user-private temporary file. From this skill's directory, invoke the co-located resolver (the assignment and command stay in one shell call):
...
Use only `settings.effective.pr_teaching_section`, `settings.effective.pr_teaching_archive`, and `settings.effective.auto_babysit`, retaining each matching `settings.provenance` entry for diagnostics; never read or parse a config source directly. The resolver, not this skill, distinguishes an active (non-commented) setting from template documentation and merges global/project values. A resolver diagnostic is a configuration blocker.

**Concept teaching gate** before composition. The gate is off only when effective `pr_teaching_section` is exactly `false`; `null` or any other unset/default state preserves the built-in **on** behavior. Effective `pr_teaching_archive` enables archival only when exactly `true`, otherwise it is **off**; a per-run `archive:on|off` token overrides the effective archive value for this invocation.
...
**Babysit handoff — default on.** In interactive full workflow, after reporting a newly-created PR URL (or after new commits land on an existing open PR), **auto-invoke `ce-babysit-pr` on that PR by default**: announce it in one non-blocking line (e.g. "Babysitting toward merge-ready — watching CI + incoming review; pass `babysit:off` to skip"), then invoke — never block on a yes/no. *Off is the explicit choice:* **`babysit:off`** skips it this run (**`babysit:continuous`** / **`babysit:checkpoint`** forces that watch mode); effective **`auto_babysit: false`** is a standing opt-out from the same resolver snapshot used above (only exactly `false` disables; `null` or any other unset/default state preserves the built-in **on** behavior; a `babysit:off` token overrides the setting for this run).
