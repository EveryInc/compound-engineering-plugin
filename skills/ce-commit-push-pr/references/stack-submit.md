...
- **Classify by PR number** wherever one exists — that is what pulls a stack down from GitHub. Exit 0 means the parent is in a stack **and checks it out**; exit 2 means standalone and checks out nothing. A bare branch name only resolves local stacks, so a branch-only parent classifies locally and no further. Because exit 0 moves `HEAD`, record your work branch and its tip **before** classifying and return to it before construction — construction reads whatever is checked out as the original branch, so classifying in place would hand it the parent and drop your commits from the layers.
- **In a stack** — attach above the parent the user named, never above a different layer. Exit 0 left that parent checked out, so run `gh stack add` from there. If it exits **5**, the parent is not the stack top: stop with a residual. Do **not** run `gh stack top` to clear the error — it succeeds by reparenting the work onto the current tip, which is a different layer than the user asked for.
- **Standalone** — exit 2 left nothing checked out, so resolve `<parent-branch>` before init. From a PR: `gh pr view "<n>" --json headRefName,headRefOid,author`, then create a local branch **at `headRefOid`** — always, not only for a fork. Exit 2 created no local ref, so a same-repo parent may be absent or stale locally, and a bare `headRefName` can match an unrelated branch; fetch `refs/pull/<n>/head` when that commit is not reachable, since reachability alone still leaves no branch to name. From a branch with no PR: fetch and verify that ref directly; ownership is unresolvable there, so it can only be a trunk. Then `gh stack init --base "<parent-branch>" "<branch>"…` to keep the parent an untouched trunk, or list its branch first to adopt it as the bottom layer — the latter only when `author` is the current user.
