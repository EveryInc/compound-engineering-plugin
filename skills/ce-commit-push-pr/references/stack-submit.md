...
- **In a stack** — attach above the parent the user named, never above a different layer. `add` runs from the top, so compare the parent with the stack top in `gh stack view --json`: if they are the same branch, add there; if the parent sits below it, stop with a residual, because `gh stack top` would silently reparent the work onto the current tip.
- **Standalone** — exit 2 left nothing checked out, so resolve `<parent-branch>` before init. From a PR: `gh pr view "<n>" --json headRefName,headRefOid,isCrossRepository,author`; when the head is cross-repository, fetch `refs/pull/<n>/head` and create a local branch **at that OID** — making the commit reachable is not enough, and a bare `headRefName` can match an unrelated branch in the base repo. From a branch with no PR: fetch and verify that ref directly; ownership is unresolvable there, so it can only be a trunk. Then `gh stack init --base "<parent-branch>" "<branch>"…` to keep the parent an untouched trunk, or list its branch first to adopt it as the bottom layer — the latter only when `author` is the current user.
