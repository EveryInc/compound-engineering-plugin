...
- **Classify by PR number** wherever one exists — that is what pulls a stack down from GitHub. Exit 0 means the parent is in a stack **and checks it out**; exit 2 means standalone and checks out nothing. A bare branch name only resolves local stacks, so a branch-only parent classifies locally and no further.
