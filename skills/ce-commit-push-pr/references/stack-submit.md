...
- **Standalone** — exit 2 left nothing checked out, so resolve the head first: `gh pr view "<n>" --json headRefName,headRefOid,author`, materializing `refs/pull/<n>/head` locally when that commit is not present. Then `gh stack init --base "<parent-branch>" "<branch>"…` to keep the parent an untouched trunk, or list its branch first to adopt it as the bottom layer — the latter only when `author` is the current user.
