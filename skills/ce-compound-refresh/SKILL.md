...
After individual docs, evaluate the set: overlaps, supersession (an older narrow doc a newer doc subsumes), and outright contradictions — first among docs in scope, then against the agent-facing guidance layer that covers the same procedure (repo-local `**/SKILL.md` / project skill dirs, runbooks for that workflow, and root instruction files `AGENTS.md` / `CLAUDE.md`). Narrow the search to guidance covering the same procedure; do not bulk-read every skill. Doc-vs-doc contradictions actively mislead and outrank individual staleness; doc-vs-instruction-layer contradictions are worse because agents load skills/runbooks/`AGENTS.md` at action time. When the learning is right and the skill/runbook is wrong, report the skill/runbook path as a recommended fix (this skill maintains `<root>/solutions/`, not product skills, unless the user asks). Note category-shape problems (a directory whose docs span unrelated themes, a near-empty category) as report-only observations — never restructure directories or create categories.
