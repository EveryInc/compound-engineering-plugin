...
Imports a project's legacy `CONTEXT-MAP.md` / `CONTEXT.md` vocabulary into the `CONCEPTS.md` protocol, then removes the legacy files. This is the only route that resolves a vocabulary blocked state; every other skill refuses to write and points here.
...
- every proposed file in full — the new root index and each context glossary;
- the complete source-to-destination operation list, including the reference updates and the legacy deletions that would follow;
...
Then delete the legacy files — and only then. Deletion requires both preconditions: every reference to them has been updated, and the user has reviewed the diff. Never delete to make a validation finding disappear.
