...
1. Spawn a single subagent to write the replacement learning. Pass it:
   ...
   - The target path and category (same category as the old learning unless the category itself changed), and the old learning's `component`/`root_cause` values, which the replacement keeps — they are the corpus's vocabulary, not the schema's suggested defaults
