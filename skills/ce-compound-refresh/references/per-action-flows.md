...
1. Spawn a single subagent to write the replacement learning. Pass it:
   ...
   - The target path and category (same category as the old learning unless the category itself changed), and the old learning's `component`/`root_cause` values — the replacement keeps them only when the surrounding corpus uses them too; when the corpus has settled on different values for that area, use those instead; the schema's suggested defaults are never preferred over either
