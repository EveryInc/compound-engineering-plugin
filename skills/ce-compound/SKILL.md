...
- `references/domain-vocabulary.md` — which glossary a vocabulary write targets, and the states that block it (read in Phase 2.4 alongside the format rules)
...
- **`CONCEPTS.md`** — create or update in Phase 2.4 (Vocabulary Capture) when a qualifying domain term surfaces (Full mode; every mode that reaches vocabulary capture may refine an existing file). When the root is a context index, the write lands in the owning context's glossary instead.
...
Before any Phase 1 subagent dispatch, read `references/orca-routing.md` and `references/orca-read-analysis.md`. For a selected stage resolved to Orca, replace only its already-selected subagent calls with that packet path and do not launch them natively. This controller keeps session gates, assembly, scratch persistence, and every product write. Lightweight mode remains subagent-free. For native routing, continue below unchanged.
...
**Resolve current vocabulary and conventions before dispatching subagents.** Use the project's active instructions and conventions already in your context. If `CONCEPTS.md` exists, read its relevant terms and pass them to the Context Analyzer; when it declares a parseable `## Contexts` index instead of terms, read the glossaries of the contexts this learning touches and pass those terms.
...
#### 4. **Session History** (internal flow after launching the parallel block — automatic in Full mode, including non-interactive)
   ...
   **Discovery pipeline.** Infer the scan window from the problem topic, starting with 7 days. Run discovery and metadata extraction:
   ...
   Pi sessions are included when present under `~/.pi/agent/sessions/`, and oh-my-pi (`omp`) sessions under `~/.omp/agent/sessions/` (named profiles: `~/.omp/profiles/<name>/agent/sessions/`; XDG: `$XDG_DATA_HOME/omp/sessions` and `$XDG_DATA_HOME/omp/profiles/<name>/sessions`); they carry `cwd` like Codex but no git branch. Codex sessions are discovered from the active runtime home (the `sessions/` directory under an inherited `CODEX_HOME`, when that variable is set), the conventional homes, Orca's legacy runtime home and per-account homes, and any additional colon-separated roots supplied via `CE_CODEX_SESSION_ROOTS`. Managed runtimes that relocate the Codex home are covered without extra configuration; duplicate routes to one directory and rollout replicas across different homes yield one candidate. If `_meta.files_processed` is `0`, return `no relevant prior sessions`. If the first pass finds no relevant branch matches, or if processing Codex, Pi, or oh-my-pi (`omp`) sessions, derive 2-4 keywords from the topic and re-run metadata extraction with `--keyword K1,K2,...`. Keep at most 5 sessions across Claude Code, Codex, Cursor, Pi, and oh-my-pi (`omp`), ranked by branch match, keyword match count, file size over 30KB, and recency. Exclude the current session.
...
**First, read `references/concepts-vocabulary.md` and `references/domain-vocabulary.md`.** This is unconditional. Do not pre-judge from memory that nothing qualifies — the criteria are non-obvious and qualifying terms often live in the surrounding conversation rather than the new doc itself. The first reference decides how an entry is written; the second decides which file it goes in. Reading both is what makes the rest of the phase possible.

Then, applying those criteria, scan the new doc **and** the surrounding conversation for qualifying domain terms. Resolve the write target per `references/domain-vocabulary.md` before touching a file: with a flat root glossary the target is the root, and when the root declares a parseable `## Contexts` index each term goes to its owning context's glossary. Add missing qualifying terms and refine existing entries when new precision surfaced. If no glossary exists and at least one qualifying term surfaced, create one. A blocked state stops capture: report the conflict and the migration route instead of writing.

A learning that crosses a context boundary may update two context glossaries and the root relation. Each change needs its own evidence, and the set is written as one all-or-nothing change — validate every target first, and if any cannot be written, leave them all unchanged. Where a declared context's glossary does not exist yet, create it at the composed path; `ce-compound` owns that creation.
...
2. **Semantic grounding validator (Full mode, including non-interactive Full; lightweight skips it).** Dispatch one read-only generic subagent built from the prompt template in the reference, covering the written doc plus any vocabulary entries added or edited this run — in the root glossary, in a context glossary, or in the root's relation entries. It verifies code-behavior claims by quoting the defining source line, merge-state claims against remote truth (`gh` primary, git reachability fallback), and internal completeness of countable assertions. Apply its verdicts per the reference (fix contradicted claims from the quoted evidence; soften or drop unverifiable ones; mark offline merge-state checks as degraded), then re-run the mechanical check if the body changed.
...
4. **Vocabulary capture (update-only)**: if `CONCEPTS.md` exists at repo root, read `references/concepts-vocabulary.md` and `references/domain-vocabulary.md`, then scan the new doc and the conversation for qualifying terms and add/refine entries silently in the resolved target (same criteria as Phase 2.4). Do **not** bootstrap or seed in lightweight mode — if `CONCEPTS.md` does not exist, or the resolved context glossary does not exist, defer creation to a Full run, which owns seeding. Record the outcome in the output (e.g., "Vocabulary: 1 entry refined" or "scanned, no qualifying terms"). If you refined `CONCEPTS.md` and the project's active instructions and conventions already in your context do not surface it, add the discoverability tip to the output below — lightweight **tips**, it does not edit instruction files (an interactive Full run owns that edit after consent; non-interactive Full also tips/reports only).
