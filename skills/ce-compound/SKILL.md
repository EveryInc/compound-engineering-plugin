...
#### 4. **Session History** (internal flow after launching the parallel block — automatic in Full mode)
   ...
   **Discovery pipeline.** Infer the scan window from the problem topic, starting with 7 days. Run discovery and metadata extraction:
   ...
   Pi sessions are included when present under `~/.pi/agent/sessions/`; they carry `cwd` like Codex but no git branch. Codex sessions are discovered from the active runtime home (the `sessions/` directory under an inherited `CODEX_HOME`, when that variable is set), the conventional homes, and any additional colon-separated roots supplied via `CE_CODEX_SESSION_ROOTS` — managed runtimes that relocate the Codex home are covered without extra configuration, and duplicate routes to the same physical directory yield one candidate. If `_meta.files_processed` is `0`, return `no relevant prior sessions`. If the first pass finds no relevant branch matches, or if processing Codex or Pi sessions, derive 2-4 keywords from the topic and re-run metadata extraction with `--keyword K1,K2,...`. Keep at most 5 sessions across Claude Code, Codex, Cursor, and Pi, ranked by branch match, keyword match count, file size over 30KB, and recency. Exclude the current session.
