...
**1. What goes into the commit — the fix-owned files and nothing else.** This holds on every route, remote or not.

- No fix-owned file carried pre-existing edits: invoke the `ce-commit` skill scoped to those files.
...
**2. Whether it ships — a PR needs a remote *and* a branch holding nothing but this fix.**

- **Both hold** — a remote is configured, the pre-fix tree was clean, and the branch holds no commits beyond this run's work (`git log <default-branch>..HEAD`). How the branch came to exist proves nothing: `git checkout -b` carries uncommitted work onto the new branch. Preview what will be committed, on what branch, and that a PR will be opened, then **invoke the `ce-commit-push-pr` skill with `branding:on`.** The preview is a statement, not a question: state it and proceed so the user can interrupt. Surface the resulting PR URL.
- **Either fails** — commit per question 1 and push nothing. Say in one line what stayed local and why (unrelated work on the branch, or no remote), and that you will push and open the PR on request. Do not ask first — a local commit is reversible, and one word gets the rest.
