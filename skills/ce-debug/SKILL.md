...
**Do not ask whether to open a PR.** What the branch *contains* picks the route; the user's permission is never the gate. Read the route off the pre-fix scope Phase 3 recorded. **Fire the action itself** via the platform's skill-invocation primitive — never merely tell the user to type a command.

- **The branch carries only this fix** — Phase 3 recorded a clean pre-fix tree (no pre-existing changed files) **and** either this skill created the branch or the branch holds no commits beyond this run's work (`git log <default-branch>..HEAD`). Branch creation never implies a clean tree: `git checkout -b` carries uncommitted work onto the new branch, so a branch this skill made off a dirty default still holds the user's WIP and belongs in one of the two routes below. Preview what will be committed, on what branch, and that a PR will be opened, then **invoke the `ce-commit-push-pr` skill with `branding:on`.** The preview is a statement, not a question: state it and proceed so the user can interrupt. Surface the resulting PR URL.
