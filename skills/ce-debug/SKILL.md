...
**The goal: ship the fix without publishing anything the user did not offer up.** Do not ask whether to open a PR — permission is not the gate; what the branch actually holds is. Establish that by checking the branch at this moment, never by assuming it from how the branch came to exist. The cases below are the shapes that judgment usually takes, not an exhaustive switch: when a branch state fits none of them cleanly, hold to the goal — if you cannot show the push carries only this fix, do not push. **Fire the action itself** via the platform's skill-invocation primitive — never merely tell the user to type a command.

- **The branch carries only this fix** — the pre-fix tree was clean and the branch holds no commits beyond this run's work (`git log <default-branch>..HEAD`). How the branch came to exist proves nothing: `git checkout -b` carries uncommitted work onto the new branch, so one this skill created off a dirty default still holds the user's WIP. Preview what will be committed, on what branch, and that a PR will be opened, then **invoke the `ce-commit-push-pr` skill with `branding:on`.** The preview is a statement, not a question: state it and proceed so the user can interrupt. Surface the resulting PR URL.
- **Unrelated work, cleanly separable** — Phase 3 recorded pre-existing changed files or the branch holds commits that are not this fix, but no fix-owned file carried pre-existing edits. Do not push and do not open a PR; that publishes work the user never offered up. Invoke the `ce-commit` skill scoped to the fix-owned files, then state in one line what stayed local, what kept it local, and that you will push and open the PR on request. Do not ask first — a local commit is reversible, and one word gets the rest.
- **Unrelated work entangled with the fix** — a fix-owned file already carried the user's pre-existing edits, so no commit separates them (`ce-commit` groups at file level and never splits a file). Ask (per **Blocking questions**): commit that file including their edits, leave the fix uncommitted for them to handle, or stop. This is the one handoff question that survives, because every option here loses something the agent cannot choose on the user's behalf. Phase 3's confirmation covered *editing* the file, never committing the user's edits along with the fix — do not reuse it to skip this question.
- **Repo with no remote configured** — a PR is impossible, so invoke the `ce-commit` skill for a local commit and say why no PR was opened.
- **Not a git repo** — stop after the summary and the quality block; there is nothing to hand off.
