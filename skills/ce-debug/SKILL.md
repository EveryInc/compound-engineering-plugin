...
- **Ships** — `origin` exists (`ce-commit-push-pr` pushes to `origin` specifically, so some other remote name is not enough), the pre-fix tree was clean, and **either** the branch already has an open PR — its commits are under review, so they are work the user has offered, and `ce-commit-push-pr` updates that PR rather than opening a second one — **or** the branch holds no commits beyond this run's work at all (`git log <default-branch>..HEAD`). Pushed is not the same as offered: commits pushed for backup or to trigger CI, with no PR open, would otherwise be swept into a *first* PR spanning work nobody put up for review. How the branch came to exist proves nothing either: `git checkout -b` carries uncommitted work onto the new branch. Preview what will be committed, on what branch, and whether a PR will be opened or updated, then **invoke the `ce-commit-push-pr` skill with `branding:on`.** It does the committing under question 1's scope, so do not commit before invoking it. The preview is a statement, not a question: state it and proceed so the user can interrupt. Surface the resulting PR URL.
