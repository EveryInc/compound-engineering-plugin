...
- **Ships** — `origin` exists (`ce-commit-push-pr` pushes to `origin` specifically, so some other remote name is not enough), the pre-fix tree was clean, and the branch carries nothing the user has not offered. `ce-commit-push-pr` pushes the **whole branch**, so that takes two conditions together: no unpushed commits other than this run's work (`git log @{u}..HEAD`), **and**, when no PR is open yet, no commits beyond this run's work at all (`git log <default-branch>..HEAD`) — because a first PR would span every one of them. Commits already in an open PR are under review and therefore offered, so a branch with a PR still ships and `ce-commit-push-pr` updates that PR rather than opening a second one. Pushed alone is not offered: commits pushed for backup or to trigger CI, with no PR open, would otherwise be swept into that first PR. How the branch came to exist proves nothing either: `git checkout -b` carries uncommitted work onto the new branch. Preview what will be committed, on what branch, and whether a PR will be opened or updated, then **invoke the `ce-commit-push-pr` skill with `branding:on`.** It does the committing under question 1's scope, so do not commit before invoking it. The preview is a statement, not a question: state it and proceed so the user can interrupt. Surface the resulting PR URL.
