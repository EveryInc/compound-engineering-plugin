...
- **Ships** — `origin` exists (`ce-commit-push-pr` pushes to `origin` specifically, so some other remote name is not enough), the pre-fix tree was clean, and the branch carries no **unpublished** commits other than this run's work: `git log @{u}..HEAD` when the branch has an upstream, `git log <default-branch>..HEAD` when it does not. Already-pushed commits are published work, not unoffered WIP — so a branch with an open PR still ships, and `ce-commit-push-pr` updates that PR rather than opening a second one. How the branch came to exist proves nothing: `git checkout -b` carries uncommitted work onto the new branch. Preview what will be committed, on what branch, and whether a PR will be opened or updated, then **invoke the `ce-commit-push-pr` skill with `branding:on`.** It does the committing under question 1's scope, so do not commit before invoking it. The preview is a statement, not a question: state it and proceed so the user can interrupt. Surface the resulting PR URL.
