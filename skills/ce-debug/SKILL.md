...
- **Ships** — three things hold. `origin` exists and is **PR-capable**: a GitHub or GitHub Enterprise remote that `gh` can reach and is authenticated for. `ce-commit-push-pr` pushes to `origin` by name and pushes *before* it creates the PR, so a route that clears a weaker gate and then fails at PR creation has already published the branch. The pre-fix tree was clean. And the branch carries nothing the user has not offered — which depends on whether a PR is already open, because `ce-commit-push-pr` pushes the **whole branch**:
  - **A PR is already open** — its commits are under review and therefore offered, and the run updates that PR rather than opening a second one. Require only that no *unpushed* commits other than this run's work exist (`git log @{u}..HEAD`; an open PR guarantees an upstream).
  - **No PR yet** — a first PR would span every commit on the branch, so require no commits beyond this run's work at all (`git log <default-branch>..HEAD`). Do not test `@{u}` on this arm: a branch just created with `git checkout -b` has no upstream and the check would fail outright, and pushed alone would not mean offered anyway — commits pushed for backup or to trigger CI belong to nobody's review.
