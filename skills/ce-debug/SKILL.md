...
Rerun the reproduction, then restore — **but only if that push actually created a stash.** `git stash push` prints `No local changes to save` and creates nothing when the dirty state is one it does not stash (a modified submodule is the common case), and an unconditional pop then takes *the user's pre-existing stash* instead, destroying unrelated work. Compare `refs/stash` with the tip from the first command: still absent or unchanged means nothing was stashed, so do not pop and do not report the tree as restored. Otherwise `git stash pop --index` in the same step regardless of the reproduction's outcome. The `--index` restores what was staged as staged; a plain pop brings everything back unstaged and silently destroys the user's staging. The `-u` is load-bearing — without it untracked files stay behind and the tree only looks clean, so a bug living in a new file survives the stash and reads as "not the WIP." Both results are evidence: the failure vanishing identifies the user's own edit as the cause and the investigation is over, and the failure persisting rules the WIP out and leaves a clean tree to trace against. Announce the stash before running it, confirm the pop restored the tree, and if the pop reports conflicts surface the conflict output and the stash ref — never auto-resolve a conflict in someone's uncommitted work.
...
- **Ships** — three things hold. `origin` exists and is **PR-capable**: a GitHub or GitHub Enterprise remote that `gh` can reach and is authenticated for. `ce-commit-push-pr` pushes to `origin` by name and pushes *before* it creates the PR, so a route that clears a weaker gate and then fails at PR creation has already published the branch. The pre-fix tree was clean. And the branch carries nothing the user has not offered — measured against **remote** refs, never local ones, after fetching `origin` so they are current. A local ref can itself be ahead of the remote, which is the trap: if the user's own default branch has unpushed commits and Phase 3 branched off it, a local-ref range reports empty and the run publishes those commits. Which remote ref depends on whether a PR is open, because `ce-commit-push-pr` pushes the **whole branch**:
  - **A PR is already open** — its commits are under review and therefore offered, and `ce-commit-push-pr` updates that PR rather than opening a second one. Require no commits beyond this run's work on top of the PR's remote head (`git log origin/<branch>..HEAD`). Do not use `@{u}`: tracking configuration is optional and may be absent even with a PR open (`git switch --no-track`, `git branch --unset-upstream`), where the range errors instead of answering.
  - **No PR yet** — a first PR would span every commit on the branch, so require no commits beyond this run's work against the remote default (`git log origin/<default-branch>..HEAD`). Pushed alone would not mean offered here anyway — commits pushed for backup or to trigger CI belong to nobody's review. `ce-commit-push-pr/references/branch-creation.md` uses the remote range for this same reason.

  How the branch came to exist proves nothing either: `git checkout -b` carries uncommitted work onto the new branch. Preview what will be committed, on what branch, and whether a PR will be opened or updated, then **invoke the `ce-commit-push-pr` skill with `branding:on`.** It does the committing under question 1's scope, so do not commit before invoking it. The preview is a statement, not a question: state it and proceed so the user can interrupt. Surface the resulting PR URL.
