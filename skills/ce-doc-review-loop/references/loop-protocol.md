...
These four primitives recur throughout the protocol. Resolve each one here; do not substitute an improvised equivalent.

- **Fingerprint.** The raw-byte SHA-256 of a file, read from an executed command — `shasum -a 256 "<path>" | cut -d' ' -f1`, or `sha256sum "<path>" | cut -d' ' -f1` where `shasum` is absent. Never state a digest you did not read from that output; an asserted digest makes every equality gate below pass vacuously. `ce-doc-review` emits receipt digests with a `sha256:` prefix; strip it before comparing. Two distinct empty values, never interchangeable: report `unavailable` when a fingerprint was required but no hashing command could run, and `not_reached` when the run ended before any fingerprint was taken.
- **Disposable snapshot.** A Markdown file inside `mktemp -d "${TMPDIR:-/tmp}/ce-doc-review-loop-XXXXXX"`. Never write a snapshot beside the product document; the Commit temp file below is not a snapshot and is the one file this protocol writes into the product document's directory. When no writable scratch location exists, return `Non-converged` with the observed reason; never fall back to reviewing or staging against the product path in place.
- **Run state.** One JSON file at `/tmp/compound-engineering-$(id -u)/ce-doc-review-loop/<run-id>/run-state.json`. Mint `<run-id>` fresh for every invocation — `RUN_ID=$(date +%Y%m%d-%H%M%S)-$(head -c4 /dev/urandom | od -An -tx1 | tr -d ' ')` — and never adopt an existing run directory: reusing one silently reopens a prior run's frozen fingerprint, matrix, and work-unit count as if they described the current bytes. Create the root under `umask 077` and repair it unconditionally with `chmod 700`, since a root left world-readable by another skill makes `mkdir -p` a no-op; apply the same mode to the per-run directory. Reject the root when it is a symlink or not owned by the current user, and return `Non-converged` with the observed reason when it cannot be created or repaired.
- **Commit.** Run the whole commit in a single shell invocation — `$$` is re-minted per invocation, so a temp file named in one call and moved in the next are two different paths. Resolve the product path to its physical target first with `readlink -f "<product>"`: a symlinked product path must be committed *through* to the file it points at. Copy the validated bytes to a temp file in that target's own directory, restore the target's mode onto the temp file with `chmod "$(stat -f %Lp "<target>" 2>/dev/null || stat -c %a "<target>")" "<temp>"`, then `mv -f` the temp file onto the target. Return `Non-converged` with the observed reason when any step fails, leaving the product path untouched. Four ways this goes wrong silently: a move out of `${TMPDIR:-/tmp}` can cross filesystems and is not atomic; a temp name built from `$$` in a separate invocation points at nothing; `mv -f` of a fresh temp file drops the original's permissions; and `mv -f` onto an unresolved symlink replaces the link with a regular file, leaving the real document unedited while the product path still fingerprints as committed.
