...
2. Invoke `ce-doc-review mode:non-interactive <snapshot-path>` through the host's callable skill mechanism. This is the **internal invocation channel**. Do not start this wave when the configured work-unit budget is exhausted. A generic task, agent, inline review, or reconstructed persona pass is not invocation of the canonical skill. Slash- and dollar-prefixed invocations are **user-facing handoff text** only. Never execute `/ce-doc-review` or `$ce-doc-review` through a shell, terminal, command parser, or ordinary text dispatch.
3. If the callable mechanism is absent or explicitly reports that the named skill is unavailable, return `Non-converged` with `ce-doc-review: skill_unreachable` and the observed reason. If an attempted slash/dollar command returns `Unknown command` or another command-parser error, return `ce-doc-review: invocation_adapter_error`; that error is not evidence that the skill is unregistered. Do not imitate the review engine. For either status, print the invocation the user can run directly as a copyable `/ce-doc-review mode:non-interactive <product-path>` block — use `$ce-doc-review …` only when the active harness is Codex — and set the envelope's `Next bounded wave` to that handoff instead of a slice. This block is output only; do not execute it or merely tell the user to type an invocation.
