...
Dispatch generic subagents using **bounded parallelism** with whatever agent-dispatch tool this harness exposes (e.g., `Agent` in Claude Code, `spawn_agent` in Codex). Attempt the dispatch. If the attempt is denied, ask the user once whether to allow it. Fall back to inline or serial execution only if there is no such tool, or the attempt failed or was declined — and say which. Omit the `mode` parameter so the user's configured permission settings apply. Respect the current harness's active-subagent limit: queue selected reviewers, dispatch only as many as the harness accepts, and fill freed slots as reviewers complete. Treat active-agent/thread/concurrency-limit spawn errors as backpressure, not reviewer failure: leave the reviewer queued and retry after a slot frees. Record a reviewer as failed only after a successful dispatch times out/fails, or when dispatch fails for a non-capacity reason.
