...
Accumulate across all rounds in the current session. Skip, Defer, Acknowledge, and Withdraw actions all count as "rejected" for suppression purposes — each signals the user decided the finding wasn't worth actioning this round (Withdraw is the revalidation variant: an earlier decision resolved or contradicted the finding; see "Withdrawing findings the user's earlier answers resolved" in `references/walkthrough.md`) (Acknowledge is the no-fix-guard variant: the user saw a finding with no `suggested_fix`, chose not to defer or skip explicitly, and recorded acknowledgement instead; for round-to-round suppression that is semantically equivalent to Skip). **Only a Withdraw retired by a user decision — a settled premise (Skip/Defer) or a user-asserted fact — enters the primer as rejected-class. An Apply-triggered Withdraw never does** (its resolution depends on the staged edit both landing and semantically resolving the finding, which round N+1 re-synthesis checks — not R29; suppressing it would hide a fix that failed or landed ineffectively). Applied findings stay on the applied list so round-N+1 personas can verify fixes landed (see R30 in `references/synthesis-and-presentation.md`).
