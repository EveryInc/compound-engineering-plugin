...
- **Walk-through `Auto-resolve with best judgment on the rest`:** same as routing option B, but scoped to the findings the user hadn't decided on. Apply findings join the in-memory Apply set with the ones the user already picked during the walk-through; all dispatch together in the single end-of-walk-through Apply pass — or, in the batch pass, when it exits back to routing, since that exit flushes the set instead of reaching the terminal pass. **When the walk-through is running as the grouped confirmation's batch pass, "the rest" is the rest of the batch, and control returns to the routing question instead of the completion report** — the decision surface has not been routed yet (`references/walkthrough.md`, grouped confirmation option B). Emitting the report here ends the run with those decisions unasked.
