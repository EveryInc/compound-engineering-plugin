...
**Probe before declaring a route unavailable.** "No concrete installed route" and "no peer resolved" must be an *observed* probe failure, not an assumption: before recording a resolved target's route as unavailable and keeping the in-process twin, probe that route's CLI for presence-and-execution — `command -v <cli>` for the route's binary and, where the route documents authentication, its documented auth/login check — and record unavailable only when a probe fails. Probing a peer CLI's own presence and auth is distinct from, and does not relax, the "do not inspect the worker source" rule above: it checks whether the route exists on this host, not how the worker maps or enforces it.
