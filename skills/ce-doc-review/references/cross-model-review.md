...
- **Interactive host, default interactive mode:** show a **prominent standalone line** that frames it as an **independent cross-model review** of the judgment lenses (say "cross-model" / "independent model", not the internal "peer" jargon). The line names the concrete **model and reasoning level** from the in-script mapping (e.g. GPT-5.6-luna at extra-high reasoning, Opus at high, Grok 4.7 at extra-high, Composer 2.5-fast). Because two different models can arrive over the *same* `cursor-agent` CLI, it names **the route as well as the model** for cursor-agent routes so Grok-4.7-via-cursor-agent, Composer-via-cursor-agent, and Grok-4.7-via-the-grok-CLI are unambiguous. **It states that full document content is sent to that provider** (third-party egress; for cursor-agent routes the content goes to Cursor *plus* the serving provider). **Announce wording follows the served-model record:** name a model as serving only where the route returns a record of which model actually served; on routes without one say "requested <model>; serving model unverified on this route" instead of asserting the concrete model. Place it with the Phase 2 team announce, not buried after it. Wording is yours; the checkable requirements are: prominent, reads as a **cross-model reviewer** (not a generic persona), names the requested model (with the unverified marker on routes without a served-model record), names the route when it is cursor-agent, names the egress. Example: `🔀 Cross-model pass — the judgment lenses are also being reviewed by an independent model: requested **Grok 4.7 (extra-high reasoning), via cursor-agent** (serving model unverified on this route). Full document content is sent to xAI/Cursor.`
  ...
  - Announce the one fixed route and every recipient before dispatch. A route failure produces no artifact; apply Step 1's retry/disclosure condition before another attempt. Reconcile `cross_model_target`, `cross_model_harness`, `cross_model_route`, `model_requested`, and `model_actual` from the artifact; never infer a serving model from the requested ID. In prose the user reads, name the peer by target and requested model; `model_actual: unverified` means no served-model record, not an unknown model, so add a serving caveat only when the record disagrees with the request or the route requested no model (Cursor default/Auto).
