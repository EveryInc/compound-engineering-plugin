...
- **Nothing in the decision surface** → skip the routing question, emit the completion report, then flow to the Phase 5 terminal question. Applied changes and an answered grouped confirmation do not warrant a routing question; they still warrant a report, since they are what the run did. When the decision surface is empty but the grouped confirmation is not, the confirmation still fires — it is a separate step, not a branch of routing.
