...
**Routing batch: `ce-doc-review.repeat-local-reviewers`.** Only after the existing next-pass decision and Phase 2 gates fix the repeated reviewer roster, and before any persona asset, multi-round primer prompt, or subagent template is read or assembled, load `references/execution-routing.md` and resolve only the selected stable IDs from `ce-doc-review.coherence-reviewer`, `ce-doc-review.design-lens-reviewer`, `ce-doc-review.scope-guardian-reviewer`, `ce-doc-review.feasibility-reviewer`, `ce-doc-review.product-lens-reviewer`, `ce-doc-review.security-lens-reviewer`, and `ce-doc-review.adversarial-document-reviewer` together in one `ce-routing/v1` `resolve_batch`. Pass the exact full first-wave `snapshot` object as the `parent_snapshot` envelope; include `parent_snapshot_id` only if it matches that envelope. Never use ID-only lineage or reread live routing sources, and reuse the frozen bindings on recovery. Routing cannot start another pass, add a conditional persona, change the primer, or alter synthesis and failure semantics.
<!-- ce-dispatch-site:ce-doc-review.repeat-local-reviewers -->
