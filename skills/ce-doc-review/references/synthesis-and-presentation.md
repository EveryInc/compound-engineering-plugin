...
**Cross-model twin exception.** Apply this exception only to a `ran_verified_independent` return: both `independence_verified` and `model_receipt_verified` are true after receipt reconciliation. Match it against its in-process twin when `normalize(section)` matches and evidence-substring overlap exceeds 50%, even if titles differ. Other returns use the ordinary fingerprint and receive no promotion.
