...
A finding is **not** an obligation when its fix would introduce a new user-visible state, limit, failure policy, retention rule, or operational commitment — however concrete that fix is. Concreteness is not authority. A fix the document does not already entail is a decision and stays in the decision surface. An obligation does not require judgment about *whether* to act — the document already resolved that. **This reclassification only ever moves `manual` up to `gated_auto`, and only when the finding carries a `suggested_fix`.** It never touches `safe_auto`: a mechanical correction the document entails is still mechanical, and demoting it to `gated_auto` would contradict the rule two paragraphs below that keeps it applying unattended at anchor `100`.

**An obligation with no `suggested_fix` stays `manual` and stays on the decision surface — it is not routed as an obligation at all.** The batch applies edits on one answer, and there is no edit here to apply; reclassifying it would only send it into the routing table's `suggested_fix` requirement and bounce it straight back to `manual`. The document entailing a correction does not mean anyone has written the correction — the reader supplies the missing text, which is judgment even though the decision behind it is settled.
...
**Cross-model peer safeguard.** A finding whose only reviewers are cross-model peers (a `<lens>-<provider>` name with no in-process co-reviewer) **never routes to Apply**, at any anchor. A peer cannot authorize an unattended edit on its own (R18). It takes the grouped confirmation instead — but only when the table would have sent it to Apply. **A peer-only `manual` finding stays a decision.** The safeguard exists to stop a peer applying something unseen, and demoting a fork into the batch would defeat the point twice over: `Apply all` would sweep a genuine choice, and a `manual` finding may carry no `suggested_fix` at all, so the batch would attempt an edit with no payload. Withhold apply authority; do not move a finding down a surface it never qualified for.
