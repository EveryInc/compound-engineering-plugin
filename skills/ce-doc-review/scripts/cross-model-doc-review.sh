...
# A cursor-agent route egresses content through Cursor even when the *model* is
# Grok or Fable. CROSS_MODEL_PEERS is an egress boundary (R19), not just a
# model-provider filter, so those transports are off-limits under an allowlist
# that does not sanction Cursor. Cursor egress is sanctioned when
