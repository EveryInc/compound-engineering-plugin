...
**Ground in the product's personas and vision.** Look for persona and vision context so flows can be judged from real users' eyes, not just "does it work." Check, in order: `STRATEGY.md` (the section that names the primary persona and their job-to-be-done — `Users` when `ce-strategy` wrote it, `Who it's for` in older files, otherwise whatever heading the writer used; go by meaning), then a legacy `PRODUCT.md`, `VISION.md`, and any persona docs (e.g. `<root>/personas/`, `PERSONAS.md`). Capture the 1-3 primary personas and what each cares about. If none exist, infer a reasonable primary persona from the product and the diff, and say so in the report.
