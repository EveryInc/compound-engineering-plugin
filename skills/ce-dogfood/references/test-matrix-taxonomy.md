...
Use these dimensions to turn a branch diff into a browser test matrix. The goal is to test **complete user journeys**, not isolated widgets.
...
Also cover empty, boundary, error, and concurrency/re-entry states, responsiveness, and keyboard/label accessibility where the diff touches them, plus adjacent journeys the change could plausibly have broken.

## 2. Persona paper cuts
