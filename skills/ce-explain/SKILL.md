...
Teach the user one thing well: a concept, a change, an idea, or a window of their own recent work.
...
**Default — the user personally.** Dense, technical, second person, free to assume the context they already carry.

**On request — rendered for another reader.** When the artifact itself will land in front of other people, adapt voice and orientation, never depth: drop second person, name the subject in third person where the evidence or the user supplies a name, and add the minimum orientation an outsider needs. Never softened into a status update, never a deck.
...
Read the rendering reference for the resolved format — `references/explainer-html.md` (default) or `references/explainer-markdown.md` (when intake resolved `output:md`) — and compose per its contract. Write the artifact to `$RUN_DIR/explainer.html` (or `$RUN_DIR/explainer.md` when intake resolved `output:md`) before anything else happens with it. Display it to the user (inline summary plus the file path; open locally per Phase 6 when chosen). The artifact exists at that stable path from this moment — a declined destination ask never loses it.
...
Detect destinations by capability — probe the agent's own toolset and session context, never a closed list, and never treat a missing binary, env var, or unloaded MCP tool as proof a destination is unavailable when a connector could supply it. Local file and Leave it are ungated and always offered. For default HTML runs, offer one preferred publisher: Claude Artifact when running in Claude Code with its Artifact tool present; otherwise ht-ml.app. Do not show both by default, but honor an explicit user request for either. ht-ml.app is public and must never be published headlessly. Ask for the destination once with the blocking question tool — that governs the menu itself, not the consent a chosen destination then requires; a publisher's warning-and-confirmation is a separate, required ask. If the user names a publisher the one-preferred-publisher rule kept off the menu, honor it by the bypassed-menu path in `references/destinations.md` (full warning, then explicit confirmation), never as though the menu had warned them — it didn't. When the visible options exceed the question tool's option cap, render a numbered list in chat with "Pick a number or describe what you want." and wait instead. Per-option routing:
...
**This offer comes first**, before any publish warning or confirmation the destination requires — consent must attach to the artifact actually being published. Settle the rendering, then run the destination's own consent gate.
