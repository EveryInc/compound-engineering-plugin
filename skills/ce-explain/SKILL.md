...
Ask for the destination once with the blocking question tool — that governs the menu itself, not the consent a chosen destination then requires. A publisher's warning-and-confirmation is a separate, required ask. Publishing always requires the user's destination choice, and ht-ml.app is public, so it must never be selected headlessly. A destination the user named before any warning is a choice of destination, not a confirmation: give the full public-page warning and get explicit confirmation after it. If confirmation cannot be obtained, do not publish; preserve the canonical HTML and report its local `$RUN_DIR/explainer.html` path. The improvement handoffs the phase closes on are offered, never auto-fired, and `ce-polish` is user-run only — never fire it through the skill primitive.
