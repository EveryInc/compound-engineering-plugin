...
**Routing batch: `ce-ideate.ideation-fleet`.** Only when the existing mode/depth rules select the default fleet, and after its exact frame/agent instances are fixed but before the shared grounding block or any dispatch prompt is assembled, load `references/execution-routing.md` and resolve every already-selected frame instance of `ce-ideate.ideation-generator` together in one `ce-routing/v1` `resolve_batch`, with one request entry per selected frame agent. Pass the exact full first-wave `snapshot` object as the `parent_snapshot` envelope; include `parent_snapshot_id` only if it matches that envelope. Never use ID-only lineage or reread live routing sources, and reuse the frozen bindings on recovery. Routing cannot choose the default fleet, add a frame or agent, change model tiers, or alter prompt bytes, permissions, or concurrency.
<!-- ce-dispatch-site:ce-ideate.ideation-fleet -->
...
**Routing batch: `ce-ideate.ideation-fleet-variants`.** Only when the existing surprise-me, `go deep`, or usable issue-theme gate selects a fleet variant, and after its exact frame/agent instances are fixed but before the shared grounding block or any dispatch prompt is assembled, load `references/execution-routing.md` and resolve every already-selected variant instance of `ce-ideate.ideation-generator` together in one `ce-routing/v1` `resolve_batch`, with one request entry per selected frame agent. Pass the exact full first-wave `snapshot` object as the `parent_snapshot` envelope; include `parent_snapshot_id` only if it matches that envelope. Never use ID-only lineage or reread live routing sources, and reuse the frozen bindings on recovery. Routing cannot select a variant, add a frame or agent, replace fallback behavior, change model tiers, or alter prompt bytes, permissions, or concurrency.
<!-- ce-dispatch-site:ce-ideate.ideation-fleet-variants -->
...
**Routing batch: `ce-ideate.ideation-axis-recovery`.** Only after the existing coverage check selects up to two empty axes and fixes their recovery frame instances, and before any recovery prompt is assembled, load `references/execution-routing.md` and resolve every already-selected recovery instance of `ce-ideate.ideation-generator` together in one `ce-routing/v1` `resolve_batch`, with one request entry per selected missing axis. Pass the exact full first-wave `snapshot` object as the `parent_snapshot` envelope; include `parent_snapshot_id` only if it matches that envelope. Never use ID-only lineage or reread live routing sources, and reuse the frozen bindings on recovery. Routing cannot create an axis, exceed the two-axis cap, select a different frame, or alter prompt, merge, and concurrency semantics.
<!-- ce-dispatch-site:ce-ideate.ideation-axis-recovery -->
