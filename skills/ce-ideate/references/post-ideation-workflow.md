...
**Routing batch: `ce-ideate.adversarial-filter`.** After the existing depth gate fixes the fresh-context critic wave and before either verifier payload or prompt is assembled, load `references/execution-routing.md` and resolve `ce-ideate.basis-verifier` plus `ce-ideate.deep-critic` only when `go deep` selected that second role, together in one `ce-routing/v1` `resolve_batch`. Pass the exact full first-wave `snapshot` object as the `parent_snapshot` envelope; include `parent_snapshot_id` only if it matches that envelope. Never use ID-only lineage or reread live routing sources, and reuse the frozen bindings on recovery. Routing cannot add the deep critic, change the fresh-context payload, generate replacement ideas, or alter arbitration and degradation semantics.
<!-- ce-dispatch-site:ce-ideate.adversarial-filter -->
