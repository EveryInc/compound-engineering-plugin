...
1. **Resolve the target directory and extension.**
   ...
   - **Lifecycle, whenever the destination is the repository's `<root>/ideation/` (repo mode, or elsewhere mode that selected it):** the ideation file is a read-once working artifact for the person who ran the skill - unlike `<root>/plans/` and `<root>/solutions/`, it is not meant to be committed. Keep it out of `git status`: ensure the repo's `.gitignore` covers the directory by appending one line naming the repo-relative path of the resolved `<root>/ideation/` (leading slash, trailing slash) when no existing entry covers it, creating `.gitignore` if absent. This edit is part of the deliverable write - do not skip it silently, and name it in the Phase 4.2 summary line so the user sees a repo file changed. An ignore rule cannot untrack a file: if the resumed destination is already tracked (a legacy committed artifact from before this contract), say so plainly in the summary instead of claiming the artifact stays untracked, and leave the untrack decision to the user.
   - **Elsewhere mode with `<root>/ideation/` already present:** use it (the lifecycle step above applies - the condition is the destination, not the mode).
