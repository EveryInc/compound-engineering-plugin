...
- **Auto-write the deliverable** (Phase 4.1). The full per-idea detail (title, description, axis, basis, rationale, downsides, confidence, complexity) and the rejection summary go in the **file** — persistence is automatic, not opt-in. Location: `docs/ideation/` when it already exists, otherwise the resolver's durable owner-scoped CE data directory — never the user's CWD or disposable run scratch. `OUTPUT_FORMAT` (SKILL.md Phase 0.0; default `html`) sets the extension.
