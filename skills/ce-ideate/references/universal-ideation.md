...
**Routing batch: `ce-ideate.universal-basis-verifier`.** Once the candidate list is fixed and before the fresh-context verifier payload or prompt is assembled, load `references/execution-routing.md` and resolve `ce-ideate.basis-verifier` in one `ce-routing/v1` `resolve_batch`. Pass the exact full first-wave `snapshot` object as the `parent_snapshot` envelope; include `parent_snapshot_id` only if it matches that envelope. Never use ID-only lineage or reread live routing sources, and reuse the frozen binding on recovery. Routing cannot create candidates, add a verifier, grant repo access, or alter the final-cut and degradation semantics.
<!-- ce-dispatch-site:ce-ideate.universal-basis-verifier -->
