...
- **Reads scale with submissions here too.** The reduced read budget is justified only by the reduced idea target — the budget is a rate sized to what an agent submits, not a constant. An explicit volume override raises the budget with it rather than returning it to a number calibrated for a smaller target, and never runs raised volume against a lowered cap. Where depth per idea still falls at a large override, say so in the artifact.
