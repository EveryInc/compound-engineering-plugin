...
- **Auto-write the deliverable** (Phase 4.1). The full per-idea detail (title, description, axis, basis, rationale, downsides, confidence, complexity) and the rejection summary go in the **file** — persistence is automatic, not opt-in. Location: `<root>/ideation/` when it already exists, otherwise the run's CE temp area (`<scratch-root>/ce-ideate/<run-id>/`) — never the user's CWD. In repo mode the deliverable is a read-once artifact that stays untracked; `post-ideation-workflow.md` Phase 4.1 owns the gitignore step. `OUTPUT_FORMAT` (`references/output-mode.md` Phase 0.0; default `html`) sets the extension.
