...
8. **Controller state is durable control state** — complete `optimize-controller.py start` before the first author or judge dispatch. It writes and verifies the immutable routing snapshot, attempt state, and manifest-bound event journal under the private run root. Import each committed monotonically numbered event into the log after CP-1; do not report or start another attempt until required writes agree. On resume, the controller discards a valid journal-only suffix because its sealed manifest was not committed; never import that suffix or reconstruct routing, receipts, process state, or leases from conversation context.
...
**Worktree backend:**
1. Select the next candidate only from controller status. OpenCode's ordinary `ce_task` adapter can attest its selected model/variant and native Task permissions, but not inherited Landlock, the sanitized process environment, launch barrier, PID/subreaper state, or all-descendant cleanup required by a routed Optimize attempt. Every configured native host candidate is therefore unavailable before worktree creation or prompt egress. A top-level no-routing/CE-default binding alone retains the characterized native Task path without strict model or process evidence.
...
5. Dispatch a generic subagent for the CE-default author through the host's unchanged native Task path, working in the experiment worktree with its existing prompt, tools, permission mode, and bounded scheduling. After the Task call returns, invoke controller `complete-native --outcome ok|failed`; this compatibility marker carries no model, confinement, PID, launch-barrier, or descendant-cleanup claim. Then call `finalize`. Never use this path for a profile candidate or treat its output as verified identity/process evidence.
...
<!-- ce-dispatch-site:ce-optimize.judge-batches -->
    ...
    - Route Codex judges through controller `dispatch`. A configured native judge candidate is unavailable; only a CE-default native judge may use the unchanged Task path followed by `complete-native`, which attests no identity or process controls. Keep each structured JSON response quarantined and call controller `finalize` before parsing it. Discard `NEXT_CANDIDATE` output; `BLOCK` makes the experiment unscored and cannot be treated as a hard-metric success.
