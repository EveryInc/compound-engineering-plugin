...
Keep each author or judge result quarantined and call `finalize_attempt` with its own frozen binding and adapter-owned serving evidence before consuming it. Author acceptance happens before scope inspection, orchestrator measurement, `result.yaml`, CP-3, commit, or merge. Judge acceptance happens before parsing, aggregation, cost accounting, or checkpointing scores. A successful `prefer` attempt without identity evidence may be accepted only as `accepted_unverified`; `require` blocks unavailable, mismatched, or unverified identity without prompt and leaves output isolated.
