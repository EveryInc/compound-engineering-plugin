...
- For Codex dispatch: set `umask 077`, allocate `PROMPT_FILE=$(mktemp "${TMPDIR:-/tmp}/optimize-exp.XXXXXX.txt")`, write the filled template there, pipe it via stdin, then remove that exact path.
