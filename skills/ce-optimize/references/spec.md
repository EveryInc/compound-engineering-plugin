...
5. Guide the user through the remaining spec fields:
   ...
   - If `type: judge`: calibration is optional evidence that the judge scores the way a person would. Mention it once: a human-labeled sample of 20-50 items with a score and the reasoning for each, stored in the repo and named in `metric.judge.calibration.labels`. Proceed without it when the user does not supply one; the approval gate then states that the judge is unchecked against a person. A run that waits between ticks through a wake after turn end is the exception: nobody is watching the judge there, so it needs the labels or the user's explicit `calibration.waived: true`
