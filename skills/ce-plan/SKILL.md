...
- **Run it as a `/goal`** — Offered on the same implementation-ready-code gate, and only where the host has goal capability. **`ce-work` does not also run.** `references/plan-handoff.md`, already loaded at the STOP above, owns the objective's text, its deletion test, and the clipboard recipe; take the objective from there rather than composing one here. Where `create_goal` is available, start the goal with it; otherwise hand the user a copyable `/goal` prompt. Then return to the menu.
