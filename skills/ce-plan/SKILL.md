...
Before dispatching a Phase 1 research subagent, read `references/orca-routing.md` and `references/orca-read-analysis.md`. Only `local-research` and `flow-analysis` can resolve to Orca; organizational/Slack research, external research, and Phase 5.3 deepening stay native because they require controller tools, network/MCP access, or mixed capabilities. For an Orca-owned stage, replace only its already-selected subagent calls with the packet path and do not also launch those nodes natively. The controller still owns every conditional selection, research decision, consolidation, synthesis, and write.
...
- If `CONCEPTS.md` exists at repo root, read it — its definitions are the canonical names for domain entities, named processes, and status concepts. Plan with those terms rather than synonyms. If it declares a parseable `## Contexts` index instead of terms, also read the glossaries of the contexts this plan touches, and qualify a term by its context when the same word means different things across them.
...
Resolve the write target with `references/domain-vocabulary.md` before adding an entry. Gap-fill is a silent path, so it never asks and never creates: when the owning context is ambiguous, or the owning context's glossary does not exist yet, skip the write and record the term under the plan's Open Questions instead. A blocked state from that reference stops gap-fill entirely.
...
3. **Prototype a remaining feel-question** - Invoke `ce-prototype` on a named remaining question that requires use, not inspection. Shown only when such a question remains. A visual-probe question that already settled fails this predicate. The option description names the proposed slice. When this option is shown, omit **Decide on the review's open items** from the same menu.
