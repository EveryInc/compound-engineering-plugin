...
3. **Config.** Apply the ordinary-key rule below: the first **active (non-commented)** `plan_output:` matching `md` or `html` (case-insensitively) wins. Missing, invalid, or commented values continue to the next layer, then step 4. Lines starting with `#` are YAML comments and must be ignored — the shipped config template includes commented examples like `# plan_output: html`, and matching those as active settings would silently force HTML on every run.
...
- **Run it as a `/goal`** — Offered on the same implementation-ready-code gate, and only where the host has goal capability. **`ce-work` does not also run.** Build the objective exactly as `references/plan-handoff.md` specifies and do not compose your own: it must pass that reference's deletion test, which keeps the substituted plan path and nothing else plan-specific — an objective that also names a command, a file path inside the plan, a U-ID dependency, a stop condition, or a DoD item is too fat, and it should otherwise read identically for any plan, it points the session at the plan's Goal Capsule and then each unit with its cited R/F/AE/KTD and any Product Contract Key Decision whose exact `Governs R…` links name that unit's cited R-IDs, and it carries that reference's PR-precedence line rather than a hardcoded open-a-PR directive. If `create_goal` is available, call it with that objective and do not call `update_goal` — the goal session completes itself. Otherwise print it as a copyable `/goal` prompt and copy it using that reference's quoted-sentinel here-doc recipe, which is what keeps the shell from evaluating the prompt's own backticks and `$`. Then return to the menu.
