...
**Frontend architecture grounding.** When planning frontend features (React components, state management, CSS architecture, performance budgets), also invoke `ce-ui-engineering` for implementation-level UI conventions (naming, component structure, state management, JSX hygiene, styling, cleanup). Then read `references/frontend-architecture-guide.md` for architecture-level decisions (component composition patterns, folder structure heuristics, CSS architecture, Web Vitals budget planning, deep module design, interface design). Apply both when composing the High-Level Technical Design and each implementation unit's component boundaries. Authority order: repo instructions (`AGENTS.md`, `architecture.md`, `projectbrief.md`) > repo `docs/solutions/*` > `ce-ui-engineering` > the architecture guide > general guidance.
