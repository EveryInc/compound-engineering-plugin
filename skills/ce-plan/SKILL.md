...
**Pipeline mode exception:** In LFG or any `disable-model-invocation` context, skip the interactive menu and return control to the caller after the plan file is written, confidence check has run, and `ce-doc-review` has run in non-interactive mode or `ce-plan` has recorded the documented `skill_unreachable` envelope (per `references/plan-handoff.md`). Pipeline mode forces `OUTPUT_FORMAT=md` at Phase 0.0, so the 5.3.8 format gate never selects the HTML skip path in pipeline runs. When returning control, emit a structured record with exactly four fields so the caller reads them instead of re-deriving them from prose: `status` (`complete`, `blocked`, or `failed`), `plan_path` (the written plan file), `artifact_readiness` (the plan's recorded readiness), and `doc_review_state` (a one-line string: the non-interactive `ce-doc-review` outcome summary, or the literal token `skill_unreachable` when the documented skip envelope was recorded). Interactive mode is unchanged.
