...
- **Prototype a remaining feel-question** -> Invoke the `ce-prototype` skill under the cross-skill invocation rule, passing the plan path as the skill argument. Do not build a prototype in this skill. If it cannot be invoked, say that it did not run and return to the menu. After it returns, re-render this menu. A markdown plan it wrote back to is now requirements-only, so the execution options drop out on their own. An HTML plan is not: `ce-prototype` cannot write back to HTML, so it returns the decisions instead and the artifact still carries its pre-prototype HOW — say that, hide the execution options, and route to `ce-plan` re-enrichment as markdown rather than offering to ship the behavior the prototype just rejected.
