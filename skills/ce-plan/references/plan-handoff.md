...
If the planning run carries a `ce-job` ledger, include that ledger path in the `ce-doc-review` invocation as `job:<ledger>` and update the ledger with the completed review state before the handoff menu. When the invoking workflow provides an executable `ce-job` helper command, record the plan with `receipt <ledger> plan --status passed --artifact <plan-path>` and the review with `receipt <ledger> doc-review ...`. Do not name or traverse into sibling skill directories from this skill. If `ce-doc-review` cannot be invoked, record the skipped review state in the ledger too; a guarded job must not be marked ready for implementation from an unreviewed plan.
...
- **Start `ce-work`** -> Classify the artifact first. If the plan contents do not support code implementation or a launch blocker remains, do not execute it; route requirements-only artifacts back to `ce-plan` enrichment and non-code artifacts to their own workflow. If it is executable, invoke the `ce-work` skill under the cross-skill invocation rule, passing the plan path as the skill argument; when this planning run carries a `ce-job` ledger, pass `job:<ledger>` too. `ce-work` then chooses the engine (inline/subagent vs goal-mode vs dynamic-workflow) and carries the implementation through verification and shipping. If `ce-work` cannot be invoked, print the existing `ce-work` fallback prompt for the user to run; in that prompt, tell the executor to read Goal Capsule, Verification Contract, Definition of Done, and active U-IDs (scanning headings to find them) rather than the whole document first. Do not merely tell the user to type an invocation when the host can invoke it directly.
