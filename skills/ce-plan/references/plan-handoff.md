...
**Summary above the menu (always):** Translate the non-interactive review envelope into the owner surface defined by `ce-doc-review`'s `references/rendering-floor.md`. Lead with whether the plan is ready and the effect on users, the product, or the team. Then state what changed, what decision remains, and the recommended action. Do not show priority codes, document IDs, reviewer names, confidence values, route classes, coverage counts, or validation mechanics. When no fixes were applied and no findings remain, print `Doc review clean — no changes or decisions are needed.` When the envelope carries `skipped_reason: skill_unreachable`, print `Doc review skipped — ce-doc-review could not be invoked (<skipped_detail>); it did not run.` If a review that actually began failed, print `Doc review failed after starting — <actual error>; the plan was not fully reviewed.` Never describe a pre-entry harness or delegation failure as a downstream skill timeout.

**Do not render the returned envelope verbatim.** It is an agent-to-agent record. Translate every decision-relevant finding into User impact / Recommended change / If unchanged. Group findings when one action fully covers them. Keep technical evidence and trace available for an explicit request. End with `Ask for technical details if you want the reviewer evidence and document trace.`
