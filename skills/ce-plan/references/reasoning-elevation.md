...
**If the calling skill already resolved this at its Phase 0.0** (ce-plan records it as `MODEL_ELEVATION`), reuse that result and skip straight to Adapter selection — do not re-resolve. The precedence below is the resolution it performed; it is restated here for callers that reach this reference without a pre-resolved choice.
