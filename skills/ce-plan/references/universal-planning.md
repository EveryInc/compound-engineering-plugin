...
1. **Save to disk** — Write the plan as a markdown file. Ask where:
   ...
   - `/tmp`
   - A custom path
   - Use filename convention: `YYYYMMDDTHHMMSSZ-<descriptive-name>-plan.md`, using the current UTC time in ISO 8601 basic form; reserve the path atomically and, on collision, retry with the smallest available numeric suffix before the extension rather than overwriting
