...
- When the user asks to check something → use `agent-browser` for screenshots and snapshots. Use `chrome-devtools-mcp` (if available) when the user's concern is about API calls (`list_network_requests`), console errors (`list_console_messages --types error`), or performance (`performance_start_trace` + `performance_stop_trace`). See the hybrid-driver reference in the `ce-test-browser` skill for the division of labor.
