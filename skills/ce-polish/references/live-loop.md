...
A `kind: "final"` batch means the riffer pressed the overlay's Done control, after confirming each unit's intended element and change. It carries the backlog plus anything newly held, and it arrives after the page has ended its side of the session. Your agent token stays valid only while this batch is unacknowledged: the endpoint retires it the moment the final batch is acked with nothing else held. So this batch reverses the usual order: do not ack it yet. Act on everything it carries per the mode (a Collect session applies its whole backlog now, as one pass; an Instant or Smart session applies whatever is left), post every `applied` or `blocked`, run `status` once to confirm nothing is left at `triaging` or `accepted`, and acknowledge it last.

Close-out is decided by the endpoint's state, not by the batch: the `final` batch can reach you before the page's `/session/end` upload has finished, or when that upload failed (`archive_write_failed`, the session stays live and the page retries Done). So after the ack, run `status` and read `session_ended`. True: close out. False: tell the riffer in one line that the session is still open on the endpoint and to press Done again (or reload and press Done) so the archive lands, then park one wait; that wait exits 1 once the session has ended. If it instead exits 0 with another batch, handle it as usual. If no end arrives within a few minutes, or the riffer says they are done, abort explicitly: `stop --root "$LIVE_ROOT"` ends the session with the log written so far, and close out from there, saying in the report that the archive upload did not complete. Never park a second wait on a session `status` says is still live without having spoken to the riffer.

A page that goes away without Done does not end the session: the endpoint reports it as a `page_lost` wake (after an edit) or nothing at all. When the riffer confirms they closed the page for good, the same explicit abort applies: `stop`, then close out with whatever was applied. `wait` exit 1 only ever means the endpoint ended the session with nothing held.
