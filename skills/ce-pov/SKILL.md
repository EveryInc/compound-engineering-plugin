...
<!-- ce-dispatch-exclude:scout-routing-overview -->
...
<!-- ce-dispatch-exclude:bounded-inline-alternative -->
...
**Routing batch: `ce-pov.grounding-scouts`.** Once the existing reversibility-tier and capability gates select the scout roster, and before any selected persona prompt is read or assembled, load `references/execution-routing.md` and resolve the selected IDs from `ce-pov.project-grounding-scout`, `ce-pov.precedent-activity-scout`, and `ce-pov.external-evidence-researcher` together in one `ce-routing/v1` `resolve_batch` against one frozen snapshot. Tier 1 keeps the precedent role absent, and an unavailable external surface does not become present because it has a route.

<!-- ce-dispatch-site:ce-pov.grounding-scouts -->
...
**Routing batch: `ce-pov.panel-peers`.** After the panel protocol fixes the participant set and before any peer payload or prompt is assembled, resolve one `ce-pov.panel-peer` request entry per selected peer in a single `ce-routing/v1` `resolve_batch`, passing the exact full Phase 1 `snapshot` object as `parent_snapshot` (and only a matching `parent_snapshot_id`) when present, and otherwise freezing this as the run's first snapshot. Routing cannot summon a panel, add a peer, replace a named peer, alter independent/skeptic mode, or change the reconciliation cap.
Invoking a named peer, an explicit cross-check, or `oracle` authorizes the panel protocol's normal read-only consultation against this project. Announce the selected peers before dispatch. A retry that would add an unsanctioned recipient or intermediary is unavailable and advances or blocks under the frozen policy without asking; an active instruction that requires separate approval therefore makes that candidate unavailable for this run. Peers request reads from the declared repository scope and cannot edit it; Cursor-family peers use an empty private project workspace, so an outside-workspace read denial makes that voice unavailable rather than widening or trusting the repository. The panel protocol preserves an unbiased initial round, bounds evidence-based reconciliation while honoring user-supplied pass limits, and attributes only receipt-supported independence.
