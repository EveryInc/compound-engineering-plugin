...
Ask through the host's blocking question tool, one at a time: `AskUserQuestion` (Claude Code; `ToolSearch` it if unloaded), `request_user_input` (Codex), `ask_question` (`agy`), `ask_user` (Pi). Numbered chat options only when none exists or the call errors; never silently skip the question.
