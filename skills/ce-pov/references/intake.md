...
- **Intent ambiguous** → propose, built from Step 1's orientation. Use the blocking question tool with the **concrete candidate framings this specific input suggests** (naming the incumbent where you know it) plus a free-text "something else." Do not offer a generic checklist; offer the real readings of *this* input. Example for a passkeys link on a password-auth project: *adopt passkeys* · *migrate auth to them (and at what cost)* · *compare them to our current sign-in* · *something else*.
