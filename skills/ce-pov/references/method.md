...
- **Project floor** — PASS requires either (a) a named incumbent **and** at least one concrete project touchpoint (a `file:line`, a dependency, an issue, a PR, or a doc passage from the dossiers), or (b) a prior decision on this question. FAIL → return **"Hold — insufficient project grounding"** with a numbered list of exactly what to inspect to make the floor passable. Forbidden from Adopt/Reject on a failed project floor, regardless of how strong the external evidence is.
