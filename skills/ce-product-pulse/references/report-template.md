...
- If `STRATEGY.md` exists, re-read its key metrics before assembling the report (`## Key metrics` when `ce-strategy` wrote it; otherwise the section that lists the success measures, by meaning). For each strategy metric, decide what to render:
