...
- If `STRATEGY.md` exists, re-read the key metrics before assembling the report from the same source setup resolved: `## Key metrics` when `ce-strategy` wrote it, otherwise the section listing the success measures by meaning, and the legacy sibling `STRATEGY.md` points to when it defers them there. For each strategy metric, decide what to render:
