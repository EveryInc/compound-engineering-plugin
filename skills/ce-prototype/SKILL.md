...
Build under `.context/compound-engineering/ce-prototype/<date>-<slug>/`, so the prototype is still there when the implementation that follows reads it alongside the decisions capsule. That path has to be gitignored first: probe `git check-ignore -q .context/compound-engineering/` — the trailing slash is load-bearing — and when it is not covered, offer to append that one line to the repo-root `.gitignore`, appending only if they agree and leaving the rest of the file alone. Fall back to `/tmp/compound-engineering-<uid>/ce-prototype/<date>-<slug>/` when they decline, when the run is not inside a git repository, or when the path fails the safety checks; survival there is best-effort, so do not promise it a lifetime. Claim the directory by creating it rather than testing whether the name is free, and take the next numeric suffix when creation reports it already exists. Give each question in a multi-question run its own child directory. Never delete a kept prototype — the directory is theirs to prune.

Load `references/preview.md` when serving that local web prototype; it resolves the run directory once and both the start and the status/stop calls take that same path. Recreate what this question needs from the current product. Do not stand up the full app unless the question is the whole-product feel.
