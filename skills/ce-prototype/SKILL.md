...
A run in a repo is about that product unless the user says otherwise. Read the current repo, this conversation, and any supplied brainstorm or plan before asking.
...
After a file-free or HTML run: if a calling skill invoked this, return the choices in `decisions.md` and let that caller continue. Otherwise recommend `ce-brainstorm` when product-level questions remain, or `ce-plan` when the session is enough to plan, and pass this session as the seed, printing that recommendation per the rendering rule above.
