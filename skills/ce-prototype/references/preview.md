...
Before handing over the URL, look at the rendered screen — a screenshot where the platform has one, otherwise measure the laid-out result in the DOM. A 200 on every asset is not that check: an image that loads correctly at the wrong size passes it and still wrecks the layout, and so do a stylesheet that fails to parse and a script that leaves the page inert. Check each variant, not just the page — one bug in shared scaffolding reads as several bad designs. Anything you tell them to try, try first. If you have no way to see the rendered result, say so when you hand over the URL rather than implying it was checked.
