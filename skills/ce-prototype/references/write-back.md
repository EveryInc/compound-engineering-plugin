...
Where a branch below sends the run to a recap, that recap — the decisions and the prototype path — is the run's complete result, not a degraded write-back. What fails closed is the write, not the run.
