...
The shape: **fetch once, judge centrally, fan out only the fixes.** You hold every thread from a single fetch, so the legitimacy judgment happens in your context. Subagents are dispatched only to *implement* fixes you have already approved.
...
Then fetch all feedback using the GraphQL script at [scripts/get-pr-comments](../scripts/get-pr-comments). In each block below that runs a bundled script, set `SKILL_DIR` inline to the absolute directory you loaded this skill's SKILL.md from — shell state does not persist between Bash calls. If a bundled script errors, report the failure rather than substituting REST calls: the REST endpoints carry no thread IDs and cannot resolve threads.

**GitHub Enterprise host.** The bundled `gh api graphql` scripts hit `gh`'s default host unless told otherwise, so on a GHE PR they would wrongly target `github.com`. Take the host from the PR URL if one was passed, otherwise from `gh repo view --json url -q .url`, and pass it as a `GH_HOST=<host>` **env prefix inline on every bundled-script call** (`gh api` honors `GH_HOST` as the request host) — a single `export` does **not** carry to the reply/resolve/verify blocks that run as later Bash calls. On `github.com`, drop the `GH_HOST=<host> ` prefix entirely.
...
**Silent drop.** Non-actionable items are dropped without narration. Do not announce, list, or count dropped items in conversation, the task list, or the step 9 summary. Review-bot wrapper text commonly appears here -- recognize it by its boilerplate content, drop silently. The fetch layer pre-filters only blank bodies and messages from the known PR author. All external identities and surfaces, including CI/status bots such as Codecov, rely on this content-aware check so identity reuse or format changes cannot silently hide actionable feedback.
...
Read [references/agents/pr-comment-resolver.md](agents/pr-comment-resolver.md) and spawn a generic subagent seeded with that fixer prompt for each fix-list item. Do not dispatch a standalone agent by type/name.
...
The fixer prompt defines what each fixer returns (`verdict`, `feedback_id`/`feedback_ids`, `reply_text`, `files_changed`, `reason`).
...
No two fixers that touch the same file run in parallel. You already know the target files from step 3 — serialize fixers that share a file (dispatch one, wait, then the next); non-overlapping items run in parallel. For a **class item**, feed the fixer its full enumerated location set and every covered feedback ID (not a single thread), and account for **all** of its sites in this check — a class fix touching files another fixer also touches must be serialized against every one of them. When one fixer handles multiple threads on the same file, it addresses them sequentially. A fix can still expand past its referenced file (renaming a method updates callers elsewhere); if validation or verify surfaces inconsistent changes, re-run the affected fixers sequentially.
...
Stage only the files fixers reported, commit with a message referencing the PR number and listing the changes from the fixer summaries, then push.
...
All replies quote the relevant part of the original feedback for continuity — the specific sentence or passage, not the entire comment if it's long. For `needs-human` verdicts, post the natural-sounding reply but leave the thread open for human input instead of resolving it.

### Review threads

0. **Verify the thread ID** before replying. GitHub Enterprise can return inconsistent node IDs for the same thread depending on the query path. Always confirm the ID from `get-pr-comments` resolves to the correct thread using [scripts/get-thread-for-comment](../scripts/get-thread-for-comment) with the comment's numeric URL ID. Extract the numeric comment ID from the comment URL (e.g. `discussion_r2589700` → `2589700`) for the `gh api` call:
```bash
SKILL_DIR="<absolute path of the directory containing the ce-resolve-pr-feedback SKILL.md>";
...
1. **Reply** using [scripts/reply-to-pr-thread](../scripts/reply-to-pr-thread). Never reply via `gh pr review` or a `/reviews` POST — those open an unsubmitted draft review that swallows this reply and every one after it.
...
3. **Resolve** using [scripts/resolve-pr-thread](../scripts/resolve-pr-thread):
```bash
SKILL_DIR="<absolute path of the directory containing the ce-resolve-pr-feedback SKILL.md>";
...
**If new threads remain**, repeat from step 2 for them. After about two fix-verify cycles, stop looping: raise **one** root-level `needs-human` naming the recurring pattern -- what has been fixed and what keeps reappearing -- and leave those threads open for the user to decide.
...
Close with a per-verdict summary of the work: counts for `fixed` / `fixed-differently` / `replied` / `not-addressing` / `declined`, one line per item saying *what was done* and why (evidence for `not-addressing`, the harm cited for `declined`), and the validation result (command plus pass/fail counts, and any pre-existing failure noted) when code was committed.

For each `needs-human` item, present its `decision_context` verbatim (composed in step 3, or by a fixer's escalation) -- it is already structured for a fast decision. Those threads keep their natural-sounding reply and stay open on the PR. Also surface any **pending decisions from a previous run** (threads step 2 classified as already responded to but still unresolved), with a link to the existing reply and what was asked.

If the harness exposes a blocking question tool and the run is interactive, use it to ask about all open decisions (new `needs-human` plus previous-run pending) together, then process what the user decides: fix the code, compose the reply, post it, resolve the thread. The capability differs per harness — `AskUserQuestion` in Claude Code (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded), `request_user_input` in Codex, `ask_question` in Antigravity CLI (`agy`), `ask_user` in Pi (requires the `pi-ask-user` extension). **Default when no such tool exists, the call errors, or the run is unattended (including `mode:pipeline`):** leave the decisions stated in the summary and the threads open on the PR rather than blocking — never silently skip them.
