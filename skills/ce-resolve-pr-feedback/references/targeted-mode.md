...
**GitHub Enterprise host.** Take the host from the URL (targeted mode is always URL-triggered). When it is **not** `github.com`, pass it as a `GH_HOST=<host>` env prefix inline on **every** `gh api` / bundled-script call below — shell state does not persist between Bash calls, so a single `export` would leave later calls hitting `github.com`. Carry the same host into the reply/resolve calls you run from Full Mode steps 5-7. On `github.com`, drop the prefix.
...
**Step 2** -- Map comment to its thread ID. Use [scripts/get-thread-for-comment](../scripts/get-thread-for-comment), setting `SKILL_DIR` inline to the absolute directory you loaded this skill's SKILL.md from. If the script errors, report the failure rather than substituting REST calls: the REST endpoints carry no thread IDs and cannot resolve threads.
```bash
