...
**Routing batch: `ce-riffrec-feedback-analysis.source-mapping`.** Only when the existing workspace/scale decision selects subagents rather than inline mapping, and after the product-area instances are fixed but before any mapper prompt is assembled, load `references/execution-routing.md` and resolve every already-selected product-area instance of `ce-riffrec-feedback-analysis.source-mapper` together in one `ce-routing/v1` `resolve_batch`, with one request entry per selected product area. If an inherited native snapshot exists, pass its exact full self-validating object as the `parent_snapshot` envelope and include `parent_snapshot_id` only if it matches; otherwise this response becomes the first frozen snapshot. Never use ID-only lineage or reread live routing sources, and reuse the frozen bindings on recovery. Routing cannot choose subagents over inline mapping, split or add product areas, or alter read-only, concurrency, evidence, and failure semantics.
<!-- ce-dispatch-site:ce-riffrec-feedback-analysis.source-mapping -->
