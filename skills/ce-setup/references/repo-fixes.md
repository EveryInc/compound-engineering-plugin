...
When the health report warns about `work_engine_effort`, the engine itself is still usable; the warned value just requests nothing. The key is a map from harness to one of that harness's own effort levels, and Cursor routes take no effort. Ask what effort the user wants for each harness rather than guessing, show the replacement map, and edit the layer the warning names. Re-run the health check and require the warning to be gone.
