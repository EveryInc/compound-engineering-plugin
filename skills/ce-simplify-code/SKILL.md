...
When the scope contains frontend code (React components, TypeScript types, CSS/Tailwind, hooks, or state management), also invoke `ce-ui-engineering` to load the authoritative UI engineering conventions into context. Then read `references/frontend-simplification-patterns.md` from this skill's directory. Fold both into the matching reviewers' prompts at dispatch — `ce-ui-engineering` carries the convention rules (naming, clarity/simplification, state management, cleanup, Red Flags); `frontend-simplification-patterns.md` carries simplification-specific checks (memoization removal, context splitting, hook over-abstraction, TS shape simplification, CSS trim, deep module vocabulary). Authority order: repo instructions (`AGENTS.md`, `architecture.md`, `projectbrief.md`) > repo `docs/solutions/*` > `ce-ui-engineering` > `frontend-simplification-patterns.md` > general guidance.
