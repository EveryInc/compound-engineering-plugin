...
<!-- ce-dispatch-exclude:no-yield-gate -->
...
**Routing batch: `ce-simplify-code.reviewers`.** Once the no-yield gate has retained the fixed three-reviewer roster and before any persona prompt is read or assembled, load `references/execution-routing.md` and resolve `ce-simplify-code.code-reuse-reviewer`, `ce-simplify-code.code-quality-reviewer`, and `ce-simplify-code.efficiency-reviewer` together in one `ce-routing/v1` `resolve_batch` against one frozen snapshot.

<!-- ce-dispatch-site:ce-simplify-code.reviewers -->
