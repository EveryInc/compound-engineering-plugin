...
Run code-reuse, code-quality, and efficiency as three distinct review lenses; separate contexts are preferred, not required. When the current tool surface exposes subagent dispatch, assign one generic subagent to each review and give it the corresponding persona text verbatim plus the resolved scope (the full diff or file set). Run them concurrently up to the platform's limit, or dispatch the same three payloads sequentially when concurrency is unavailable. Without a subagent primitive, the parent agent performs the reviews inline, following each persona as a separate pass. If dispatch is permission-gated, use the platform's blocking question tool to ask the user to authorize the remaining subagents, then retry once when granted. If the user declines or the question tool is unavailable or errors, run only the affected reviews inline and disclose the substitution. For any other failure besides concurrency backpressure, run only the affected review inline and disclose it. Describe a review as delegated or independent only when a subagent actually returned it; inline agreement is not independent corroboration.
