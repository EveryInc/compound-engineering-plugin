...
For each section: ask the opening question, push back when the answer falls into a named anti-pattern, and capture the final answer in the user's own language.

## Overall Rules

1. **Push back once, maybe twice.** If the first answer is weak, name the specific issue and ask a sharper question. If the second answer is still weak, capture what the user has given and note in the draft that the section is worth revisiting. Do not let the interview spiral.
2. **Quote the user back at them.** When challenging an answer, use the user's own words verbatim. Paraphrasing softens the challenge and is easier to dismiss.
3. **Keep each answer to 1-3 sentences.** Longer answers are usually hiding something vague. If the user writes a paragraph, ask them to pick the sentence that matters most.
4. **Don't leak the anti-pattern names.** The user does not need to hear "that's a vanity metric" - just ask the sharper question that follows.
...
**Capture:** One or two sentences naming the user's situation and the crux that makes it hard right now. No solution language.
...
- **Feature list** ("we're building AI-powered X, Y, and Z") -> "That's a feature list. What's the underlying bet that makes you pick those features over others?"
- **Product description as approach** ("we use AI to draft replies") -> "That's what the product does - every competitor will say the same. What are you betting on that they're not?"
- **Goal restated** ("our approach is to be the market leader") -> "That's still the goal. What choice are you making that competitors aren't?"
...
- **Role without situation** ("PMs") -> "PMs doing what? Running a roadmap review? Writing a spec at midnight? The situation is where the product matters."
- **Generic job** ("they want to be more productive") -> "Productive at what specifically? They're hiring this product to do *what*?"

**Capture:** One primary persona plus a JTBD verb phrase (additional personas allowed but secondary). Example: "Solo founders running their own roadmap. They're hiring the product to keep strategy and execution aligned without a PM on staff."
...
- **Too many** ("here are 12 metrics we watch") -> "A dashboard isn't a strategy. Pick the 3-5 you'd stake the quarter on."
...
**Capture:** A list of 3-5, mixing leading (moves weekly) and lagging (moves quarterly). Each with a one-line definition. Note where each is measured (analytics, DB, qualitative, etc.) if known. If measurement is undefined, ask: "Where does this metric live today? If nowhere, is this something you can start measuring?"
...
- **Doesn't connect to approach** (approach: "win by being the easiest to onboard"; track: "enterprise SSO") -> "How does that track serve the approach? If it's a separate bet, name it as one."
...
**Capture:** 2-4 tracks, each broad enough that multiple features live inside it. For each: a name, a one-line purpose, and a short note on why this serves the approach.

---

## Optional sections (6-8)

Ask once each, accept a skip, and do not push the user to invent content.

**6. Milestones:** "Are there any dated milestones worth anchoring - a launch, a fundraise, a conference, a renewal? Skip if none apply." Only externally visible, real milestones, captured verbatim with dates. Not an internal schedule.

**7. Not working on:** "Is there anything you've explicitly decided *not* to do right now that's worth naming? This is for things the team keeps being tempted by." One line per item.

**8. Marketing:** "Any positioning or narrative language you want the doc to carry - a one-liner, a tagline, a key message? Skip if not yet." 2-3 lines max.
