...
- Set `SKILL_DIR` to this skill directory, resolve one private run directory for this item's attempt with `RUN_DIR="$(python3 "$SKILL_DIR/scripts/scratch-root.py" run-dir --skill ce-sweep --run-id <run-id>)"`, and download attachments there; raw media is never committed. A download failure increments `media_attempts`, removes that exact run with `python3 "$SKILL_DIR/scripts/scratch-root.py" remove-run-dir --skill ce-sweep "$RUN_DIR"`, sets the item `needs_download`, and continues. If exact cleanup fails, record the sweep `partial`, report the retained absolute path, and never claim the sensitive media was removed.
- Dispatch one generic subagent per recording, in parallel, at the **generation tier**, using `references/subagent-template.md` filled from `references/agents/media-analyzer.md`. Fill the template's `{skill_dir}` slot with the same absolute ce-sweep skill directory you resolve for your own `SKILL_DIR` Bash calls (a fresh subagent does not inherit your shell state, so it cannot run the bundled analyzer without being told the path). Pass the absolute media PATHS, a scratch artifact path, and the item's `sensitive` flag; collect the compact 1-2 line summary each returns. After every analyzer has returned and its summary/artifact has been consumed, remove the exact `RUN_DIR`. An analyzer failure also increments `media_attempts`, removes the run instead of retaining raw media, and sets the item back to `needs_download` (not `needs_analysis`) so a later invocation re-fetches into a fresh opaque run. The item state persists the count; it never persists or reconstructs a disposable run path.
- Track attempts on the item (the `media_attempts` count is upserted on each download/analysis try). The next invocation of a `needs_download` item creates a fresh run, re-downloads, and retries with the persisted count. After 3 failed attempts across runs, clean the current exact run first, then set the item `manual_stuck` and list it separately — out of the routine nag. No success, failure, or manual-stuck path intentionally retains raw media.
