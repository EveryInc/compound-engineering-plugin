...
Run this once at the start of this invocation, before any subagent dispatch, and follow the directives it prints; where one conflicts with this skill's own rules on asking the user questions, those rules win whatever their scope, and no blocking question is asked. Run the fence exactly as written, as its own command: never pipe it, filter it (`head`, `tail`, `grep`), truncate its output, or bundle it with other commands. Its output opens with a `=== skill context` header and ends with `CE_CONTEXT_END`; one without the other means it was truncated, so rerun the fence verbatim once — the only rerun, since a later invocation of this or any other skill runs its own. If no Node runtime is available the skill proceeds unchanged.
