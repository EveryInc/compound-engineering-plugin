...
Multiple agents may run on the same machine, so never assume the preferred port is free: scan upward from the preferred port to the first free one, then start the dev server there in the background (`bin/dev`, `bin/rails server -p`, or `npm run dev`), logging to a temp file and polling up to ~30s for it to listen. If it never listens, report the last log output and stop.
