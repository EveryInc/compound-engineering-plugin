...
2. **Read `.user-test-last-run.json`:**
   - Missing: abort with "No run results found. Run `/ce-user-test` first."
   - `completed: false`: abort with "Last run was incomplete. Run `/ce-user-test` again."
3. **Read `.user-test-last-report.md`:**
   - Missing: abort with "No report artifact found. The skill version may predate report persistence — run `/ce-user-test` again with the latest skill."
...
Proposed changes to the ce-user-test skill based on eval failures.
