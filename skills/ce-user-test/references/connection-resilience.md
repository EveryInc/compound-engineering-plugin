...
7. When `mcp_call_counter` reaches `mcp_restart_threshold` (default governed by `mcp_restart_threshold` in `../scripts/caps-registry.json`, configurable in test file frontmatter): navigate to the app entry URL (full page reload). Reset `mcp_call_counter` to 0. Log: "Proactive restart at call #N to prevent connection degradation."
