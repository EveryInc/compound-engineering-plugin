...
4. **Choose Execution Engine, then Strategy**
   ...
   **Routing batch: `ce-work.native-implementation`.** Once strategy and the Parallel Safety Check fix the unit wave, and before unit packet assembly, load `references/execution-routing.md` and consume the already-frozen `ce-routing/v1` `resolve_batch` binding for `ce-work.implementation-worker`; that batch has one request entry per already-selected unit instance. Do not resolve a second binding. Routing cannot create a unit, change isolation, or make an unsafe wave parallel. Before accepting a profile candidate's native output, call `finalize_attempt` with the exact snapshot, unit instance's candidate lock, typed outcome, and complete prior receipt history; never send a binding. A top-level `ce-default` binding keeps the built-in path and has no candidate attempt to finalize. `next_candidate` is allowed only after terminal unintegrated evidence, and `block` leaves the unit unchanged.
