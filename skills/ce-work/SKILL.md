...
1. **Review receipt:** you invoked `ce-code-review mode:agent` through the host's normal skill-invocation mechanism and hold a **completed** JSON receipt with `status: complete` plus `artifact_path` or `run_id`, then ran apply/residuals per `shipping-workflow.md`. The default owner summary is for people, not programmatic consumption. Do **not** treat `status: failed`, `degraded`, or `skipped` as a completed receipt even if `artifact_path`/`run_id` is present; those enter the unavailable path in `shipping-workflow.md`; or
