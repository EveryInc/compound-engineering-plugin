...
`mode:subagent <plan-path>` runs a strict, subagent-only execution: **every implementation unit runs in a subagent, including trivial 1-2 file work** that Phase 0 triage would normally run inline. The main loop performs no implementation itself — it orchestrates, integrates, stages and commits, owns **all** user interaction, and runs whatever post-implementation tail the run's tail-ownership mode calls for. This keeps the orchestration context clean on long multi-unit runs, and lets subagents carry knowledge forward through a shared notes document so later workers stop re-deriving setup and re-fixing the same bugs.
...
- **Tail ownership is unchanged by this mode.** Subagent Mode governs *how implementation is dispatched*, not who owns the post-implementation tail (review, PR). In a standalone run the main loop runs the shipping tail and dispatches review. Under `mode:return-to-caller` — the two tokens compose (Phase 0) — it still implements and locally verifies only, then returns the structured envelope; it does **not** dispatch review or open a PR, because the caller (e.g. `lfg`) owns that tail. Never run review twice or bypass the caller-owned gate.
...
For the shared notes document — its schema, the claim/ledger protocol, how it is seeded into each worker packet, where it persists, how the orchestrator curates it, and the end-of-run `ce-compound` handoff — read `references/subagent-mode.md`.
