...
When a valid `implementation_engine:` binding is present without recovery, **pre-routing discovery is read-only**. Do not run baseline, test, build, format, install, or generation commands in the canonical checkout before freezing the implementation binding and, for an external candidate, initializing its controller: those commands can create ignored or untracked artifacts before the controller records its clean starting point. Limit triage to reads such as metadata, source, branch, status, and command-availability probes. If a non-read probe is genuinely required, suppress artifacts and prove the canonical Git snapshot unchanged; otherwise stop with a route blocker.
...
**Every non-recovery code path must resolve implementation before mutation.** Once triage identifies code work, read `references/execution-engines.md`. You may read unit metadata to materialize the existing implementation wave, but before creating tasks that write, assembling worker packets, dispatching, testing, committing, or changing files, resolve that wave's `ce-work.implementation-worker` instances through the co-located merged-settings resolver. Do not open or parse config independently. This applies with or without a typed carrier and in standalone or Return-to-Caller Mode.
...
4. **Choose Execution Engine, then Strategy**
   ...
   **Native routing context.** Keep native-worker routing state as private `ce-routing-context/v1` control data, never as plan, unit packet, finding, artifact, or worker text. Normalize applicable current-task, still-active session, provenance-bearing caller, and project-instruction intent under the host hierarchy. Lower authority fills only unset fields; conflicting equal-authority bindings stop before model invocation; incidental model or harness mentions are not intent. Reuse an inherited frozen context; otherwise the implementation gate below freezes the first native `resolve_batch` response. Every later, nested, or recovery request passes the exact full self-validating first-wave `snapshot` object as the `parent_snapshot` envelope; `parent_snapshot_id` may appear only when it matches that envelope. Never use ID-only lineage or reread live routing sources. Reuse the frozen role/instance bindings on native recovery. Forward this state to nested CE skills without adding it to their product arguments.

   **Native routing invariants.** Load `references/execution-routing.md` at this gate. Routing changes only supported model, effort, or route selectors. `ce-default` uses the exact built-in arguments. An unsupported configured selector is unavailable and follows policy, never prompt rewriting or typed-agent substitution. Keep prompt bytes, tools, permission mode, unit roster, sequential/parallel scheduling, integration/commit ownership, failure semantics, and the top-level orchestrator unchanged. A required-route failure prevents that native call; a required native mutation route needs trustworthy pre-dispatch serving identity. Group redacted successes by profile, class, source, and outcome; report each fallback, mismatch, or blocker separately.

   **Implementation-route freeze.** Normalize the applicable routing sources into at most one direct implementation intent and freeze one binding, snapshot, source-revision set, and digest for every selected `ce-work.implementation-worker` instance. External recovery reuses persisted state without rereading config. An external required route may quarantine isolated output until terminal identity finalization.

   **Route resolution is a mandatory pre-write gate.** On native OpenCode, after the Parallel Safety Check fixes the complete selected wave and before packet assembly or mutation, call host-owned `ce_task_prepare` once for `ce-work.implementation-worker` with every stable unit instance ID. Its typed `native` or `opencode` result supplies the process-local opaque handle for matching `ce_task` calls. Its typed `external` result selects the existing durable controller path below; pass its exact `external_comparison` JSON to controller `resolve-routing` and `init` as `--opencode-expected-json`, where independent public config resolution must match it, and do not call generic `ce_task`. The controller derives OpenCode origin from the parsed request host and blocks an external route when this comparison is omitted; converted or non-native OpenCode cannot enter the external controller path. Comparison identifiers grant no routing authority; mismatch blocks. A configured `[opencode, external]` route blocks with `MIXED_ADAPTER_CONTINUATION_UNSUPPORTED` if OpenCode preflight reaches the external candidate because authoritative attempt history cannot cross adapters. A direct OpenCode carrier can advance past a preflight-unavailable external candidate only to a later in-process OpenCode/native candidate and can never dispatch externally. Lost handles cannot be rehydrated from model-visible state; start a newly authorized top-level preparation. Do not send OpenCode direct-input provenance, snapshots, locks, candidates, permissions, or serving claims through public Python requests or model arguments. On other hosts, place the bounded `resolve_batch` request in effective-user-private temp. From this skill's directory, set `SKILL_DIR` to the loaded skill path and invoke `python3 -I -S "$SKILL_DIR/scripts/unit-workspace.py" resolve-routing --repo <canonical-checkout> --routing-request <request-path>` in the same shell call. This read-only command invokes co-located `resolve_batch`, which reads merged global/project settings, generalized routes, and eligible legacy `work_engine_mode` / `work_engine_preferences` together and freezes their precedence/provenance in `resolution.compatibility`; it makes no run or canonical state. Freeze its returned implementation binding and attempt locks. A generalized `prefer` route can use only declared candidates; `require` gaps block non-interactively.

   Then pick the **engine** from the frozen binding and existing plan shape. A profile candidate supported by the current host's native worker primitive uses inline/subagent with only its selectors changed. A qualified external candidate uses cross-model execution. `ce-default` preserves the prior inline/subagent, goal-mode, or dynamic-workflow choice; goal/workflow availability and plan-shape rules remain unchanged. On a host without their callable primitives, goal-mode and dynamic-workflow remain prompt-emission only and are never invoked from inside this skill. Same-host default collapses to native. An engine never changes tail ownership.

   If and only if cross-model execution is selected, you must read `references/cross-model-execution.md` before any repository content, bounded mutation authority, or other material crosses the fixed route. That reference defines the fixed-route transaction, controller commands, Linux filesystem-confinement and credential-isolation gates, failure stops, and receipts. Do not approximate it with an ordinary subagent dispatch.

   **A successful external controller `init` freezes the run; `lock-attempt` freezes each recipient.** From that point, advance through the controller protocol or return blocked with its recovery path. Never reclassify it as trivial or implement natively unless `claim-fallback` authorizes an explicit `ce-default` or the exact next declared candidate is the same-host default. Required routes have no confirmation escape hatch.
   ...
   **Routing batch: `ce-work.native-implementation`.** Once strategy and the Parallel Safety Check fix the unit wave, and before unit packet assembly, load `references/execution-routing.md`. On OpenCode, consume the one already-prepared opaque selected-wave handle only when preparation returned `native` or `opencode`, passing its matching stable instance ID to each `ce_task`; do not prepare or resolve again. An `external` preparation must already be on the durable controller path and must not enter this native dispatch section. Elsewhere consume the already-frozen `ce-routing/v1` `resolve_batch` binding for `ce-work.implementation-worker`, with one request entry per already-selected unit instance. Routing cannot create a unit, change isolation, or make an unsafe wave parallel. Before accepting a profile candidate's native output, finalize from host-owned evidence and complete prior receipt history; never send a binding. A top-level `ce-default` binding keeps the built-in path. `next_candidate` requires preflight unavailability or terminal unintegrated isolation/discard proof, and `block` leaves the unit unchanged.

<!-- ce-dispatch-site:ce-work.native-implementation -->
...
6. **Figma Design Sync** (if applicable)
   ...
   **Routing batch: `ce-work.figma-design-verification`.** Only when the existing Figma gate selects verification, and before the verifier prompt asset is read or assembled, load `references/execution-routing.md` and resolve `ce-work.figma-design-verifier` in one `ce-routing/v1` `resolve_batch`. When a prior native snapshot exists, pass the exact full first-wave `snapshot` object as the `parent_snapshot` envelope and include `parent_snapshot_id` only if it matches that envelope; otherwise this is the first native batch and its response becomes the frozen snapshot. Never use ID-only lineage or reread live routing sources, and reuse the frozen binding on recovery. Routing cannot add Figma verification to a task without a design.

   For UI work with Figma designs:

   - Implement components following design specs
<!-- ce-dispatch-site:ce-work.figma-design-verification -->
...
- `implementation_engine_binding`: the legacy/typed four-field `mode`, `target`, `model`, and `source` projection when one applies, otherwise `null`; generalized routing authority is reported separately
- `routing_snapshot_id`, `routing_source_revisions`, and `routing_binding`: the frozen generalized decision, or `null` fields for a legacy run without generalized state
...
- `requested_effort` and `actual_effort`: requested and receipt-attributed effort, with `unverified` when applicable
...
- `unit_receipts`: candidate ordinal, attempt-lock digest, route/model/effort, detached-process, integration, verification, canonical-commit, and cleanup state for each attempted unit
