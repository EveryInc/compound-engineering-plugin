...
4. **Choose Execution Engine, then Strategy**
   ...
   **Route resolution is a mandatory pre-write gate.** Before any implementation write, native worker dispatch, or implementation commit, read `references/execution-engines.md`; inspect applicable live/session/project intent, any typed caller binding, and `.compound-engineering/config.local.yaml` when it exists; then resolve and record the engine. In that same config read, also capture any active (non-commented) `quality_commands` keys (`test`, `lint`, `typecheck`, `build`) and hold them for verification steps and dispatch packets — do not run a second config pass for them. Do not infer native execution merely because no typed carrier was supplied. Native is eligible only after this gate finds no higher-authority cross-model selection or exhausts a `prefer` route under the reference's fallback contract.
   ...
   **Native dispatch (inline/subagent engines only)** uses your harness's subagent/worker mechanism. Once a unit is selected for cross-model execution, use the loaded controller protocol for that unit; it must not re-enter this ordinary subagent dispatch. Give each native worker:
   ...
   - The resolved quality commands — the captured `quality_commands` entries when configured, otherwise the commands the orchestrator resolved from the project's active instructions — so the worker runs those for its focused self-checks instead of rediscovering them.
...
2. **Incremental Commits**
   ...
   **Commit workflow:**
   ```bash
   # 1. Verify tests pass (the configured quality_commands.test command when set;
   #    otherwise the project's test command — e.g. bin/rails test, npm test, pytest, go test)
...
When the caller supplies a journal path alongside the plan path, append unit-level records — one at each unit's start, one for its verification result, and one for its integration result — to that file via the bundled `journal` subcommand. Shell state does not persist between Bash calls, so each append is a fresh self-contained command that re-declares the `SKILL_DIR` anchor and interpreter probe inline. First write the record JSON to a mktemp file with the file-write tool (`mktemp "${TMPDIR:-/tmp}/ce-work-journal-record-XXXXXX"`) — never hand-escape record JSON into an inline bash string — then run in one command:

```bash
SKILL_DIR="<absolute path of the directory containing the SKILL.md you just read>";
PY="$(for c in python3 python py; do command -v "$c" >/dev/null 2>&1 && "$c" -c '' >/dev/null 2>&1 && { echo "$c"; break; }; done)"; [ -n "$PY" ] || { echo "no working Python 3 interpreter on PATH" >&2; exit 1; };
"$PY" "$SKILL_DIR/scripts/factory-gates.py" journal --file <journal-path> --record-file <record-path>
```

Each record is `{ts, run_id, skill, phase, status, detail?, artifacts?}` with `skill` set to `ce-work`, `ts` an ISO-8601 UTC timestamp, and `status` one of `started|completed|failed|skipped`. Standalone runs (no caller-supplied path) may journal the same records under `/tmp/compound-engineering-$(id -u)/ce-work/<run-id>/journal.jsonl`, applying the scratch-root symlink/ownership check before use. A journal append that fails is noted in one line and never blocks, retries beyond the script's own retry, or fails the run.
