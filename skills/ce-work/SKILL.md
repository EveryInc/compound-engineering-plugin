...
4. **Choose Execution Engine, then Strategy**
  ...
  - **Frontend/UI units — `ce-ui-engineering` for workers.** When the unit touches frontend/UI code (`.tsx`, `.jsx`, `.css`, `.scss`, component, page, layout, or state files), instruct the worker to invoke `ce-ui-engineering` before implementing. If the harness does not support skill invocation from a subagent, include in the dispatch prompt: "Follow the UI engineering conventions from `ce-ui-engineering` — naming, component structure, state management, JSX hygiene, styling, cleanup, and the Red Flags checklist." The worker must treat repo instructions and `docs/solutions/*` as higher authority than `ce-ui-engineering` on conflict.
...
3. **Follow Existing Patterns**
   ...
   - **Frontend/UI work — invoke `ce-ui-engineering`.** When the work touches frontend/UI code (`.tsx`, `.jsx`, `.css`, `.scss`, component, page, layout, or state files), invoke `ce-ui-engineering` to load the UI engineering conventions into context. Its rules are authoritative for frontend code — authority order: repo instructions (`AGENTS.md`, `architecture.md`, `projectbrief.md`) > repo `docs/solutions/*` > `ce-ui-engineering` > skill-local frontend reference files > general guidance.
