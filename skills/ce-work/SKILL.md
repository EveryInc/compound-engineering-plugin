...
2. **Setup Environment**
   ...
   **Find the default branch** — what `origin/HEAD` resolves to; when that is unset, what the host reports as the default (on GitHub, `gh repo view --json defaultBranchRef -q .defaultBranchRef.name`); failing that, `main` or `master` when one exists on origin or locally. If you still cannot tell, treat the branch you are on as the default and take the create-a-branch path below: a spare feature branch is a one-command undo, while incremental commits on the real default are not. Do not treat a bare `main`/`master` as a feature branch under any fallback.

   **Record the pre-work scope** — before editing anything, capture `git status --short --untracked-files=all` (the user's in-progress files, untracked ones listed individually rather than as a collapsed directory) and whether `HEAD` carries commits not on the remote default (`git log origin/<default>..HEAD`; note "unknown" when there is no remote default to compare against). Both feed the shipping gate in Phase 4. Incremental commits stage only work-owned files and are path-limited, so untouched pre-existing dirt never enters a commit; Phase 4 passes every pre-work file this run did not commit as `exclude:<paths>` on the shipping invocation, so the shipping skill's own dirty-file scan cannot sweep it in either. It rides along on any branch move (`git checkout -b` and `git branch -m` both carry uncommitted changes), which is the intended outcome — do not stash it, do not ask about it, and do not derive the branch name from it: name the branch from the plan or work description regardless of what is dirty. Where a unit later edits a file that was already dirty at start, one commit cannot separate their edits from yours (`ce-commit` and the incremental-commit rule group at file level). Ask once — one question covering every such file, at the first commit that would include one — whether to commit those files with their pre-existing edits included, or leave them uncommitted for the user to handle. That is the single branch/tree question this skill asks, because both answers lose something the agent cannot choose for the user. "Leave them uncommitted" is an exclusion that holds for the rest of the run: those files stay out of every later commit, including the shipping handoff — pass them as `exclude:<paths>` on the `ce-commit-push-pr` / `ce-commit` invocation (both honor it; without it either skill would commit the still-dirty file at file granularity) — and name them in the final summary as work-owned changes left unshipped. In Return-to-Caller Mode decide before the edit, not at the commit: when a unit is about to touch a file that was dirty at start, do not edit it — return `status: blocked` naming the file and why (the caller runs headless and would push whatever you commit; the user's uncommitted edits are not yours to publish). Stopping before the edit keeps the user's WIP intact, so committing or stashing it and rerunning is a clean recovery.

   **If on the default branch, or detached (`git branch --show-current` prints nothing):** create a feature branch from `HEAD` without asking — `git checkout -b <name>`, named from the plan title or work description (e.g., `feat/user-authentication`, `fix/email-validation`) — and say which branch you moved to; re-read `git branch --show-current` afterwards so the rest of the run uses the new name. Branching from `HEAD` keeps any local commits the default branch is ahead by, so the idempotency check still sees them; the shipping gate, not the branch point, decides whether they get published. If the user asked for a worktree in this session, invoke the `ce-worktree` skill instead of branching in place. Continue on the default branch only when the user explicitly said so this session.

   **If already on a feature branch:** continue on it — the branch you were invoked on is where the work belongs. The one exception is a branch whose open PR is plainly a different change from this plan (compare the PR's title and diff to the plan; a shared name or scope is enough to continue): then create the new feature branch from the remote default (`git fetch origin <default>` then `git checkout -b <name> origin/<default>`) instead of adding unrelated commits to that PR, and say so. If git refuses the switch because uncommitted files differ from that tip, stop and say so — the user's WIP stays where it is; they commit or stash it, or tell you to continue on the current branch. In Return-to-Caller Mode that stop is `status: blocked` naming the branch and the files, not a continuation on the wrong PR branch. If its name is opaque or auto-generated (for example `worktree-jolly-beaming-raven`) and you can positively confirm it was never published — `git ls-remote --exit-code --heads origin "<current_branch>"` (quote the name — branch names can carry shell metacharacters) exits 2 (no matching ref) — rename it to a name derived from the work with `git branch -m <meaningful-name>` and say so. Any other result keeps the name: exit 0 means published, and a non-zero exit other than 2 (unreachable remote, auth failure, no `origin`) means unknown, which is not a license to rename. A missing upstream is not the test either: a branch pushed without `-u` has none yet is published. Leave a published branch's name alone — a rename forks the local and remote names, so the shipping tail would push a second branch and miss any open PR, which is not a one-command undo.
