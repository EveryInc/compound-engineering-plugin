...
4. **Choose Execution Engine, then Strategy**
   ...
   **Route resolution is a mandatory pre-write gate.** Place the bounded `resolve_batch` request in effective-user-private temp. From this skill's directory, set `SKILL_DIR` to the loaded skill path and invoke `python3 -I -S "$SKILL_DIR/scripts/unit-workspace.py" resolve-routing --repo <canonical-checkout> --routing-request <request-path>` in the same shell call. This read-only command invokes co-located `resolve_batch`, reads merged global/project settings, normalizes eligible legacy `work_engine_mode` / `work_engine_preferences`, and makes no run or canonical state. Freeze its returned implementation binding. A generalized `prefer` route can use only declared candidates; `require` gaps block non-interactively.
