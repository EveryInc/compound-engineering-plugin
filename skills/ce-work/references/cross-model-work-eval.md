...
| E38 plugin-bundled reference load | Cursor loads CE Work through `--plugin-dir`; the target repository does not contain CE Work's `references/` or `scripts/` directories, and the request requires Claude Opus | Resolve required files from the loaded `SKILL.md` full path, not by globbing the target repository. If that path is unavailable, disclose the unavailable route once and continue on the current harness/session model before any external egress; otherwise load the engine and cross-model protocols and keep the required Claude route. |
