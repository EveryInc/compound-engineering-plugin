...
On native OpenCode, freeze the complete selected implementation wave through host-owned `ce_task_prepare`. Carry its process-local opaque session/role/instance-bound handle to `ce_task` only for typed `native` or `opencode` results. A typed `external` result uses the existing durable controller path from configured global/project routing and supplies only `external_comparison`; pass that JSON as `--opencode-expected-json` so controller `resolve-routing` and `init` independently resolve public config without task intent and block on source/binding mismatch. The controller derives OpenCode origin from the parsed request itself and refuses an external route when comparison data is omitted, so converted or non-native OpenCode cannot use this path. Non-OpenCode hosts require no comparison. Candidate family follows the first selected ordinal. Mixed-adapter fallback is unsupported in both directions: native OpenCode cannot transfer authoritative preflight history to a later external candidate (`MIXED_ADAPTER_CONTINUATION_UNSUPPORTED`), and the external controller cannot transfer a later OpenCode candidate back into a process-local handle. A direct-input list may continue past an unavailable external candidate only to a later in-process native candidate. Lost handles are not rehydrated. On other hosts, build one private `ce-routing/v1` `resolve_batch` request for role `ce-work.implementation-worker`, with one entry per selected implementation instance, the current harness/serving family, runtime instance metadata, the normalized task intent, and a full parent snapshot envelope only when inheriting one. A direct candidate assignment that cannot name a configured profile travels in the request's CE Work-only `implementation_intent` control field with exactly `source`, `policy`, and `candidates`; do not also send another applicable task intent. From this skill's directory, set `SKILL_DIR` to the loaded skill path and invoke `python3 -I -S "$SKILL_DIR/scripts/unit-workspace.py" resolve-routing --repo <canonical-checkout> --routing-request <private-request>` in the same shell call. The read-only gate returns one binding without creating run or canonical state.

Consume legacy work-engine values only from `resolution.compatibility`; do not inspect or manually normalize merged settings separately. The result freezes those field-level values/provenance, role instances, final binding, binding digest, and candidate `attempt_lock`s in the resolver snapshot. Reuse it for native waves. If an external adapter is selected, pass the same private request to controller `init`; its returned persisted snapshot is authoritative and must match the pre-write decision. Recovery uses that stored snapshot and recipient even when live config changes; never resolve a replacement route on resume.
...
Do not parse these files separately. `resolve_batch` reads them with generalized routing and returns the normalized decision in the same `resolution.compatibility` and snapshot. A role route wins over compatibility at one layer; owning compatibility wins over that layer's broader implementation class route. Project beats global, a narrower `ce-default` reset stops lower inheritance, legacy `prefer` appends its shipped built-in fallback explicitly, and legacy `require` has no native weakening path.

Do not put CLI commands or flags in configuration. Composer remains `{ harness: cursor, model: composer }`, while `{ harness: cursor }` means Cursor's configured default. The bare `composer` model alias normalizes to the fixed Composer-family default; it does not make a Composer model valid for another harness.
...
Traverse ordered `prefer` candidates during host-owned qualification. A candidate equivalent to the current host's default collapses to native execution rather than shelling out to self; an explicit different same-harness model remains distinct. Record every preflight-unavailable candidate in order. An external candidate qualifies only when the host can enforce the co-located Linux Landlock filesystem boundary and authenticate without placing secret bytes in the model process's readable filesystem domain. Credential-free and externally brokered routes remain eligible; a route requiring copied JSON tokens, ambient credentials, home/config roots, or another readable CLI store is unavailable before dispatch. Before external egress, retain the resolver's candidate lock and call controller `lock-attempt` with the same selected ordinal plus independently authorized recipient, intermediaries, exact material scope, restrictions, credential-minimized environment, and controller-probed confinement identity. Both locks are immutable for that unit attempt; finalization passes the self-validating resolver snapshot, resolver attempt lock, typed outcome, and complete prior receipt history rather than a caller-supplied binding.
...
| **Cross-model execution** | A resolved fixed route has a qualified, write-capable adapter, Linux Landlock confinement, and satisfies every caller restriction. Load `cross-model-execution.md` only after this engine is selected. | Availability depends on the installed target CLI, its qualified adapter, and the host confinement probe, not on native subagent tools. A same-host default request collapses to native execution. |
