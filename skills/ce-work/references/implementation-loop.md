...
**Quality commands** — When the Phase 1 config read captured an active `quality_commands` key matching the check being run (`test`, `lint`, `typecheck`, `build`), run that configured command verbatim as a single argv-style shell call and treat its exit status as the result; do not substitute, wrap, or re-derive it. When no matching key is configured, discover the command from the project's active instructions and context. Precedence when sources conflict: task instruction > session/project instructions > config. This file owns the precedence rule; other steps cite it.
