...
while (tasks remain):
  ...
  - **Before writing the first test,** re-check that this unit's named files exist on disk and that named fixtures still match the plan (`ce-plan-grounding-check` steps 2-3, scoped to this unit). A missing path or unsatisfiable fixture is a blocker, not a discovery to invent around.
