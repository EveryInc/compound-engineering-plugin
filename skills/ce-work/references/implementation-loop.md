...
while (tasks remain):
  ...
  - **If the unit's deliverable is out-of-repo state** (a console setting, DNS record, CMS object, live-system rows) there is no git-derived completion signal. Do not treat a clean tree as not-started and re-apply a non-idempotent side effect. Decide from observed desired state, not from a tracker write. Mark complete only when that state is already satisfied. Execute only when that state is positively unsatisfied and first-run or retry has positive authorization and safety. Ask or block whenever execution lacks that positive authorization or safety. Recording a new tracker status is not itself that evidence. Git completeness is not this unit's completeness. Apply this before any git-derived shortcut whenever any part of the unit's completion depends on out-of-repo state.
  - **If the unit's entire completion signal is repository-derived and that work is already present and matches the plan's intent** (files exist with the expected capability, or the unit's `Verification` criteria are already satisfied by the current code), the work has likely shipped on a prior branch or session. Verify it matches, mark the task complete, and move on. Do not silently reimplement. Git evidence does not complete a unit whose completion depends on out-of-repo state.
