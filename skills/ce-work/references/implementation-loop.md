...
while (tasks remain):
  ...
  - **If the unit's deliverable is out-of-repo state** (a console setting, DNS record, CMS object, live-system rows) there is no git-derived completion signal. Do not treat a clean tree as not-started and re-apply a non-idempotent side effect. Proceed or mark complete only from positive external-state or tracker evidence that the prior mutation already happened. Ask or block when completion is unknown and retry is unsafe. Recording a new tracker status is not itself that evidence. Git completeness is not this unit's completeness.
