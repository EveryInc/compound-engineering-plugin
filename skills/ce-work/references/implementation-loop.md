...
while (tasks remain):
  ...
  - **If the unit's deliverable is out-of-repo state** (a console setting, DNS record, CMS object, live-system rows) there is no git-derived completion signal. Do not treat a clean tree as not-started and re-apply a non-idempotent side effect. Decide from observed desired state, not from a tracker write: mark complete when that state is already satisfied; execute when it is positively unsatisfied and first-run or retry is authorized and safe; ask or block when the state is unknown and execution is unsafe. Recording a new tracker status is not itself that evidence. Git completeness is not this unit's completeness.
