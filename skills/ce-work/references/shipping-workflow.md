...
2. **Commit and Create Pull Request**
   ...
   **Project-defined shipping process wins.** If the project's active instructions already in your context define how work is committed, pushed, or turned into a PR — a named skill or command (e.g. a `/create-pr` skill), a stacking tool, or documented steps — use that process instead of the default below, and hand it the same context this step would hand `ce-commit-push-pr` (plan summary, testing notes, evidence, review receipt, Known Residuals, and the `exclude:` paths). Presence of a skill directory alone is not a directive; the instruction has to say so. The ship-handoff gate and the publish rule above hold whichever process runs; if the project process cannot take a piece of the context, state it in the shipping summary instead of dropping it. Precedence: the user's stated preference for this run > the project-defined process > the default. Absent a project-defined process:
