...
3. **Code Review**
   ...
   **Completion gate (standalone shipping).** This shipping tail is **not done** until exactly one of: (1) a **review receipt** from an actual `ce-code-review` invocation — `mode:agent` JSON with `status` plus `artifact_path` or `run_id`, or markdown containing Actionable Findings, Coverage, and Verdict — or (2) an **explicit skip phrase** in the shipping summary: `Code review: skipped (mechanical diff)` or `Code review: skipped (ce-code-review unavailable)`, each with a one-line reason. Silent omit is invalid. **Never substitute** mental self-review, "external / prior findings already applied," ad-hoc skimming, or harness-native `/review` alone when `ce-code-review` can load.

   **Skip dedicated review only for a purely mechanical diff** — formatting, dependency-version bumps, lint-only fixes, generated artifacts (the same class step 2 skips for simplify). **Not mechanical:** multi-file or behavior-bearing edits, control-flow / error-class / tests-for-behavior changes, or applying external or prior review findings. Note the exact skip phrase above. Everything else gets reviewed.

   **Review is not fix — two steps:**

   **3a. Review (read-only).** Invoke `ce-code-review` through the host's normal skill-invocation mechanism with `mode:agent` (add `plan:<path>` when known; `base:<ref>` when the diff base is resolved). Pass **`depth:full`** when the plan, the task, or the user explicitly asked for a full / deep / thorough review — that is the one escalation signal `ce-code-review` cannot infer from the diff alone. Do not pass `mode:autofix`. Parse the JSON and retain the receipt (`status`, `artifact_path` / `run_id`).
...
2. **Commit and Create Pull Request**
   ...
   **Ship-handoff gate.** Before loading `ce-commit-push-pr` or `ce-commit`, confirm the Phase 3 code-review completion gate is satisfied (review receipt **or** exact skip phrase). If neither is present, stop and run step 3 (or write the legitimate skip) — do not push "and review later." Pass the receipt summary (status + `artifact_path`/`run_id`) or the skip phrase into the shipping summary and PR-description context the same way Known Residuals already travel.
...
- [ ] Code review completion gate: receipt (`status` + `artifact_path`/`run_id` or markdown Actionable/Coverage/Verdict) **or** exact skip phrase (`Code review: skipped (mechanical diff)` / `Code review: skipped (ce-code-review unavailable)`); residuals handled via the Residual Work Gate
- [ ] Ship-handoff gate passed before `ce-commit-push-pr` / `ce-commit` (receipt or skip phrase in shipping context)
...
**Completion gate:** shipping is not done without a review receipt or an exact skip phrase. **Skip** only for a purely mechanical diff (formatting, dep-bumps, lint-only, generated) — not for applying external findings or behavior-bearing multi-file work. Everything else is reviewed.
...
**If `ce-code-review` can't run** (no subagent dispatch): interactive → harness-native review if present, fix inline; non-interactive → exact unavailable skip phrase + manual diff scan in Final Validation. Never silently ship a non-mechanical change unreviewed.
