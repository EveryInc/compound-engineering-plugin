...
3. **Code Review**
   ...
   **3a. Review (read-only).** Invoke `ce-code-review` with `mode:agent` (add `plan:<path>` when known; `base:<ref>` when the diff base is resolved). Pass **`depth:full`** when the plan, the task, or the user explicitly asked for a full / deep / thorough review — that is the one escalation signal `ce-code-review` cannot infer from the diff alone. Do not pass `mode:autofix`. Parse the JSON.
   ...
   Parse and retain its per-lane `critique_receipts` summary. Each entry carries `lane`, caller-owned `required`, `receipt_version`, `model_requested`, `model_actual`, `receipt_source`, `receipt_status`, and `critique_status`. A lane passes only with `receipt_version: critique-author/v1`, `critique_status: usable`, and `receipt_status: matched`. Legacy/missing/tampered entries are unverified; never infer actual author from the request. Producer `required: false` cannot downgrade a consumer-declared required lane. Preserve selected and valid sibling lanes.
