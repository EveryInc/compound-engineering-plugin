...
Write the trajectory under the CE artifact root's `solutions/{category}/` tree — the same location `/ce-work` already resolves for learnings — using the project's existing solution-doc conventions. If no such directory exists, write to the project root as a markdown file and let the engineer decide where to put it.
...
- The learnings-researcher review persona searches the `solutions/` tree by frontmatter metadata — the `trajectory: true` field lets it specifically find execution trajectories
- Future `/ce-work` sessions benefit when the learnings-researcher surfaces a relevant trajectory before implementation starts
