...
| [`ce-plan`](./ce-plan.md), [`ce-brainstorm`](./ce-brainstorm.md) | `plan_effort`, `brainstorm_effort` | Reasoning effort for the elevated step: `low`, `medium`, `high`, `xhigh`, or `max`. Unset keeps the existing `high` default. An unrecognized value is rejected and the run skips elevation rather than dispatching at a different effort than configured. Only takes effect on the Claude CLI route — the native-dispatch route has no per-call effort override. |
