...
8. Ship: the goal is the remaining work committed, pushed, and in an open PR whose URL you hold. If the project's active instructions name a process that owns that handoff (a named skill or command, a stacking tool, or documented steps — not commit or PR-title conventions, which the default honors; a skill directory alone is not a directive), run it non-interactively with the same plan path and context below. It is done only when it yields the PR URL; if it cannot run headlessly, is unavailable, or ends without that URL, stop as **blocked** naming the process — do not fall through to the default or to step 9. Otherwise: Invoke the `ce-commit-push-pr` skill with `mode:pipeline branding:on`. Thread the recorded plan path from step 1 into the invocation, along with any proceeded-and-flagged `settled_decision_conflicts` entries from step 2, so the PR body's settled-decisions provenance line and its proceed-under-flag clause can fire.
