...
Run the steps below in order; plan before implementing.
...
1. Invoke the `ce-plan` skill with the sanitized feature request prepared above (or the unchanged arguments you were invoked with when no routing directive was present). When a planning-stage directive resolved, prefix the invocation with its `plan_model:<alias>` carrier — structured routing data beside the request, never woven into it — so `ce-plan`'s model elevation authors the plan on the chosen model even in pipeline mode.
   ...
   Read the plan metadata before continuing. If the plan has `artifact_contract: ce-unified-plan/v1`, proceed only when it has `artifact_readiness: implementation-ready` and `execution: code`. Stop the pipeline for `artifact_readiness: requirements-only`, `execution: knowledge-work`, or any unrecognized readiness value. LFG never launches `/goal` directly; when goal-mode or dynamic workflows are appropriate, `ce-work` owns that implementation engine choice and must return control to LFG afterward.
...
4. Invoke the `ce-code-review` skill with `mode:agent plan:<plan-path-from-step-1>`.
   ...
   `mode:agent` reports only — it never edits the tree; LFG applies the eligible findings in step 5.
