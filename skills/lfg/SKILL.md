...
8. If the project's active instructions name a process that owns the shipping handoff — committing, pushing, and opening the PR (a named skill or command, a stacking tool, or documented steps; not mere commit or PR-title conventions, which the default honors) — run that non-interactively instead, passing the same plan path and context below and recording the resulting PR URL; presence of a skill directory alone is not a directive. Otherwise: Invoke the `ce-commit-push-pr` skill with `mode:pipeline branding:on`. Thread the recorded plan path from step 1 into the invocation, along with any proceeded-and-flagged `settled_decision_conflicts` entries from step 2, so the PR body's settled-decisions provenance line and its proceed-under-flag clause can fire.
