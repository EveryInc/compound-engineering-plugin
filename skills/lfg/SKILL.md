...
9. **Watch the PR to CI-decided via `ce-babysit-pr`** (only when an open PR exists for the current branch)
   ...
   **Skip when step 8 already handed off a stack-aware babysit.** If step 8's `ce-commit-push-pr` completed a stack-mode submit and started `ce-babysit-pr` on the bottom open non-draft PR with `posture:stack-ready` or `posture:stack-land`, do **not** invoke babysit again here — a second bare `mode:pipeline` call (no posture, current-branch URL) can supersede that stack-aware run as target-only or watch the wrong layer. Collect the structured result already returned from that handoff and proceed to step 10.
