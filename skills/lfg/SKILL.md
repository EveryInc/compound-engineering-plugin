...
4. Invoke the `ce-code-review` skill with `mode:agent plan:<plan-path-from-step-1>`.
   ...
   Pass the plan file path from step 1 so ce-code-review can verify requirements completeness. Read the **Actionable Findings** summary and the per-lane `critique_receipts` the skill emits. Each receipt entry carries `lane`, LFG/caller-owned `required`, `receipt_version`, `model_requested`, `model_actual`, `receipt_source`, `receipt_status`, and `critique_status`. A critique lane passes only with `receipt_version: critique-author/v1`, `critique_status: usable`, and `receipt_status: matched`. Legacy/missing/tampered entries are unverified; never copy requested model into actual author. Producer `required: false` cannot downgrade a caller-declared required lane, and selected and valid sibling lanes remain visible.
