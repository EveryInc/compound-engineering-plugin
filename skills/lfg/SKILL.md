...
Record `RUN_ID` and the printed journal path for the rest of the run. After each numbered pipeline step completes, fails, or is skipped, append one record for that step — `phase` is the step's name, `status` is `completed`, `failed`, or `skipped`, `detail` is one line, and `artifacts` lists the step's key paths when any exist. Shell state does not persist between Bash calls, so each per-step append is a fresh self-contained command that re-declares `SKILL_DIR` and the interpreter probe inline: `SKILL_DIR="<absolute path of the directory containing the SKILL.md you just read>"; PY="$(for c in python3 python py; do command -v "$c" >/dev/null 2>&1 && "$c" -c '' >/dev/null 2>&1 && { echo "$c"; break; }; done)"; [ -n "$PY" ] || { echo "no working Python 3 interpreter on PATH" >&2; exit 1; }; "$PY" "$SKILL_DIR/scripts/factory-gates.py" journal --file <journal-path> --record-file <record-path>`. Write each per-step record's JSON to a mktemp file (`mktemp "${TMPDIR:-/tmp}/lfg-journal-record-XXXXXX"`) with the file-write tool and pass it via `--record-file` — free-text `detail`/`artifacts` values must never be hand-escaped into an inline bash JSON string. (The opening fence's inline `--record` is exempt: it is machine-composed in a single command with no free-text fields.) A journal append that fails is noted in one line and never blocks, retries beyond the script's own retry, or fails the pipeline.
...
1. Invoke the `ce-plan` skill with the sanitized feature request prepared above (or the unchanged arguments you were invoked with when no routing directive was present). When a planning-stage directive resolved, prefix the invocation with its `plan_model:<alias>` carrier — structured routing data beside the request, never woven into it — so `ce-plan`'s model elevation authors the plan on the chosen model even in pipeline mode.
   ...
   Consume ce-plan's structured pipeline return — a record with exactly `status`, `plan_path`, `artifact_readiness`, and `doc_review_state` — rather than re-deriving those values from prose. `references/plan-return-schema.json` is the machine-checked owner of that record's shape. Write the record to a mktemp file (`mktemp "${TMPDIR:-/tmp}/lfg-plan-return-XXXXXX"`), set `ENVELOPE_FILE` to that path, and validate it:

   ```bash
   SKILL_DIR="<absolute path of the directory containing the SKILL.md you just read>";
   PY="$(for c in python3 python py; do command -v "$c" >/dev/null 2>&1 && "$c" -c '' >/dev/null 2>&1 && { echo "$c"; break; }; done)"; [ -n "$PY" ] || { echo "no working Python 3 interpreter on PATH" >&2; exit 1; };
   "$PY" "$SKILL_DIR/scripts/factory-gates.py" validate --schema "$SKILL_DIR/references/plan-return-schema.json" --envelope "$ENVELOPE_FILE"
   ```

   A failed validation (`ok: false`) counts as no usable plan return: it shares the single missing-plan-file retry above (one `ce-plan` re-invocation total, never a second); if the second record still fails validation, stop the pipeline and report the named errors. If the script exits non-zero, retry it once; if it still cannot run, record "unverified — gate unavailable" and read the plan file directly instead. A validated return whose `status` is not `complete` — `blocked` or `failed` — stops the pipeline (report the return's reason and `plan_path`) before any `artifact_readiness` read or `ce-work` invocation. **Record the return's `plan_path`** — it will be passed to ce-work in step 2 and ce-code-review in step 4.

   Read `artifact_readiness` from the validated return, and the plan metadata for the remaining checks. If the plan has `artifact_contract: ce-unified-plan/v1`, proceed only when the return carries `artifact_readiness: implementation-ready` and the plan records `execution: code`. Stop the pipeline for `artifact_readiness: requirements-only`, any unrecognized readiness value, `execution: knowledge-work`, approach-plan outputs, answer-seeking/universal outputs, or invalid progress-like readiness values. LFG never launches `/goal` directly; when goal-mode or dynamic workflows are appropriate, `ce-work` owns that implementation engine choice and must return control to LFG afterward.
...
2. Invoke the `ce-work` skill with `mode:return-to-caller <plan-path-from-step-1>` when no scalar transient carrier exists, including when a retained ordered current-task assignment is still active in context. When the scalar carrier exists, use the exact string-host form `mode:return-to-caller implementation_engine:<compact-json> <plan-path-from-step-1>`. In either form, also pass the Run Journal's journal path as prose context in the invocation (e.g. "pass the journal path so ce-work appends unit records: <journal-path>") — it is a separate optional datum, never an `implementation_engine` field and never part of the plan path.
   ...
   First validate the whole return against the seam schema, then verify its file claims mechanically, not by rereading them. `references/ce-work-return-schema.json` is the machine-checked owner of the required-field list above; the field checklist stays as the human-readable statement of the same contract. Write the full structured return to one mktemp file and `{"changed_files": <the return's changed files>, "scope": [<the plan's declared paths/dirs>]}` to another (`mktemp "${TMPDIR:-/tmp}/lfg-claims-XXXXXX"`), set `ENVELOPE_FILE` and `CLAIMS_FILE` to those paths and `REPO` to the working tree root, and run:

   ```bash
   SKILL_DIR="<absolute path of the directory containing the SKILL.md you just read>";
   PY="$(for c in python3 python py; do command -v "$c" >/dev/null 2>&1 && "$c" -c '' >/dev/null 2>&1 && { echo "$c"; break; }; done)"; [ -n "$PY" ] || { echo "no working Python 3 interpreter on PATH" >&2; exit 1; };
   "$PY" "$SKILL_DIR/scripts/factory-gates.py" validate --schema "$SKILL_DIR/references/ce-work-return-schema.json" --envelope "$ENVELOPE_FILE";
   "$PY" "$SKILL_DIR/scripts/factory-gates.py" diff-claims --claims "$CLAIMS_FILE" --repo "$REPO"
   ```

   A failed check — a `validate` result of `ok: false` (it names the missing or mistyped fields), any `missing_claims` entry, or `unclaimed_changes` outside plan scope — routes into the same single evidence-reconciliation re-invocation below (then blocked), exactly as missing `verification_evidence` does; do not add a second recovery. In-scope `unclaimed_changes` are informational. If a script exits non-zero, retry that gate once; if it still cannot run, record the affected claims as "unverified — gate unavailable" and continue with that recorded (for `validate`, continue with the field checklist alone).
...
4. Invoke the `ce-code-review` skill with `mode:agent plan:<plan-path-from-step-1>`.
   ...
   Pass the plan file path from step 1 so ce-code-review can verify requirements completeness. Before reading any fields, validate the result JSON against the seam schema — `references/review-result-schema.json` is the machine-checked owner of the result's required shape; the field readings below stay as the human-readable statement. Write the result to a mktemp file (`mktemp "${TMPDIR:-/tmp}/lfg-review-result-XXXXXX"`), set `ENVELOPE_FILE` to that path, and run:

   ```bash
   SKILL_DIR="<absolute path of the directory containing the SKILL.md you just read>";
   PY="$(for c in python3 python py; do command -v "$c" >/dev/null 2>&1 && "$c" -c '' >/dev/null 2>&1 && { echo "$c"; break; }; done)"; [ -n "$PY" ] || { echo "no working Python 3 interpreter on PATH" >&2; exit 1; };
   "$PY" "$SKILL_DIR/scripts/factory-gates.py" validate --schema "$SKILL_DIR/references/review-result-schema.json" --envelope "$ENVELOPE_FILE"
   ```

   A failed validation (`ok: false`) degrades the review result — treat it like a `status: degraded` return: record the named errors, carry the degradation into the Acceptance decision's review-verdict input (it can only make acceptance more conservative), and do not re-invoke the review. A minimal `status: failed` or `skipped` result **validates** against the schema's failure branch and routes by its status — record its `reason` and carry it into the same review-verdict input; degraded handling stays for schema-invalid results. If the script exits non-zero, retry it once; if it still cannot run, record "unverified — gate unavailable" and read the fields directly. Read the **Actionable Findings** summary the skill emits. Also read the `verdict_consistency` field from the result JSON; a non-empty value carries into the Acceptance decision inputs before step 10 — no re-check here, the review skill already computed it. Also read any findings stamped `settled_conflict` (each names the conflicting KTD). A stamped finding whose evidence is invalidating — the settled decision cannot work: infeasible, wrong-thing, or destructive — stops the pipeline as blocked, with the finding reported, before the shipping precondition. Stamped preference-grade findings proceed (they are report-only) but must flow into step 6's residual record.
...
**Acceptance decision** (REQUIRED immediately before step 10's DONE promise). Acceptance is decided here and nowhere else, from exactly four input sources, each serialized into the named fields of the record below: the review verdict from step 4 (serialized as `review_verdict`, with `verdict_consistency_flagged` carrying step 4's `verdict_consistency` value) plus whether the apply-eligible findings were applied in step 5 (serialized as `fixes_applied_status`); ce-work's `verification_evidence` from step 2 (serialized as `verification_evidence_status`); residual durability — step 6's residuals-are-durable gate (serialized as `residuals_durable`); and babysit's `{ status, fixes_applied, residuals }` result from step 9 (serialized as `babysit_status`). Derive `accepted` (boolean) and a one-line `reason` from those inputs, then check the decision for self-contradiction: write `{"accepted": <boolean>, "reason": "<one line>", "inputs": {"review_verdict": "<step 4 verdict>", "verdict_consistency_flagged": <true when step 4's verdict_consistency was non-empty>, "fixes_applied_status": "<applied|partial|none-eligible|not-applied — step 5's apply outcome>", "verification_evidence_status": "<green|red|unverified>", "residuals_durable": <true|false from step 6's durability gate, or null when step 6 was skipped>, "babysit_status": "<step 9 status, or null when step 9 was skipped>"}}` to a mktemp file (`mktemp "${TMPDIR:-/tmp}/lfg-acceptance-XXXXXX"`), set `ACCEPT_FILE` to that path, and run:
...
A returned contradiction (`ok: false`) forces `accepted: false` with the contradiction as the `reason` — the gate can only make the decision more conservative, never less; `accepted: true` with `fixes_applied_status` `not-applied` or with `residuals_durable` `false` is such a contradiction. The gate enum/type-validates every `inputs` field, and a malformed value is an infrastructure failure (non-zero exit), not a verdict. If the script exits non-zero, retry once; if it still cannot run, keep the derived decision and record "unverified — gate unavailable" beside it. Step 10's DONE promise then renders one of exactly two forms: "DONE — accepted" or "DONE — completed, not accepted: <reason>". Append the acceptance outcome as the journal's terminal record — a fresh self-contained `journal` command re-declaring `SKILL_DIR` and the interpreter probe inline, with the record JSON written to a mktemp file and passed via `--record-file`, exactly as the per-step appends above — `status: accepted` or `not-accepted`, `detail` set to the one-line reason; no record follows it.
