...
The fix is the only thing this run was asked to publish. `pre_fix_scope` says what else the tree and branch already carried. Every later step that stages, commits, or pushes stays inside the fix-owned files plus what the run itself added (review fixes, a captured learning): pass `pre_fix_scope.dirty_files` as `exclude:<paths>` to `ce-commit-push-pr`, and on the no-remote path commit those files by name. When `pre_fix_scope.unpushed_commits` is greater than zero, the branch already carries commits no remote has and the user did not offer them in this request; do not push and do not open a PR. Commit the fix locally, report the held-back commits and files, and stop, as `ce-debug`'s own handoff does on a branch carrying unoffered work.
