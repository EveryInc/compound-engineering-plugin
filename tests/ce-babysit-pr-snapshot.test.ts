...
function pySnapshot(expr: string): unknown {
  const r = spawnSync(
    "python3",
    [
      "-c",
...
function eyesReactionReactors(pages: unknown): Array<{ identity: string; login: string | null }> {
  return pySnapshot(
    `print(json.dumps(m._eyes_reaction_reactors(json.loads(${JSON.stringify(JSON.stringify(pages))}))))`,
  ) as Array<{ identity: string; login: string | null }>
}

function checkRunsByApp(pages: unknown): Record<string, { work: string; checks: unknown[] }> {
  return pySnapshot(
    `print(json.dumps(m._check_runs_by_app(json.loads(${JSON.stringify(JSON.stringify(pages))}))))`,
  ) as Record<string, { work: string; checks: unknown[] }>
}

function reviewSignalAttribution(
  reactors: unknown,
  checkApps: unknown,
): Array<{ identity: string; login: string | null; app: string | null; work: string }> {
  return pySnapshot(
    `print(json.dumps(m._review_signal_attribution(` +
      `json.loads(${JSON.stringify(JSON.stringify(reactors))}), ` +
      `json.loads(${JSON.stringify(JSON.stringify(checkApps))}))))`,
  ) as Array<{ identity: string; login: string | null; app: string | null; work: string }>
}

function reviewSignalWorkTerminal(attribution: unknown): boolean {
  return pySnapshot(
    `print(json.dumps(m._review_signal_work_terminal(json.loads(${JSON.stringify(JSON.stringify(attribution))}))))`,
  ) as boolean
}

function headCheckAppsCommand(
  head: string,
  payloadValue: unknown = [[{ name: "n", status: "completed", conclusion: "success", app: { slug: "cursor" } }]],
  returncode = 0,
): { argv: string[]; result: unknown } {
  ...
  return pySnapshot(
    `captured = []\n` +
      ...
      `    return R()\n` +
      `m._run = fake_run\n` +
      `out = m.fetch_head_check_apps('o', 'r', ${JSON.stringify(head)})\n` +
      `print(json.dumps({'argv': captured[0], 'result': out}))`,
  ) as { argv: string[]; result: unknown }
}

function fetchProbeCalls(reactors: unknown): { urls: string[]; work_terminal: boolean } {
  const checkRuns = JSON.stringify([{
    total_count: 1,
    check_runs: [{ name: "Cursor Security Agent: Security Reviewer", status: "completed", conclusion: "success", app: { slug: "cursor" } }],
  }])
  const view = JSON.stringify({
    state: "OPEN", isDraft: false, mergeable: "MERGEABLE", mergeStateStatus: "CLEAN",
    reviewDecision: "APPROVED", headRefOid: "a".repeat(40), baseRefOid: "b".repeat(40),
    baseRefName: "main", headRefName: "feat/x", url: "https://github.com/o/r/pull/1", number: 1,
    statusCheckRollup: [], author: { login: "someone" }, comments: [], reviews: [],
  })
  return pySnapshot(
    `urls = []\n` +
      `class R:\n` +
      ...
      `        urls.append('check-runs')\n` +
      `        return R(${JSON.stringify(checkRuns)})\n` +
      `    return R('{}')\n` +
      `m._run = fake_run\n` +
      `m._run_checked = lambda cmd, label: R(${JSON.stringify(view)})\n` +
      `m.fetch_eyes_reactors = lambda *a: json.loads(${JSON.stringify(JSON.stringify(reactors))})\n` +
      `m.fetch_pr_merge_identity = lambda *a, **k: None\n` +
      `m.fetch_base_ref = lambda *a, **k: {'identity': 'current'}\n` +
      `m.fetch_pr_chain = lambda *a, **k: m._default_pr_chain()\n` +
      `m.fetch_threads = lambda *a, **k: []\n` +
      `m.fetch_awaiting_approval = lambda *a, **k: 0\n` +
      `out = m.fetch(1, 'o/r')\n` +
      `print(json.dumps({'urls': urls, 'work_terminal': out['review_signal_work_terminal']}))`,
  ) as { urls: string[]; work_terminal: boolean }
}
...
`
  ...
  test("snapshot: an eyes reactor carries its login so its work can be attributed", () => {
    expect(eyesReactionReactors([[{ content: "eyes", user: { login: "cursor[bot]", node_id: "BOT_cursor" } }]])).toEqual([
      { identity: "BOT_cursor", login: "cursor[bot]" },
    ])
    // A deleted user still yields an identity from the reaction itself, with no login to attribute.
    expect(eyesReactionReactors([[{ content: "eyes", node_id: "REACT_1" }]])).toEqual([
      { identity: "REACT_1", login: null },
    ])
    // The identity view stays exactly what the persisted lifecycle has always compared.
    expect(eyesReactionIdentities([[{ content: "eyes", user: { login: "cursor[bot]", node_id: "BOT_cursor" } }]])).toEqual(["BOT_cursor"])
  })

  test("snapshot: an app's current-head work is terminal only when every one of its checks has finished", () => {
    const runs = [[
      { name: "Cursor Security Agent: Security Reviewer", status: "completed", conclusion: "success", app: { slug: "cursor" } },
      { name: "test", status: "completed", conclusion: "success", app: { slug: "github-actions" } },
    ]]
    expect(checkRunsByApp(runs).cursor.work).toBe("terminal")
    expect(checkRunsByApp(runs).cursor.checks).toEqual([
      { name: "Cursor Security Agent: Security Reviewer", status: "COMPLETED", conclusion: "SUCCESS" },
    ])
    // A terminal FAILURE is still terminal work — it stays a CI blocker via has_failing_checks,
    // and must not masquerade as a review still running.
    expect(checkRunsByApp([[{ name: "r", status: "completed", conclusion: "failure", app: { slug: "cursor" } }]]).cursor.work)
      .toBe("terminal")
    // One unfinished check from the same app keeps the whole app's work running: a repo that runs
    // both Cursor products under the `cursor` app must not clear while the slower one is going.
    expect(checkRunsByApp([[
      { name: "Cursor Security Agent: Security Reviewer", status: "completed", conclusion: "success", app: { slug: "cursor" } },
      { name: "Cursor Bugbot", status: "in_progress", conclusion: null, app: { slug: "cursor" } },
    ]]).cursor.work).toBe("running")
  })

  test("snapshot: attribution correlates a bot reactor with its own app, and never guesses for anyone else", () => {
    const cursorApp = { cursor: { work: "terminal", checks: [{ name: "Cursor Security Agent: Security Reviewer", status: "COMPLETED", conclusion: "SUCCESS" }] } }
    const reactor = [{ identity: "BOT_cursor", login: "cursor[bot]" }]

    expect(reviewSignalAttribution(reactor, cursorApp)).toEqual([
      { identity: "BOT_cursor", login: "cursor[bot]", app: "cursor", work: "terminal",
        checks: [{ name: "Cursor Security Agent: Security Reviewer", status: "COMPLETED", conclusion: "SUCCESS" }] },
    ])
    // A bot whose app has no check on this head is unattributable — the conservative read stands.
    expect(reviewSignalAttribution(reactor, { "github-actions": { work: "terminal", checks: [] } })[0].work).toBe("none")
    // A human reactor has no app to finish.
    expect(reviewSignalAttribution([{ identity: "U_person", login: "someone" }], cursorApp)[0])
      .toEqual({ identity: "U_person", login: "someone", app: null, work: "none", checks: [] })
    // An unreadable check-run probe is unknown, never "finished".
    expect(reviewSignalAttribution(reactor, null)[0].work).toBe("unknown")
  })

  test("snapshot: every reactor's work must be terminal before the lifecycle reads as finished", () => {
    const done = { identity: "BOT_cursor", login: "cursor[bot]", app: "cursor", work: "terminal", checks: [] }
    expect(reviewSignalWorkTerminal([done])).toBe(true)
    expect(reviewSignalWorkTerminal([done, { identity: "U_person", login: "someone", app: null, work: "none", checks: [] }]))
      .toBe(false)
    expect(reviewSignalWorkTerminal([{ ...done, work: "running" }])).toBe(false)
    expect(reviewSignalWorkTerminal([{ ...done, work: "unknown" }])).toBe(false)
    // No reactor at all is not a finished lifecycle; review_in_progress is already false there.
    expect(reviewSignalWorkTerminal([])).toBe(false)
  })

  test("snapshot: check runs are read from the current head, so an older head's work cannot attribute the reaction", () => {
    // The real endpoint returns one {total_count, check_runs} object per page; --slurp yields a
    // list of those objects, and a run can be spread across pages.
    const probe = headCheckAppsCommand("deadbeefdeadbeefdeadbeefdeadbeefdeadbeef", [
      { total_count: 2, check_runs: [{ name: "Cursor Security Agent: Security Reviewer", status: "completed", conclusion: "success", app: { slug: "cursor" } }] },
      { total_count: 2, check_runs: [{ name: "Cursor Bugbot", status: "in_progress", conclusion: null, app: { slug: "cursor" } }] },
    ])
    expect(probe.argv.some((a) => a.includes("commits/deadbeefdeadbeefdeadbeefdeadbeefdeadbeef/check-runs"))).toBe(true)
    // The unfinished run on the second page must still hold the app open.
  ...
  test("snapshot: a probe that cannot answer reads unknown, never finished", () => {
    const head = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef"
    // A failed gh call, unparseable output, and a payload that is not a list of pages.
    expect(headCheckAppsCommand(head, [[]], 1).result).toBeNull()
    expect(headCheckAppsCommand(head, "not json").result).toBeNull()
    expect(headCheckAppsCommand(head, { check_runs: [] }).result).toBeNull()
    // A field in a shape the grouping cannot read makes the whole payload untrustworthy: skipping
    // that run alone would hide a check still going and read its app as finished.
    expect(headCheckAppsCommand(head, [[{ name: "n", status: "completed", conclusion: "success", app: "cursor" }]]).result).toBeNull()
    expect(headCheckAppsCommand(head, [[{ name: "n", status: 7, conclusion: null, app: { slug: "cursor" } }]]).result).toBeNull()
    // A run with no app at all is ordinary (a legacy commit status), not malformed.
    expect(headCheckAppsCommand(head, [[{ name: "legacy", status: "completed", conclusion: "success" }]]).result).toEqual({})
  })

  test("snapshot: the extra check-runs probe fires only when a bot is holding the signal", () => {
    const bot = fetchProbeCalls([{ identity: "BOT_cursor", login: "cursor[bot]" }])
    expect(bot.urls).toEqual(["check-runs"])
    expect(bot.work_terminal).toBe(true)

    // A human reactor cannot own an app's checks, so the PR pays nothing for the probe — and the
    // signal keeps blocking exactly as it did before this change.
    const human = fetchProbeCalls([{ identity: "U_person", login: "someone" }])
    expect(human.urls).toEqual([])
    expect(human.work_terminal).toBe(false)

    const none = fetchProbeCalls([])
    expect(none.urls).toEqual([])
    expect(none.work_terminal).toBe(false)
  })

  test("snapshot: a reactor's work going terminal is movement, so the settle clock restarts", () => {
    const green = {
      ...FAILING,
      merge_state_status: "CLEAN",
      review_decision: "APPROVED",
      ...
      review_in_progress: true,
      review_signal_count: 1,
      review_signal_identities: ["BOT_cursor"],
    }
    const entry = (work: string) => ({ identity: "BOT_cursor", login: "cursor[bot]", app: "cursor", work, checks: [] })
    const state = path.join(dir, "rswt")

    snapshot(state, fetchFile(dir, "rswt-running.json", { ...green, review_signal_attribution: [entry("running")] }))
    const statePath = path.join(state, "state.json")
    const settled = JSON.parse(readFileSync(statePath, "utf8"))
    settled.last_change_at = "2026-07-17T12:00:00+00:00"
    writeFileSync(statePath, JSON.stringify(settled))

    // The reactor finishing is review movement even when the checks payload has not moved: the
    // two come from different endpoints, so the settle clock cannot ride on that coincidence.
    const finished = snapshot(state, fetchFile(dir, "rswt-terminal.json", { ...green, review_signal_attribution: [entry("terminal")] }))
    expect(finished.review_signal_work_terminal).toBe(true)
    expect(finished.changed_this_tick).toBe(true)
    expect(finished.quiet_seconds).toBeLessThan(2)
  })

  test("snapshot: attribution only speaks for the reactors currently holding the signal", () => {
    const green = {
      ...FAILING,
      merge_state_status: "CLEAN",
      review_decision: "APPROVED",
    ...
    const terminal = { identity: "BOT_cursor", login: "cursor[bot]", app: "cursor", work: "terminal", checks: [] }

    const attributed = snapshot(path.join(dir, "rsa1"), fetchFile(dir, "rsa1.json", {
      ...green, review_signal_attribution: [terminal],
    }))
    expect(attributed.review_signal_work_terminal).toBe(true)

    // A finished reactor that is no longer holding the signal cannot release it for whoever is.
    const stale = snapshot(path.join(dir, "rsa2"), fetchFile(dir, "rsa2.json", {
      ...green, review_signal_identities: ["BOT_other"],
      review_signal_attribution: [terminal],
    }))
    expect(stale.review_signal_attribution).toEqual([])
    expect(stale.review_signal_work_terminal).toBe(false)

    // A payload that describes only some of the current reactors leaves the rest unaccounted for.
    const partial = snapshot(path.join(dir, "rsa3"), fetchFile(dir, "rsa3.json", {
      ...green, review_signal_count: 2, review_signal_identities: ["BOT_cursor", "U_person"],
      review_signal_attribution: [terminal],
    }))
    expect(partial.review_signal_work_terminal).toBe(false)

    // Attribution is matched by identity SET, not by count: a duplicated finished entry must not
    // fill the slot of a reactor nobody described, who may still be working.
    const duplicated = snapshot(path.join(dir, "rsa5"), fetchFile(dir, "rsa5.json", {
      ...green, review_signal_count: 2, review_signal_identities: ["BOT_cursor", "U_person"],
      review_signal_attribution: [terminal, terminal],
    }))
    expect(duplicated.review_signal_work_terminal).toBe(false)

    // A fetch payload predating attribution keeps the conservative pre-#1606 behavior.
    const legacy = snapshot(path.join(dir, "rsa4"), fetchFile(dir, "rsa4.json", green))
    expect(legacy.review_signal_attribution).toEqual([])
    expect(legacy.review_signal_work_terminal).toBe(false)
  })

  test("watch: a leftover eyes reaction whose reactor has finished its work no longer imposes the 900s stale-review floor", () => {
    const base = {
      pr_state: "OPEN",
      counts: { threads: 0, ci: 0, comments: 0 },
      stack_blocker: null,
    ...
    // Before the fix this returned null until 900 quiet seconds elapsed.
    expect(wakeReason({ ...base, review_signal_work_terminal: true }, 300)).toBe("merge-ready")
    // A reactor still working, unattributable, or unknown keeps the floor exactly as it was.
