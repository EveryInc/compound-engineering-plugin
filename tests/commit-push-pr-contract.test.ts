...
describe("PR concept teaching contract", () => {
  ...
  test("opt-in stack mode constructs or submits a stack and hands off with posture", async () => {
    ...
    expect(submit).toMatch(/already a managed stack member.+do \*\*not\*\* `gh stack init`/is)
    ...
    expect(submit).toMatch(/never substitute the repo default branch when a parent was named/i)
    expect(submit).toMatch(/soft.{0,120}refuse the stack.+required intent is not refusable/is)
