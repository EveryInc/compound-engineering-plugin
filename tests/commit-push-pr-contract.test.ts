...
describe("PR concept teaching contract", () => {
  ...
  test("opt-in stack mode constructs or submits a stack and hands off with posture", async () => {
    ...
    expect(submit).toMatch(/\*\*Managed member\*\*.{0,160}Not `gh stack init`/is)
    expect(submit).toContain('gh stack link "<stack-number>" "<your-branch>"')
    expect(submit).toMatch(/not `gh stack add` \(creates a \*new\* top branch/is)
    expect(submit).toContain('gh stack init --base "<parent-branch>" "<bottom-branch>"')
    expect(submit).toMatch(/parent's head tip is `<base>` for every construction path.{0,160}branch-creation\.md` must not be followed/is)
