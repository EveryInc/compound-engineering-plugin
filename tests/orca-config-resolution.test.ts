...
describe("CE-Orca canonical configuration resolution", () => {
  ...
  test("serializes only allowlisted identities and target data, never credentials", async () => {
    ...
    expect(resolved.identities).toMatchObject({ ceVersion: "3.21.1", integrationVersion: "3.21.1-orca.3", registryVersion: expect.any(String), protocolVersion: "orca.local-protocol/v1", requestVersion: "orca.execution-config/v1" })
