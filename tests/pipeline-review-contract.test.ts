...
describe("ce-work testing evidence contract", () => {
  test("requires evidence strategy before behavior changes and evidence in return-to-caller", async () => {
    ...
    expect(content).toContain("**Proof quality**")
    expect(content).toContain("public seam")
    expect(content).toContain("independent source of truth")
    expect(content).toContain("boundaries this codebase does not control")
    expect(content).toContain("one behavior slice")
