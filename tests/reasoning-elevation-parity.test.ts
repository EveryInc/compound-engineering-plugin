...
describe("reasoning-elevation engine parity", () => {
  ...
  test("the shared engine routes both stable roles through one frozen generalized snapshot", async () => {
    ...
    expect(body).toContain("resolution.compatibility")
    expect(body).toContain("resolve_batch")
    expect(body).toContain("finalize_attempt")
    expect(body).toContain("attempt_lock")
    expect(body).toContain("`ok`, `unavailable`, or `failed`")
    expect(body).toMatch(/never send a binding/i)
