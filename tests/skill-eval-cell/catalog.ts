...
Return the completion result to the coordinator.`,
  ...
  },
  {
    ...
    task: `Use the ce-compound-refresh skill to refresh this repository's learnings store. For the CONCEPTS.md vocabulary step, state exactly what you would change in that file and why. Do not write any files. End your reply with one GLOSSARY: <verb> <entry> line for every entry you considered, using exactly one verb from add, refine, fold, retire, delete, or leave. For a fold, name the destination in the same line as: fold <term> into <entry>.`,
    grade: {
