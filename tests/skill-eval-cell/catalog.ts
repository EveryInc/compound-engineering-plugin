...
Return the completion result to the coordinator.`,
  ...
  },
  {
    ...
    task: `Use the ce-compound skill. I just fixed a bug where a Delivery Run could advance to settled twice under concurrent workers - the fix compares the run's settle marker against its requested marker in src/delivery-worker.js before advancing. Do the vocabulary capture step against CONCEPTS.md only: state exactly which entries you would add, change, or leave, and why. Do not write any files. End your reply with one GLOSSARY: <verb> <entry> line for every entry you considered, using exactly one verb from add, refine, fold, retire, delete, or leave. For a fold, write the verb and destination together as: GLOSSARY: fold into <entry> - <term>.`,
    grade: {
      must_include: ["GLOSSARY: fold into Delivery Run"],
      actions: "none",
    },
  },
  {
  ...
  },
  {
    ...
    task: `Use the ce-compound-refresh skill to refresh this repository's learnings store. For the CONCEPTS.md vocabulary step, state exactly what you would change in that file and why. Do not write any files. End your reply with one GLOSSARY: <verb> <entry> line for every entry you considered, using exactly one verb from add, refine, fold, retire, delete, or leave. For a fold, write the verb and destination together as: GLOSSARY: fold into <entry> - <term>.`,
    grade: {
  ...
  },
  {
    ...
    baseline_ref: GLOSSARY_RETENTION_BASE_REF,
    key_behavior: "judgment",
    // Containment is a claim about what was NOT edited, and no prose grade can assert
    // an absence. The cell therefore writes, and the grade reads the resulting file.
    read_only: false,
    git_init: true,
    fixture: `${FIX}/concepts-refresh`,
    ...
    task: `Use the ce-compound-refresh skill to refresh only the delivery learnings in this repository. Apply the CONCEPTS.md vocabulary step to the file, then state what you changed and why. End your reply with one GLOSSARY: <verb> <entry> line for every entry you considered, using exactly one verb from add, refine, fold, retire, delete, or leave. For a fold, write the verb and destination together as: GLOSSARY: fold into <entry> - <term>.`,
    grade: {
      must_include: ["GLOSSARY: retire Suppression"],
      // The billing entries must survive byte-for-byte: a scoped run never saw them.
      workspace_contains: [
        { path: "CONCEPTS.md", needle: "The org-specific percentage added on top of a vendor's raw cost when billing." },
        { path: "CONCEPTS.md", needle: "Periods close on the first of the following month and never reopen." },
      ],
    },
  },
  {
