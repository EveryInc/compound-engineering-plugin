...
- U2. test/greet.test.js: add both cases using node:test.`,
  ...
  {
    id: "ce-plan/medium-feature-stays-durable",
    baseline_ref: RIGHT_SIZE_BASE_REF,
    skill: "ce-plan",
    cohort: "resized",
    key_behavior: "judgment",
    ...
    fixture: `${FIX}/tiny-lib`,
    timeout_secs: 900,
    why: "A multi-file feature with real design decisions is Durable: the plan file is written and the handoff menu is presented, exactly as before the gate existed.",
    pre_contract: "Write the plan file, run the confidence check and document review, then present the Phase 5.4 menu.",
    task: `Use ce-plan: add a CLI entrypoint bin/greet.js that prints greet(process.argv[2]), a --json flag that prints {"greeting": ...} instead, a config file that sets the default greeting word and is read by both paths, and tests for each behavior.`,
    grade: {
      must_include: ["docs/plans", "What would you like to do next"],
      git: "dirty",
    },
  },
  {
    id: "ce-brainstorm/standard-scope-writes-file",
    baseline_ref: RIGHT_SIZE_BASE_REF,
    skill: "ce-brainstorm",
    cohort: "resized",
    key_behavior: "judgment",
    ...
    fixture: `${FIX}/tiny-lib`,
    timeout_secs: 900,
    why: "Standard-tier scope with several decisions still earns the requirements-only plan file; the Lightweight chat default must not leak upward.",
    pre_contract: "Standard scope writes a requirements-only unified plan under docs/plans after the scoping synthesis.",
    task: `Use ce-brainstorm: greet should support localization — multiple languages, pluralized greetings, a fallback chain when a language is missing, and a way for callers to register new languages at runtime. Assume the obvious defaults for anything you would otherwise ask me, confirm the scope, and write it up.`,
    grade: {
      must_include: ["requirements-only"],
      git: "dirty",
    },
  ...
  {
    id: "ce-work/behavior-fix-keeps-review",
    baseline_ref: RIGHT_SIZE_BASE_REF,
    skill: "ce-work",
    cohort: "resized",
    key_behavior: "mutation",
    ...
    fixture: `${FIX}/tiny-lib`,
    timeout_secs: 900,
    why: "A behavior-bearing one-file fix is not mechanical: it is committed, reviewed through ce-code-review, and the shipping handoff keeps the default watch (no babysit:off).",
    pre_contract: "Behavior-bearing edits are reviewed; only a purely mechanical diff skips review.",
    task: `Use ce-work: greet should trim leading and trailing whitespace from the name before formatting. Ship it.`,
    grade: {
      committed_must: ["src/greet.js"],
      must_include: ["ce-code-review"],
