...
describe("ce-work cross-model engine contract", () => {
  ...
  test("defines an executable serial external-unit transaction before any parallel protocol", async () => {
    ...
    expect(serial).toContain("Missing, substituted, changed, forged, or incomplete confinement/executable/supervisor evidence blocks advancement")
    ...
    expect(serial).toContain("Call `prepare` without an auth manifest")
    expect(serial).toContain("credential-free or uses authentication brokered outside the model process's readable filesystem domain")
    expect(serial).toContain("exact staged-secret bytes found in workspace or controller results block terminalization and transport")
