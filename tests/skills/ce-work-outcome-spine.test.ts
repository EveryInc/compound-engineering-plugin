...
describe("ce-work out-of-repo unit completion (#1574)", () => {
  test("implementation loop does not treat a clean tree as not-started for external deliverables", async () => {
    ...
    expect(loop).toContain("mark complete when that state is already satisfied")
    expect(loop).toContain("execute when it is positively unsatisfied")
    expect(loop).toContain("ask or block when the state is unknown and execution is unsafe")
    const docs = await readRepoFile("docs/guides/ce-work.md")
    ...
    expect(docs).toContain("execute when it is positively unsatisfied")
  })
})
