...
describe("ce-work cross-model engine contract", () => {
  ...
  test("defines an executable serial external-unit transaction before any parallel protocol", async () => {
    ...
    expect(serial).toContain("introduced precious artifact")
    expect(serial).toContain("disclosed regenerable divergence")
    expect(serial).toContain("owner-attributed repair action")
    ...
    expect(serial).toContain("captures precious artifacts exactly")
    expect(serial).toContain("detect-only stat manifest for regenerable roots")
    expect(serial).toContain("proves precious and semantic restoration")
    expect(serial).toContain("never runs that argv or claims regenerable restoration")
